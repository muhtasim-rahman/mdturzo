...
// NAVBAR — v2.2.5
// Tasks completed:
//   TASK 1: navbar-inner and nf-inner use max-width 1120px + clamp padding
//   TASK 2: transparent bg only on '/' and '/home'; floating nav always glass
//   TASK 4: desktop search popup REMOVED; search icon → toast.info; sidebar search no results
//   TASK 5: CSS ::after tooltips on all 4 icon buttons (data-tooltip); glass bg on all icon btns
//   TASK 6: glass pill hover on top+floating nav links; mega nav item hover with accent border
//   TASK 7: mega panel ALWAYS has .mega-glass class (not conditional on floating)
// ============================================================
...
// ── Theme Toggle — Task 5: glass icon btn, data-tooltip="Toggle theme" ──
...
// ── Icon Button — Task 5: glass bg, CSS tooltip via data-tooltip ─────────
...
// ── Mega Menu — Task 7: always .mega-glass (not conditional) ─────────────
function MegaMenu({ onClose, isLoggedIn }) {
  ...
  // Task 6b: mega nav item hover with accent border
    ...
    <AnimatePresence>
      <motion.div variants={megaVariants} initial="hidden" animate="visible" exit="exit"
        ...
        <div className="max-w-[1120px] mx-auto px-4 pt-2">
          {/* Task 7: always mega-glass — no conditional on floating */}
...
// ── Nav Right Icons ───────────────────────────────────────────
// Task 4: Desktop search → toast.info; Task 5: glass icon btns
function NavRight({ user, profile, isAdmin: isAdminProp, avatar, displayName, isLoggedIn, authLoading, unreadCount, openSearch, notifOpen, setNotifOpen, userOpen, setUserOpen, megaOpen, setMegaOpen, onMenuOpen, onMobileSearch }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-shrink-0" data-nav-right>
      ...
      {/* Task 5: "More" tooltip as per spec */}
...
export function Navbar() {
  ...
  // Task 2: transparent top navbar ONLY on '/' and '/home'
  // Floating navbar always keeps glass — never transparent
  ...
  // Mobile/tablet search: open sidebar and focus search input (Task 4c)
  ...
  // Desktop search: toast.info (Task 4b) — no popup
  ...
  // Task 6a: top nav link — glass pill hover via .top-nav-link class (defined in index.css)
  ...
  return (
    <>
      {/* ╔══ TOP NAVBAR ═══════════════════════════════════════╗ */}
      {/* Task 2: transparent bg only on home; border transparent on home */}
      <nav
        ...
        <div className="navbar-inner flex items-center h-full gap-5">
          ...
          {/* Center nav — desktop: Task 6a glass pill hover */}
      ...
      {/* ╔══ FLOATING NAVBAR — always glass (Task 2) ══════════╗ */}
      <AnimatePresence>
        {floatVisible && (
          <motion.div
            ...
            {/* Task 1: nf-inner max-width 1120px + clamp padding */}
            <nav className="float-nav nf-inner relative pointer-events-auto flex items-center gap-4 w-full h-[52px] px-4 rounded-full">
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside key="sidebar" variants={sidebarVariants} initial="closed" animate="open" exit="closed"
              ...
              {/* Task 4c: Beautiful sidebar search bar
                  - Full container glow on focus (not just input border)
                  - × clear button appears when typing
                  - placeholder="Search..."
                  - No results shown — no function yet
               */}
              ...
              {/* Main scrollable area — always shows nav (no results) */}
      ...
      <style>{`
        ...
        /* Bug fix v2.2.6: float-nav — overflow:visible so tooltips are not clipped */
        .float-nav {
        ...
        [data-theme="light"] .float-nav {
          background: rgba(224,232,244,0.90);
          border: 1px solid rgba(180,196,216,0.85);
          box-shadow:
        ...
        /* Bug fix v2.2.6: float-nav right icon buttons — rounded-full + glass */
        ...
        /* Bug fix v2.2.6: float-nav tooltips — show below pill, not clipped */
        ...
        /* Ensure float-nav-right tooltips use same z-index above the pill */
        .float-nav-right [data-tooltip]::after,
        .float-nav-right [data-tooltip]::before {
