// GithubActivity.jsx -- v2.3.6 (renamed from GithubStats.jsx)
// CHANGES (v2.3.6):
//   * Fixed dead herokuapp.com streak-stats domain -> streak-stats.demolab.com
//   * Fixed light-mode invisible text (sideNums/currStreakNum/sideLabels/dates
//     were hardcoded near-white regardless of theme)
//   * Added onError fallback for both stat images (failed external embed no
//     longer shows a broken-image icon -- shows a small neutral placeholder)
// CHANGES (v2.3.4):
...
export default function GithubActivity() {
  ...
  const [imgErr,  setImgErr ] = useState({ streak: false, awesome: false })
  ...
  // v2.3.6 fix: herokuapp.com free dynos were discontinued — that domain is dead.
  // Migrated to the actively-maintained demolab.com fork. Also fixed sideNums/
  // currStreakNum/sideLabels/dates being hardcoded near-white (invisible in light mode).
  const streakUrl = `https://streak-stats.demolab.com/?user=${GH_USER}&hide_border=true&theme=${STREAK_THEME}&stroke=3B82F6&ring=3B82F6&fire=f97316&sideLabels=${dark ? '94a3b8' : '374151'}&currStreakLabel=3B82F6&dates=${dark ? '64748b' : '4B5563'}&background=${dark ? '0F172A' : 'F7F9FC'}&border=1E293B&sideNums=${dark ? 'F8FAFC' : '0F172A'}&currStreakNum=${dark ? 'F8FAFC' : '0F172A'}`
  ...
  return (
    <section className="section section-alt" id="github">
      <div className="container-xl">
        ...
        {!loading && isRateLimited && !profile && (
          <div className="flex flex-col items-center py-14 gap-3 text-center">
            <FontAwesomeIcon icon={faGithub} className="text-3xl text-[var(--text-tertiary)] opacity-40" />
            <p className="text-[var(--text-secondary)] text-sm max-w-xs">
              GitHub API rate limit reached. This resets every hour.
            ...
            <button onClick={load}
              className="mt-1 inline-flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--accent-primary)] transition-all active:scale-[.97]">
                ...
                <div className="gh-img-wrap">
                  {imgErr.streak ? (
                    <div className="gh-img-fallback">
                      ...
                      <span>Streak stats unavailable</span>
                    </div>
                  ) : (
                    <img
                      src={streakUrl}
                      ...
                      loading="lazy"
                      key={`streak-${dark}`}
                      onError={() => setImgErr(s => ({ ...s, streak: true }))}
                    />
                  )}
                </div>
              </div>
              <div className="gh-img-panel">
                ...
                <div className="gh-img-wrap">
                  {imgErr.awesome ? (
                    <div className="gh-img-fallback">
                      <FontAwesomeIcon icon={faGithub} />
                      <span>Profile grade unavailable</span>
                    </div>
                  ) : (
                    <img
                      src={awesomeUrl}
                      ...
                      loading="lazy"
                      key={`awesome-${dark}`}
                      onError={() => setImgErr(s => ({ ...s, awesome: true }))}
                    />
                  )}
                </div>
              </div>
      ...
      <style>{`
        ...
        .gh-img-fallback svg { font-size: 1.1rem; opacity: .5; }
