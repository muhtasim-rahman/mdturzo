// ============================================================
// AboutValues.jsx — v2.3.5
// CHANGES:
//   * Hobbies chips: data-click-fx-ignore so only parent card fires click effect
//   * Mobile (≤560px): card layout → icon left + right column (title row 1, desc row 2)
// ============================================================
  ...
  return (
    <section className="section section-alt" id="about-values">
      <div className="container-xl">
        ...
        <div className="abv-grid">
          {VALUES.map(({ icon, color, title, desc }, i) => (
            <motion.div key={title}
              ...
              {/* Wrap title+desc in div so mobile row layout stacks them vertically */}
              <div className="abv-text-col">
        ...
        {/* Hobbies — only the outer card has click effect, chips are ignored */}
        <motion.div
      ...
      <style>{`
        ...
        /* Card — default: icon above text block */
        .abv-card {
        ...
        .abv-text-col { display: flex; flex-direction: column; }
        .abv-title {
          font-size: .9rem; font-weight: 700; color: var(--text-primary); margin-bottom: .35rem;
        }
        .abv-desc { font-size: .82rem; color: var(--text-secondary); line-height: 1.65; margin: 0; }

        /* Mobile: row layout — icon left, text col right (title top, desc below) */
        @media (max-width: 560px) {
          .abv-card {
            flex-direction: row; align-items: flex-start;
            gap: .9rem; padding: 1rem 1.1rem;
          }
          .abv-icon { margin-bottom: 0; margin-top: .05rem; }
          .abv-text-col { flex: 1; }
        }
