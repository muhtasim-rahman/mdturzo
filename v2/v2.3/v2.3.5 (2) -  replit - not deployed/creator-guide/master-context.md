...
## v2.3.5 — About Page Polish (2026-05-29)

### Summary

Targeted UX improvements across all About page sections: wider arc timeline, hero padding/badge cleanup, CV card image, click-effect isolation, skills note-card hover suppression, personal details row hover, values mobile layout, goals grid responsiveness, and a new global `.click-fx` utility class.

### Changes

**AboutTimeline.jsx — Arc improvements:**
- `RADIUS` increased 260 → 310, `arcW` increased 700 → 880 — wider spread, less empty side-space on desktop
- Dot sizes increased: `[38, 28, 20, 15]`, active dot = 44px (was 32/28/22/18/14)
- Detail card redesigned as compact — padding reduced, minimal single-section layout
- Prev/Next nav buttons extracted from inside the card and placed on left/right sides of card via `.awt-detail-outer` flex row layout
- Year labels repositioned to sit just below each dot

**AboutHero.jsx:**
- Removed floating age badge div (`.abh-badge`, absolute positioned on image — desktop only)
- `Turzo` nick badge nudged: `position: relative; top: -2px; left: -3px`
- Section `padding-top` corrected: removed extra `34px` breadcrumb compensation (breadcrumb bar was removed in v2.3.4 but padding remained). Now just `var(--navbar-h)`
- `.abh-inner` min-height updated to `calc(100dvh - var(--navbar-h))`

**AboutStory.jsx:**
- CV card top badge: replaced `<div className="ast-cv-badge">CV</div>` (text only) with `<img src="/logo.webp">` inside `.ast-cv-img-wrap` — uses the same site logo as the navbar
- Personal details rows (`.ast-info-row`): added `transition: background .15s` + `:hover { background: var(--bg-surface-2) }` — subtle highlight on hover

**AboutValues.jsx:**
- Hobbies chips (`.abv-chip`) each get `data-click-fx-ignore="true"` — App.jsx `closest()` was matching the chip first (class matches `[class*="-chip"]`), creating a double effect. Now only the outer `.abv-hobbies` div fires the burst
- Mobile card layout (`≤560px`): wrapped `<h3>` and `<p>` inside `<div className="abv-text-col">` so that in `flex-direction: row`, icon is on the left and title+desc stack correctly in a column on the right

**AboutSkills.jsx:**
- `.absk-note-card` hover CSS overridden with `!important` on `transform`, `box-shadow`, and `border-color` — the `.card` base class was applying lift+shadow on hover which looked wrong for this static info card. Click burst effect is still active

**AboutGoals.jsx:**
- Grid changed from `repeat(auto-fit, minmax(260px, 1fr))` to explicit `repeat(3, 1fr)` at `≥750px` and `1fr` below — 3 goals always in one row on desktop, each full-width stacked on mobile/tablet

**App.jsx + index.css — `.click-fx` global utility:**
- `.click-fx` added to `CLICKABLE_SELECTOR` in App.jsx — adding this class to any element enables click burst effect
- `index.css`: `.click-fx { cursor: pointer; }` with explanatory comment
- Complements existing `data-click-fx` attribute approach; both continue to work

### Files changed
- `src/components/about/AboutTimeline.jsx` — arc geometry, dot sizes, compact card, side nav buttons
- `src/components/about/AboutHero.jsx` — removed age badge, badge nudge, padding fix
- `src/components/about/AboutStory.jsx` — logo image in CV card, info-row hover
- `src/components/about/AboutValues.jsx` — chip click-fx-ignore, mobile text-col wrapper
- `src/components/about/AboutSkills.jsx` — note-card hover suppression
- `src/components/about/AboutGoals.jsx` — 3-col explicit grid
- `src/App.jsx` — `.click-fx` added to CLICKABLE_SELECTOR
- `src/index.css` — `.click-fx` utility class
- `src/config/site.config.js` — version → `v2.3.5`
- `package.json` — version → `2.3.5`
- `CHANGELOG.md` — v2.3.5 entry
- `creator-guide/master-context.md` — this update

### Decisions & notes
- **Breadcrumb padding bug (v2.3.4 leftover):** `AboutHero` had `padding-top: calc(var(--navbar-h) + 34px)` to account for the breadcrumb sticky bar. The breadcrumb was removed from `About.jsx` in v2.3.4 but the padding wasn't cleaned up, causing a visible extra gap at the hero top. Fixed by dropping the `+ 34px`.
- **Arc nav placement:** Moving prev/next buttons outside the card (`awt-detail-outer` flex row) avoids the card feeling cluttered on small widths and makes the navigation more spatial / intuitive relative to the arc.
- **CV image rationale:** Using `/logo.webp` (same image as navbar) gives the CV card a recognizable brand marker instead of a plain text badge — reinforces identity without adding a new asset.
- **`.click-fx` vs `data-click-fx`:** Both are registered in CLICKABLE_SELECTOR. Use the attribute (`data-click-fx`) for HTML elements where adding a class is awkward; use the class (`.click-fx`) for JSX elements where `className` is more natural.
- **Chip click-fx isolation:** `data-click-fx-ignore="true"` must be on the chip element itself (not a parent) because App.jsx uses `closest()` upward — the chip is matched before the parent is ever checked. Parent-only ignoring doesn't stop the chip match.