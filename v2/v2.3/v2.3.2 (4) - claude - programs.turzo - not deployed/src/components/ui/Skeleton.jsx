...
// Advanced auto-generating skeletons by layout type.
// All shapes use .sk class (wave animation from index.css).
// Improvements: shimmer uses CSS custom property for color
// control; all shapes support animationDelay staggering.
// ============================================================

// ── CSS injected once ─────────────────────────────────────────
const SKELETON_STYLE = `
  .sk {
    background: var(--sk-base, var(--bg-surface-2));
  ...
  .sk::after {
    ...
    background: linear-gradient(
      90deg,
      transparent 0%,
      var(--sk-shine, rgba(255,255,255,.06)) 50%,
      transparent 100%
    );
...
// layout: 'hero' | 'list' | 'grid' | 'detail' | 'profile' | 'admin' | 'form' | 'blank'
const LAYOUTS = {
  hero: () => (
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-center py-8">
        <div className="flex-1 space-y-5">
          ...
          <div className="flex gap-2 flex-wrap">
            {[...Array(4)].map((_, i) => (
              <SkeletonBox key={i} w="w-28" h="h-7" rounded="rounded-full" delay={i * 0.05} />
            ))}
          </div>
        </div>
        <SkeletonBox w="w-72" h="h-96" rounded="rounded-2xl" className="flex-shrink-0" />
      </div>
  ...
  // Profile layout now resembles the About page structure
  profile: () => (
    <div className="container py-10 space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start pt-6">
        <div className="flex-1 space-y-4">
          <SkeletonBox w="w-28" h="h-5" rounded="rounded-full" />
          <SkeletonBox w="w-3/4" h="h-11" delay={0.05} />
          <SkeletonBox w="w-1/2" h="h-6"  delay={0.08} />
          <SkeletonText lines={2} />
          <div className="flex gap-2 flex-wrap">
            {[...Array(4)].map((_, i) => (
          ...
          <div className="flex gap-3">
            <SkeletonBox w="w-32" h="h-10" rounded="rounded-lg" delay={0.12} />
            <SkeletonBox w="w-28" h="h-10" rounded="rounded-lg" delay={0.15} />
          </div>
        </div>
        <SkeletonBox w="w-56" h="h-72" rounded="rounded-2xl" className="flex-shrink-0" />
      </div>
      {/* Stat cards */}
      ...
      <div className="card p-5 space-y-3">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="flex gap-4">
            <SkeletonBox w="w-24" h="h-4" delay={i * 0.04} />
            <SkeletonBox w="w-40" h="h-4" delay={i * 0.06} />
          </div>
        ))}
      </div>
      {/* Skills bars */}
        ...
        {[...Array(5)].map((_, i) => (
          <div key={i} className="space-y-1.5">
            <div className="flex justify-between">
              <SkeletonBox w="w-28" h="h-4" delay={i * 0.05} />
              <SkeletonBox w="w-10" h="h-4" delay={i * 0.05} />
            </div>
            <SkeletonBox w="w-full" h="h-2" rounded="rounded-full" delay={i * 0.06} />
          </div>
        ))}
      </div>
    </div>
...
export function PageSkeleton({ layout = 'blank' }) {
  ...
  return (
    <div className="animate-in fade-in duration-300 min-h-[60vh]"
      aria-hidden aria-label="Loading...">
      <Render />
    </div>
  )
}
