// Hero.jsx -- v2.2.9
// CHANGES:
//   * Large screen: "Muhtasim Rahman" and "(Turzo)" split to two lines
//     "(Turzo)" absolute-positioned right-aligned under "Rahman"
//   * Large screen: explicit min-height set via CSS
//   * "Muhtasim Rahman" in single tag, uniform white; "(Turzo)" uses primary color
//   * Hero image bottom gradient starts 5px below image bottom (gap fix)
//   * `--` replaced with `—` (em dash) throughout
//   * Mobile: "Download" text hidden, both CTA buttons in same row
//   * Greeting waving_hand: light mode color fixed
//   * hero-inner max-width unified with container-xl (1120px) for alignment
...
// Minimal rising particles -- subtle chars
const PT = [
...
// Subtle twinkle stars for texture
...
.hero-inner{
  ...
  /* FIX v2.3.2: reduced excessive navbar padding (was clamp(3rem,8vh,5rem)) */
  padding-top:calc(var(--navbar-h) + clamp(1.25rem,2.5vh,2rem));
  padding-bottom:clamp(3rem,6vh,5rem);
  min-height:100dvh;
}

/* Large screen: set explicit height so hero fills viewport nicely */
...
.hgreet-salam{font-family:'Georgia',serif;font-style:italic;font-weight:600;font-size:.88rem}
/* dark mode: amber; light mode: blue accent for better contrast */
[data-theme=dark]  .hgreet-salam{color:#F59E0B}
[data-theme=light] .hgreet-salam{color:var(--accent-primary)}
.hgreet-wave{
  font-size:1.1em;vertical-align:middle;line-height:1;
  display:inline-flex;align-items:center;
}
[data-theme=dark]  .hgreet-wave{color:#F59E0B}
[data-theme=light] .hgreet-wave{color:var(--accent-primary)}
...
/* Full name — white/primary on dark, primary-dark on light */
.hname-full{color:var(--text-primary)}

/* (Turzo) — primary accent color on all themes */
.hname-turzo-block{
  display:block;
  font-size:.38em;
  font-weight:600;
...
/* Large screen: Turzo positioned absolute, right-aligned under "Rahman" */
@media(min-width:900px){
  .hname-wrap{position:relative;display:inline-block;}
  .hname-turzo-block{
    display:block;
    position:absolute;
    top:calc(100% + .08em);
    ...
    white-space:nowrap;
    opacity:0;
    animation:hfade .5s ease 1.1s forwards;
  }
}
/* Small/mid screen: normal block under name */
@media(max-width:899px){
  .hname-wrap{display:inline;}
  .hname-turzo-block{
    display:block;
    position:static;
    font-size:.38em;
...
/* cta buttons */
.hcta{display:flex;gap:.7rem;flex-wrap:nowrap;align-items:center}
...
.hbtn-p:active,.hbtn-o:active{transform:scale(.94)!important;box-shadow:0 1px 4px rgba(37,99,235,.18)!important}
...
/* Mobile: hide "Download" text in Download CV button */
@media(max-width:479px){
  .hbtn-dl-text{display:none}
}
...
.hsocial:active{transform:scale(.9)!important}
...
.hvisual{
  ...
  padding-right:clamp(0px,1.5vw,1rem);
  opacity:0;
...
/* Image container -- fixed size per breakpoint */
.himg-box{
  position:relative;
  width:clamp(280px,32vw,440px);
  height:clamp(375px,40vw,590px);
  flex-shrink:0;
}
...
/* Bottom fade gradient — starts 5px below image bottom to close zoom gap */
.himg-frame::after{
...
@media(min-width:1280px){
  .hero-inner{grid-template-columns:1.1fr 1fr;max-width:1120px}
  .himg-box{width:420px;height:570px}
}

/* lg: 1024-1279 */
@media(min-width:1024px) and (max-width:1279px){
  .hero-inner{grid-template-columns:1.1fr 1fr;gap:2rem}
  .himg-box{width:360px;height:490px}
}

/* md-lg: 900-1023 */
@media(min-width:900px) and (max-width:1023px){
  .hero-inner{grid-template-columns:1fr 1fr;gap:1.5rem}
  .himg-box{width:300px;height:420px}
}

/* md: <900 — single column, image on top */
@media(max-width:899px){
  ...
  .hvisual{order:1;height:auto;justify-content:center;padding-right:0}
...
/* xs: <480 */
@media(max-width:479px){
  .hero-inner{padding-top:calc(var(--navbar-h) + 1.25rem);padding-bottom:2.5rem;gap:1.5rem}
  .himg-box{width:clamp(160px,42vw,200px);height:clamp(160px,42vw,200px)}
  .hname{font-size:clamp(1.9rem,9vw,2.4rem)}
  .hcta{flex-wrap:nowrap;gap:.5rem}
  .hbtn{padding:.52rem .9rem;font-size:.8rem}
}
        ...
        <div className="hero-inner">
          ...
          <div className="hcontent">
            <div className="hgreet hup" style={{'--d':'0ms'}}>
              ...
              <span
                className="material-symbols-outlined hgreet-wave"
                aria-hidden="true">
                waving_hand
              </span>
            ...
            <h1 className="hname" aria-label="Muhtasim Rahman (Turzo)">
              <span className="hname-line">
                <span className="hname-in hname-wrap" style={{'--d':'70ms'}}>
                  <span className="hname-full">Muhtasim Rahman</span>
                  <span className="hname-turzo-block">(Turzo)</span>
                </span>
              </span>
            </h1>
              ...
              <span className="hbtn-wrap">
                {cvEnabled && cvUrl && cvUrl !== '#'
                  ? <a href={cvUrl} target="_blank" rel="noopener noreferrer" className="hbtn hbtn-o">
                      ...
                      <span className="hbtn-dl-text"> Download</span> CV
                    </a>
