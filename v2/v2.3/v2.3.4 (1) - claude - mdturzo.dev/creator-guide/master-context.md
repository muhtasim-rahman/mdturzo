...
## v2.3.4 — Navbar Transparent About, Breadcrumb Strip, Hero Redesign, Story Redesign, Wheel Timeline, Connect Hover, GitHub Fix (2026-05-29)

### Changes

**Navbar.jsx:** Added `/about` to `isHomePage` array → navbar is now transparent on the About page, same as Home. (was: `['/', '/home']`, now: `['/', '/home', '/about']`)

**About.jsx:** 
- Added `padding-top: var(--navbar-h)` to `.ab-page` to push all content below the transparent navbar.
- Added sticky breadcrumb strip as first child: `position: sticky; top: var(--navbar-h); z-index: 30; height: 36px`. Full width, backdrop-blur background, border-bottom. Shows "Home › About" with proper link.
- Removed breadcrumb from AboutHero (it's now in this strip).

**AboutHero.jsx — Full redesign (editorial layout):**
- Removed breadcrumb `ab-bc` (moved to About.jsx strip).
- Layout: CSS grid 1fr/1fr. Left: eyebrow, h1, role chip, bio, fact pills (2×2), CTAs, age+location footer. Right: editorial-style full-column image (`hero-back.webp`).
- Image frame: same `object-fit: cover; object-position: center top` as home hero. Has `abh-img-fade` (bottom-to-top gradient, same as home's `himg-frame::after`). NO rounded frame anywhere (not on mobile, not on tablet, not on desktop).
- `abh-right::before`: left-edge gradient fades image into left column background on desktop.
- `abh-section-grad`: full-section bottom gradient (covers entire width, same concept as home's `hero-grad-btm`), z-index: 3.
- Left content: z-index 6 (above gradients).
- `min-height: calc(100dvh - var(--navbar-h) - 36px)` (accounts for ab-page padding-top + breadcrumb strip height).
- Mobile: right image stacks above, bottom gradient on right column, left content below, text centered.

**AboutStory.jsx — Complete redesign (new title "Background"):**
- New section header: label "Background", h2 "The person behind the projects", subtitle.
- Added **stats row** at top (4-col grid): Years Dev, Years Design, Years Video, Projects — each as a pill with icon+value+label. Responsive: 2-col on mobile.
- Two-column grid: left = personal info card + CV card + quote. Right = journey milestones (numbered cards with icon).
- Journey cards: numbered badge (01/02/03) + icon row + title + text. Clean and readable.
- CV card: 4-button grid (Preview / Download / Print / Share) — 2-col on mobile.
- Stats moved to top row (was inline in right column).

**AboutTimeline.jsx — Wheel/Arc Timeline:**
- Desktop: semicircular arc with 7 visible dots (selected = center top, ±1/2/3 on sides). Dot size/opacity decreases with distance from center. Animated pulsing ring on active dot. Click any dot to navigate. SVG dashed arc guide. Vertical connector (colored line + dot) below arc leads to detail card. Detail card shows: period, badge (current/upcoming), icon, school, level, description, prev/next navigation. AnimatePresence for smooth transitions.
- Mobile (< 640px): horizontal scrollable strip of year chips (icon + short year label), scroll-snapping. Below: detail card with same content. Both have prev/next arrows.
- Tablet (640–780px): arc scaled to 80% via CSS transform.
- Arc geometry: RADIUS=260, MAX_ANGLE=60°. Items at offset [-3..+3] positioned with sin/cos. Arc container height ≈ 120px.

**AboutSkills.jsx — absk-tabs mobile fix:**
- Changed `flex-wrap: wrap` → `flex-wrap: nowrap` + `overflow-x: auto; scrollbar-width: none`.
- Added `flex-shrink: 0; white-space: nowrap` to tabs.
- Mobile (≤480px): tabs container is `width: 100%`, tab padding slightly reduced.

**AboutValues.jsx — Hobbies click effect:**
- Added `data-click-fx` attribute to `.abv-hobbies` container div → App.jsx ripple now fires on the entire container.
- Added `cursor: pointer`, hover border + shadow transition to `.abv-hobbies` CSS.

**AboutGoals.jsx — Remove abg-topbar:**
- Removed `<div className="abg-topbar">` element.
- Removed `.abg-topbar` CSS rule.
- Added `border-left: 3px solid var(--item-color)` to `.abg-card` as replacement color accent.
- Added `--item-color: {color}` style prop to each card's motion.div.
- Added hover box-shadow to cards.

**GithubStats.jsx — Remove retry loop:**
- Removed retry loop entirely. `load()` function no longer takes `attempt` param.
- Removed `retryCount`, `retryTimer` state.
- On rate limit (403/429): sets `error: 'rate_limited'`, shows clean message + manual Retry button.
- `useEffect` cleanup removed (no timers to clean up).
- See **GitHub caching plan** below for next version fix.

**AboutConnect.jsx — Updated hover effect:**
- `::before` now does a shimmer sweep (translateX -100% → +100%) on hover.
- `::after` (new) is the left accent bar (scaleY transition from center).
- Background tints to platform color on hover (4% mix).
- `.abc-name` color transitions to platform color on hover.
- `.abc-icon-wrap` scales up slightly (1.08) on hover + gets color tint.
- `.abc-arrow` offset increased: `translate(2px, -2px)`.
- Icon wrap size: 36px (was 34px).

### Files changed
- `src/components/layout/Navbar.jsx` — isHomePage: added `/about`
- `src/pages/About.jsx` — ab-page padding-top, sticky breadcrumb strip
- `src/components/about/AboutHero.jsx` — full editorial redesign
- `src/components/about/AboutStory.jsx` — full redesign "Background" theme
- `src/components/about/AboutTimeline.jsx` — wheel/arc timeline
- `src/components/about/AboutSkills.jsx` — absk-tabs mobile scroll fix
- `src/components/about/AboutValues.jsx` — hobbies container click effect
- `src/components/about/AboutGoals.jsx` — removed abg-topbar, added left border accent
- `src/components/home/GithubStats.jsx` — retry loop removed, single fetch
- `src/components/about/AboutConnect.jsx` — updated hover effect
- `creator-guide/master-context.md` — this update

---

## 📌 PLAN FOR NEXT VERSION: GitHub Data Caching (important)

**Problem:** GitHub REST API has a 60 req/hr unauthenticated limit. Homepage loads currently hit this limit quickly (each page load = 2+ API calls + language calls per repo).

**Solution (for next version — needs GitHub token + backend function):**

### Step 1: GitHub Personal Access Token (PAT)
- Create a fine-grained PAT on GitHub with read-only access to public repos.
- Grants 5000 req/hr (vs 60). Token should NOT be exposed in client-side code.

### Step 2: Edge/Serverless Function (backend cache)
Options (all free tier):
- **Cloudflare Workers** (recommended): 100K req/day free. Store cached data in KV.
- **Supabase Edge Functions**: 500K invocations/month free. Store in Supabase DB.
- **Firebase Cloud Functions**: 125K invocations/month free. Store in Firestore.

### Step 3: Cache refresh schedule
- Function fetches GitHub API (with token) once every 24-48 hours.
- Stores result as JSON in KV/DB.
- All website visits read from the cached JSON — never hit GitHub API directly.
- Fallback: if cache miss or stale, fetch live (with token).

### Step 4: Client-side change
- Replace `fetch('https://api.github.com/...')` with `fetch('https://your-worker.workers.dev/github-stats')`.
- The worker returns the cached data.
- Rate limit errors disappear entirely (users hit YOUR worker, not GitHub directly).

### Implementation notes for next AI
- Recommended: Cloudflare Workers + KV (zero config, free, fast globally)
- Worker URL will be something like: `https://mdturzo-github-stats.YOUR-SUBDOMAIN.workers.dev/`
- The worker needs CORS headers for the Firebase hosting domain.
- `GithubStats.jsx` just needs the base URL changed + minor error handling update.
- Token stored as Cloudflare Worker secret (never in source code).
