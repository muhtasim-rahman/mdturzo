...
## v2.3.2 - About Restructure, Timeline, Click UX, Skeleton Update

### Summary

v2.3.2 completed the requested About page refinement and project-structure cleanup. The previous single-file About page was replaced with a route-level composer and separate section files under `src/components/about/`, while preserving the intended About flow: hero, story/details, academic timeline, skills/languages, values, goals, connect, and shared CTA.

### About Page Changes

**Story/details:** rebuilt after the hero with a clearer personal narrative, quick information list, and CV action controls for download, preview, print, and share. It uses `SITE_CONFIG.owner` and `calculateAge()` so name, age, email, and location stay centralized.

**Academic Timeline:** updated to keep the scroll progress bar, activate timeline dots as the progress reaches each milestone, and show small icons inside each dot. Mobile now collapses to a left-line layout with safer spacing.

**What I Know:** all tabs now follow the same programming-style layout with animated progress bars. Programming, design, video, and tools use consistent row structure, percentage labels, notes, and animation timing. Language proficiency uses the same bar animation style.

**Values & Personality:** layout is explicitly 3 cards per row on desktop, 2 on tablet, and 1 compact card per row on mobile.

**Goals:** rebuilt from the supplied copy-4 direction and added requested progress bars: Short-Term 85%, Mid-Term 50%, Long-Term 25%.

**Connect:** rebuilt from the copy-2 direction with a cleaner responsive card grid and better light/dark theme behavior.
...
- `src/components/about/AcademicTimeline.jsx`
- `src/components/about/AboutKnowledge.jsx`
- `src/components/about/AboutValues.jsx`
- `src/components/about/AboutGoals.jsx`
- `src/components/about/AboutConnect.jsx`
- `src/components/about/AboutSectionHeader.jsx`
- `src/components/about/aboutData.js`
- `src/components/about/about.css`

**Updated route file:** `src/pages/About.jsx` is now a slim page composer with SEO, tracking, settings, and section imports only.

### Shared UX Changes

**Hero spacing:** Home and About hero top spacing was tightened so the fixed navbar offset no longer feels doubled.

**Click effect:** global click feedback in `App.jsx` and `index.css` is now more minimal and applies to buttons, button-like anchors, cards, badges, and key chips while avoiding plain text links.

**Same-page navigation:** clicking an internal link to the page the visitor is already on now scrolls to the top smoothly without reload or route churn.

**Skeleton system:** `src/components/ui/Skeleton.jsx` now includes `AutoSkeleton`, a reusable blueprint-based skeleton generator for sections, hero-like blocks, content grids, cards, and actions. No new dependency was added because the local skeleton primitives already support the needed behavior.

### Version Metadata

**Updated version files:**
- `src/config/site.config.js` -> `v2.3.2`
- `package.json` -> `2.3.2`
- `package-lock.json` -> `2.3.2`
- `CHANGELOG.md` -> v2.3.2 notes added

### Notes For Future Work

The context file should be normalized more deeply in a future documentation-only pass if a full historical heading rewrite is needed. For this release, no old historical information was removed or rewritten destructively.
