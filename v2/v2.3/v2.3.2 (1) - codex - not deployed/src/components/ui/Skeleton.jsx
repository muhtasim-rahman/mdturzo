...
const AUTO_BLUEPRINTS = {
  section: [
    { kind: 'line', w: 'w-28', h: 'h-4', rounded: 'rounded-full' },
    { kind: 'line', w: 'w-2/3', h: 'h-8' },
    { kind: 'text', lines: 2 },
    { kind: 'cards', count: 3 },
  ],
  heroSection: [
    { kind: 'line', w: 'w-24', h: 'h-4', rounded: 'rounded-full' },
    { kind: 'line', w: 'w-5/6', h: 'h-12' },
    { kind: 'line', w: 'w-3/5', h: 'h-12' },
    { kind: 'text', lines: 2 },
    { kind: 'actions', count: 2 },
  ],
  contentGrid: [
    { kind: 'cards', count: 4 },
  ],
...
function AutoSkeletonBlock({ block, index }) {
  if (block.kind === 'text') return <SkeletonText lines={block.lines || 3} />
  if (block.kind === 'actions') {
    return (
      <div className="flex flex-wrap gap-3">
        {Array.from({ length: block.count || 2 }, (_, i) => (
          <SkeletonBox key={i} w={i === 0 ? 'w-32' : 'w-28'} h="h-10" rounded="rounded-xl" delay={(index + i) * 0.04} />
        ))}
      </div>
    )
  }
  if (block.kind === 'cards') {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {Array.from({ length: block.count || 3 }, (_, i) => (
          <div key={i} className="card p-4 space-y-3">
            <SkeletonBox w="w-10" h="h-10" rounded="rounded-xl" delay={(index + i) * 0.04} />
            <SkeletonBox w="w-2/3" h="h-5" delay={(index + i) * 0.05} />
            <SkeletonText lines={2} />
          </div>
        ))}
      </div>
  ...
  return <SkeletonBox w={block.w || 'w-full'} h={block.h || 'h-4'} rounded={block.rounded || 'rounded'} delay={index * 0.04} />
}

export function AutoSkeleton({ blueprint = 'section', className = '' }) {
  const blocks = Array.isArray(blueprint) ? blueprint : (AUTO_BLUEPRINTS[blueprint] || AUTO_BLUEPRINTS.section)
  return (
    <div className={`space-y-5 ${className}`}>
      {blocks.map((block, index) => (
        <AutoSkeletonBlock key={`${block.kind}-${index}`} block={block} index={index} />
      ))}
    </div>
  )
}
