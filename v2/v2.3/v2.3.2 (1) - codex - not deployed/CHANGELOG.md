...
### Added
- `src/components/about/` section architecture for About hero, story, timeline, skills/languages, values, goals, connect, shared data, and scoped CSS.
- CV actions in the About story section: download, preview, print, and share.
- Goal progress bars: Short-Term 85%, Mid-Term 50%, Long-Term 25%.
- Auto skeleton primitives via `AutoSkeleton` for reusable generated loading layouts.

### Changed
- `src/pages/About.jsx` is now a slim route composer instead of one large monolithic file.
- Home and About hero top spacing was tightened to avoid double navbar padding.
- Academic timeline dots now activate as the scroll progress reaches each milestone.
- All "What I Know" tabs now use the same animated progress-bar layout.
- Global click feedback is more minimal and applies to buttons, button-like links, cards, badges, and important chips.
- Same-page internal route clicks now scroll to the top without reload or route churn.
- Version metadata bumped to `v2.3.2`.
