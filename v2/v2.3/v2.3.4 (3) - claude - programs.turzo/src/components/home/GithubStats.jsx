...
export default function GithubStats() {
  ...
  // Single API call on mount — no retry loop.
  // v2.3.4 note: retry/loop system removed to prevent rapid API rate limit exhaustion.
  // Future plan (v2.4.x): cache GitHub data server-side (Firebase/Cloudflare/Supabase free fn).
  // Refresh every 24-48h. Website reads cached data — never calls GitHub API from browser directly.
  // GitHub token lives in server env (never exposed to client).
  useEffect(() => {
    let cancelled = false
    const load = async () => {
