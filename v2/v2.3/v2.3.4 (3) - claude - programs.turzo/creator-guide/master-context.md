...
## v2.3.4 — About Page Polish + GitHub Fix + Navbar/Connect Updates (2026-05-29)

### Changes summary

**Navbar:** `/about` added to `isHomePage` array — About page now gets transparent/absolute navbar same as Home (no border, no background, no backdrop-filter at page top).

**AboutBreadcrumb (NEW — `src/components/about/AboutBreadcrumb.jsx`):**
Sticky full-width bar `(height: 36px)` that sits immediately below the transparent navbar on the About page. Shows `Home > About` with FontAwesome icons. `position: sticky; top: var(--navbar-h)`. Always visible including mobile. Semi-transparent bg with blur. Imported and rendered as first child inside `ab-page` wrapper in `About.jsx`. `ab-page` div gets `padding-top: var(--navbar-h)` so breadcrumb starts right below the absolute navbar.

**AboutHero — full redesign:**
- Completely rebuilt. New 2-col layout: left = stacked identity (eyebrow, h1, role-chip, bio, 2×2 fact pills, CTA buttons), right = `hero-back.webp` image.
- Image: NO rounded frame on any screen size (mobile too). `object-fit: cover`, `object-position: center top`, natural edges.
- Gradient: full-section bottom-to-top gradient (`ab-hero-grad` div) — same style as home `hero-grad-btm`. Covers bottom 52% of section. Applied via absolute positioned div, NOT on the image itself. Left content sits above gradient (`z-index: 5/6`).
- Responsive: single column on ≤860px (image on top, content below), text-align center.
- Breadcrumb removed from this component (now in `AboutBreadcrumb.jsx`).
- `min-height: calc(100dvh - var(--navbar-h, 68px) - 36px)`.

**AboutStory — full redesign ("Who I Am"):**
- Title changed from "My Story" → "Who I Am".
- Headline: "From circuits to clean code".
- Stats bar: 4-col grid (Years Dev, Years Design, Years Video, Projects) with colored icons.
- 2-col grid: left = personal info card (9 rows with icon, key, value) + CV card (preview/download/print/share); right = journey timeline (3 steps with connector line) + quote block.
- CV card: badge, name, subtitle, 4 action buttons.
- Journey: Seedling (Early Spark) → Code (Self-Taught Path) → Rocket (What's Next) with connector lines between steps.
- Clean, fresh design with minimal colors.

**AboutTimeline — Wheel Timeline:**
- Complete redesign based on wheel/radial timeline concept.
- **PC:** SVG elliptical arc track with 8 clickable dots positioned around the arc. Each dot has a connector line to center. Active dot highlighted in its color with glow ring. Center panel shows selected entry details (icon, badges, period pill, school, location, level, description). Dot-nav row at bottom. AnimatePresence for smooth center panel transitions.
- **Mobile:** Vertical card list with left accent line. Scroll-driven line fill (blue→purple→green gradient). Dots go from muted to full color as user scrolls past each entry. Cards have colored left border that activates on scroll.
- 8 education entries: 2013–2014 Nursery → 2026 SSC (current) → Future BSc CSE.
- Mobile detection via `useEffect + window resize`.

**AboutSkills — `absk-tabs` responsive:**
- At ≤560px: `.absk-tabs` becomes a 2-column CSS Grid (was `flex; flex-wrap: wrap; width: fit-content`). Tab buttons get `justify-content: center`, smaller padding and font.

**AboutValues — Hobbies click effect:**
- `data-click-fx` added to the `.abv-hobbies` container div (was only on inner badge elements). Full hobbies section now responds to click effect.

**AboutGoals — remove `abg-topbar`:**
- `<div className="abg-topbar" style={{ background: color }} />` removed from JSX.
- `.abg-topbar` CSS rule also removed.
- Cards now start directly with `.abg-body` padding — cleaner minimal look.

**GithubStats — remove retry loop:**
- All retry state (`retryCount`, `retryTimer`), `useCallback` import, retry `setTimeout` logic, `RATE_LIMIT_WAIT` constant, and `isRateLimitedWait` removed.
- API now called **once on mount** via `useEffect` with `cancelled` flag for cleanup.
- Rate limit display simplified: "GitHub API rate limit reached. Please try again in a few minutes." + "Reload the page to try again" (no spinner, no retry button).
- `useCallback` import removed from React imports.
- **Future plan (v2.4.x):** Implement server-side GitHub data caching via Firebase/Cloudflare/Supabase free function. Refresh data every 24–48 hours. Website reads cached data on load — GitHub API is never called from browser. GitHub token stored in server env. This eliminates rate limit issues entirely.

**AboutConnect — hover effect update:**
- Card `::before` pseudo now creates background tint on hover (`opacity: 0; → 0.06`) — subtle color wash matching `--soc` platform color.
- Card `::after` is the left accent bar, now uses `transform: scaleY(0) → scaleY(1)` with `transform-origin: center` (was `bottom`).
- Card hover: `transform: translateY(-2px) scale(1.012)` + stronger `box-shadow`.
- Icon wrap hover: `transform: scale(1.1) rotate(-4deg)` — playful tilt.
- `.abc-info` and `.abc-arrow` get `position: relative; z-index: 1` so they render above the `::before` tint layer.
- Arrow: `transform: translate(2px, -2px)` on hover (was 1px).

### Files changed
- `src/components/layout/Navbar.jsx` — isHomePage array includes `/about`
- `src/pages/About.jsx` — imports AboutBreadcrumb, renders it first inside ab-page; `ab-page` gets `padding-top: var(--navbar-h)`
- `src/components/about/AboutBreadcrumb.jsx` — **NEW**
- `src/components/about/AboutHero.jsx` — full redesign
- `src/components/about/AboutStory.jsx` — full redesign
- `src/components/about/AboutTimeline.jsx` — wheel timeline redesign
- `src/components/about/AboutSkills.jsx` — absk-tabs mobile CSS
- `src/components/about/AboutValues.jsx` — data-click-fx on hobbies div
- `src/components/about/AboutGoals.jsx` — abg-topbar removed
- `src/components/about/AboutConnect.jsx` — hover effect polish
- `src/components/home/GithubStats.jsx` — retry loop removed, single call
- `package.json` — version 2.3.4
- `creator-guide/master-context.md` — this update

### Notes for next AI (v2.3.5+)
- About page breadcrumb is in `AboutBreadcrumb.jsx` — sticky below navbar via `top: var(--navbar-h)`. `ab-page` has `padding-top: var(--navbar-h)` to push breadcrumb below absolute navbar.
- AboutHero image: NO rounded frame, no border, no box-shadow frame — on any screen size. The `ab-img` is raw `object-fit: cover`.
- Gradient on AboutHero: `ab-hero-grad` absolute div, `bottom: 0; height: 52%`, `linear-gradient(to top, var(--bg-page) 0%, ..., transparent 100%)` — this is on the SECTION, not the image.
- isHomePage in Navbar includes `'/about'` — if future pages need transparent navbar, extend this array.
- GithubStats retry loop is permanently removed. The comment inside `useEffect` documents the v2.4.x caching plan.
- Wheel timeline: PC uses SVG with `arcPosition()` function placing dots on an ellipse. Center panel is `position: absolute`. Mobile uses scroll-driven vertical list. Mobile detection: `window.innerWidth < 860`.
- AboutConnect `--soc` CSS variable must be set inline on each card for the hover tint/accent to use the right platform color.
