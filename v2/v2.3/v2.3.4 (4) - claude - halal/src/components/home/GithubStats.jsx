// GithubStats.jsx -- v2.3.4
// CHANGES:
//   * API call: single call on mount — no retry loop
//   * Rate limited: show error + manual retry button only
//   * retryCount / retryTimer states removed
//   * Repos: 6 desktop, 4 tablet (≤900px), 3 mobile (≤480px)
//   * Language bar preserved
//
// CONTEXT FOR NEXT VERSION (v2.3.5+):
//   GitHub API is rate-limited to 60 req/hr for unauthenticated.
//   Plan: use a GitHub Personal Access Token (PAT) via a
//   Firebase Function / Cloudflare Worker / Supabase Edge Function
//   to proxy requests. Cache the response in Firestore or KV store.
//   Refresh cached data every 24–48 hours server-side.
//   Website fetches from cache → no API limits, no token exposed.
//   No loop retries needed; error state = manual "Retry" button only.
...
export default function GithubStats() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Rate limited — show error, let user retry manually
  ...
  useEffect(() => {
    load() // single call on mount — no auto-retry
  }, [])
