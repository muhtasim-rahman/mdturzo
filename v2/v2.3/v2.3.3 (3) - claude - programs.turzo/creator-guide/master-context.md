...
## v2.3.3 — About Page Polish & Redesign
...
- `src/components/about/AboutGoals.jsx`
- `src/components/about/AboutConnect.jsx`
- `public/icons/social/` (10 SVG icons added)
- `creator-guide/master-context.md` — this update

### Changes summary

**AboutHero:** Left content fully redesigned — editorial chapter-style layout (large decorative "01" number, breadcrumb inline, name with gradient surname, dot-separated role row, nick badge, bio with left-border accent, 2×2 fact grid instead of pills, 3-button CTA). Image-only gradient (`ab-hero-img-frame::after`) removed. Full section gradient (`ab-hero-grad`) now handles blending across entire section width — same position on mobile & PC, sits below (z-index 2) left content (z-index 5/6).

**AboutStory:** Minimal rebuild. Personal info rows use a compact 3-col grid (icon, key, value). CV card redesigned with larger icon, divider line, and 4 action buttons in a 4-column compact grid — each button is column-stacked icon+label. CV actions: Preview = new tab `target="_blank"`, Download = `download` attribute, Print = opens URL in new tab then triggers `w.print()`, Share = `navigator.share()` API with clipboard fallback. Journey items use a vertical connector line (dot + line). Experience stats replaced with compact inline badges.

**AboutTimeline:** Section heading is center-aligned (PC+mobile). Card design rebuilt: each card has a thin colored top-line accent bar, clean body with period badge + school + level + desc. Left-col cards are right-aligned on PC. Scroll threshold changed from `vh*0.72` to `vh*0.5` (fills to viewport center). Before scroll reaches an item: `opacity: 0.38`, muted dot/colors; when reached: full color with transitions. Dot also animates from muted to active. Fully responsive.

**AboutSkills:** Added shimmer sweep animation to all progress bar fills (`::after` pseudo-element) — same pattern as home Skills section (`sk2-bar-fill::after`). Added `position: relative; overflow: hidden` to `.absk-bar-fill`.

**AboutLanguages:** Flag moved to right side of each language row (small 20×15px). Percentage text below bar removed (`abl-pct` element removed). Layout compacted — tighter gap, smaller font sizes. Added shimmer to language progress bars too.

**AboutGoals:** Removed `.abg-progress-header` div from `GoalProgressBar` (was showing "Progress X%"). Now only the progress bar track remains.

**AboutConnect:** Switched from FontAwesome icons to custom uploaded SVG icons from `public/icons/social/`. For dark mode: `github.svg`, `x-twitter.svg`, `threads.svg` (all black fills) get `filter: brightness(0) invert(1)` via `.abc-svg-invert` class + `[data-theme=dark]` selector. Card hover effect changed to a left-side reveal bar (scaleY animation). Arrow icon changed to `faArrowUpRightFromSquare`.

### Decisions & notes
- SVG icon files copied to `public/icons/social/` (10 files): email, facebook, github, instagram, linkedin, telegram, threads, tiktok, x-twitter, youtube.
- `invertDark: true` flag on SOCIALS data for GitHub, X, Threads to apply CSS filter in dark mode.
- `ab-hero-grad` z-index remains 2 (same as before), confirmed to work correctly as it sits above bg texture (z-index 0) but below `.ab-hero-inner` content (z-index 5+).
- Goals progress caption removed as requested — `abg-progress-header` div and its CSS left in file but not rendered (the CSS is now unused, can be cleaned later).

### Commit message suggestion

```
feat(about): v2.3.3 — hero redesign, story rebuild, timeline polish, skills shimmer, connect SVG icons

- AboutHero: editorial left layout (chapter num, 2×2 fact grid, gradient surname);
  gradient moved to full section level — no more image-only cutoff
- AboutStory: minimal rebuild; CV buttons (preview/download/print/share) redesigned
- AboutTimeline: centered heading, new card design, scroll fills to viewport center,
  muted → active animation on scroll, fully responsive
- AboutSkills: shimmer sweep added to all progress bars (same as home skills)
- AboutLanguages: compact right side, flags moved right, percentage removed, shimmer added
- AboutGoals: removed "Progress X%" caption — progress bar only
- AboutConnect: SVG icons from /icons/social/; dark mode invert for GitHub/X/Threads
- public/icons/social/: 10 brand SVG icons added (email, facebook, github, instagram,
  linkedin, telegram, threads, tiktok, x-twitter, youtube)
```
