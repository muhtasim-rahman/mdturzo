...
export function Navbar() {
  ...
  // -- DevBanner height tracking (for floating nav top offset) ----
  useEffect(() => {
    const measure = () => {
      const el = document.querySelector('.dev-banner')
      setDevBannerH(el ? el.getBoundingClientRect().height : 0)
    }
    measure()
  ...
  // v2.5.2: All pages use transparent/absolute navbar style (same as home/about).
  // Each page component manages its own top padding via paddingTop: 'var(--navbar-h)'.
  ...
  // v2.4.8: mega nav closes on outside click only (not scroll)
  // The backdrop div below handles outside clicks reliably
  ...
  // Mega menu top offset:
  // floating navbar: devBannerH + pt-3(12px) + float-nav-h(52px) = devBannerH + 64px
  // top navbar: devBannerH + var(--navbar-h) [DevBanner is outside the inner wrapper]
  const megaTopOffset = floatVisible
    ? `${devBannerH + 64}px`
    : `calc(${devBannerH}px + var(--navbar-h))`
    ...
    <>
      ...
      <nav
        ...
        <div className="navbar-inner flex items-center h-full gap-5">
          ...
          <div className="hidden md:flex lg:hidden items-center gap-1.5 ml-auto">
            {authLoading
              ? <div className="w-20 h-8 rounded-full sk" />
              : !isLoggedIn && <SignInBtn className="h-8 text-xs px-3 py-0" />}
            <ThemeToggle size="sm" />
      ...
      <AnimatePresence>
        {floatVisible && (
          <motion.div
            ...
            className="fixed left-0 right-0 z-[200] flex justify-center px-4 pt-3 pointer-events-none"
            style={{ top: devBannerH }}>
      ...
      */}
      <AnimatePresence>
        {megaOpen && (
          <>
            {/* Full-screen backdrop — clicking outside closes. Starts below navbar. */}
            <div
              key="mega-backdrop"
