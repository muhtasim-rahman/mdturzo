...
// ── Smart View Tracking (v2.5.2) ──────────────────────────────
// Deduplication: logged-in users tracked by UID, guests by device fingerprint.
// 3-day cooldown before re-counting same viewer.
...
function getViewCacheKey(contentType, contentId) {
  return `view_${contentType}_${contentId}`
}

function shouldTrackView(contentType, contentId, userId) {
  try {
    const key = getViewCacheKey(contentType, contentId)
    const id = userId || getDeviceFingerprint()
    const stored = localStorage.getItem(key)
    if (stored) {
      const { viewers, ts } = JSON.parse(stored)
      const now = Date.now()
      const THREE_DAYS = 3 * 24 * 60 * 60 * 1000
      // Clean up old entries
      const fresh = Object.fromEntries(
        Object.entries(viewers || {}).filter(([, t]) => now - t < THREE_DAYS)
      )
      if (fresh[id] && now - fresh[id] < THREE_DAYS) {
        localStorage.setItem(key, JSON.stringify({ viewers: fresh, ts: now }))
        return false // already viewed recently
      }
      fresh[id] = now
      localStorage.setItem(key, JSON.stringify({ viewers: fresh, ts: now }))
    } else {
      localStorage.setItem(key, JSON.stringify({ viewers: { [id]: Date.now() }, ts: Date.now() }))
    }
    return true
  } catch { return true } // if localStorage fails, allow increment
}

export async function incrementProjectViews(id, userId = null) {
  if (!shouldTrackView('project', id, userId)) return
...
// ── About Page Data (v2.4.8) ───────────────────────────────
// All about page data now served from Supabase instead of aboutData.js
...
export async function getAboutSkills(category = null) {
  let query = supabase
    .from('about_skills')
    .select('*')
    .order('sort_order', { ascending: true })
...
export async function getPublishedBlogs({ limit, offset = 0, category, tag } = {}) {
  let query = supabase
    .from('blogs')
    .select(`
      id, slug, title, short_description, thumbnail_url, cover_image_url,
      reading_time, category, series, tags, status, visibility,
      pinned, seo_title, seo_description,
...
export async function getRelatedBlogs(currentSlug, category, tags = [], limit = 3) {
  let { data: catData } = await supabase
    .from('blogs')
    .select('id, slug, title, short_description, thumbnail_url, category, tags, reading_time')
    .eq('status', 'published')
    .eq('visibility', 'public')
    .eq('category', category)
    .neq('slug', currentSlug)
  ...
  if (tags?.length) {
    ...
    const existing = new Set((catData || []).map(x => x.id))
    const extra = (tagData || []).filter(x => !existing.has(x.id))
    return [...(catData || []), ...extra].slice(0, limit)
  }

  return catData || []
}

export async function incrementBlogViews(blogId, userId = null) {
  if (!shouldTrackView('blog', blogId, userId)) return
  const { error } = await supabase.rpc('increment_blog_views', { blog_id: blogId })
  if (error) console.warn('[BlogViews] Increment failed:', error.message)
}

// ── Posts (v2.5.0) ─────────────────────────────────────────

export async function getPublishedPosts({ limit, offset = 0, category, tag } = {}) {
  let query = supabase
    .from('posts')
    .select(`
      id, slug, title, content, description, embed_url, platform, thumbnail_url,
      media_items, tags, category, status, visibility,
      views_count, likes_count, dislikes_count, comments_count,
      created_at, updated_at
    `)
    .eq('status', 'published')
...
export async function getRelatedPosts(currentSlug, category, tags = [], limit = 3) {
  let { data: catData } = await supabase
    .from('posts')
    .select('id, slug, title, content, description, embed_url, platform, thumbnail_url, media_items, views_count, likes_count')
    .eq('status', 'published')
    .eq('visibility', 'public')
    .eq('category', category)
    .neq('slug', currentSlug)
...
export async function incrementPostViews(postId, userId = null) {
  if (!shouldTrackView('post', postId, userId)) return
  const { error } = await supabase.rpc('increment_post_views', { post_id: postId })
  if (error) console.warn('[PostViews] Increment failed:', error.message)
}

// ── Feed Saved / Bookmarks (v2.5.0) ────────────────────────

export async function getFeedSavedStatus(userId, contentType, contentId) {
  if (!userId) return false
  const { data, error } = await supabase
    .from('feed_saved')
    .select('id')
...
export async function toggleFeedSaved(userId, contentType, contentId) {
  const saved = await getFeedSavedStatus(userId, contentType, contentId)
  if (saved) {
    await supabase
      .from('feed_saved')
      .delete()
