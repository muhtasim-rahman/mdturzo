...
## v2.5.2 — Feed System Major Improvements (2026-06-08)

### Summary
Large-scale improvement pass across the entire Feed system. Key themes: navbar transparency everywhere, reading progress below floating nav, proper image/thumbnail logic, complete PostCard/PostDetail redesign, deduped view tracking, shared ImagePreviewModal+Carousel, full database rebuild (SQL), clickable tags, improved feed sidebar with real user data.

### Auth Decision
Kept **Firebase Auth**. Supabase Auth migration deferred to v2.8+ (admin panel phase). Firebase UID is stored as `users.id TEXT` in Supabase. All social actions use anon key + JS-side UID. RLS is anon-friendly. Admin ops via Cloudflare Worker with service role key.

### Folder Restructure
`creator-guide/` renamed to `.creator/` (hidden folder). Internal structure:
```
.creator/
├── CONTEXT.md
├── master-prompt.md
...
│   │   └── supabase-schema-v2.5.2.sql  ← full database rebuild
│   └── demo-data/
│       ├── blogs-demo-v2.5.2.csv
│       └── posts-demo-v2.5.2.csv
└── workflows/
    └── v2-workflow.html
```

### Database Rebuild (supabase-schema-v2.5.2.sql)
**FULL REBUILD** — all tables except `projects` dropped and recreated.

New/rebuilt tables (21 total):
| Table | Purpose |
|-------|---------|
| `users` | Firebase UID mirror (id = TEXT) |
| `admins` | Admin user list |
| `blogs` | Blog articles (enhanced) |
| `posts` | Social posts (enhanced) |
| `likes` | Reactions for projects/blogs/posts (like/dislike + emoji reaction) |
| `comments` | Comments for all content types |
| `comment_likes` | Likes on comments |
| `reposts` | Reshares |
| `feed_saved` | Bookmarks |
| `views_tracking` | Deduped view tracking (3-day cooldown per viewer) |
| `notifications` | User notifications |
| `reports` | Content reports |
| `spam_tracking` | Rate limit / spam detection |
| `project_reviews` | Star reviews for projects |
| `review_likes` | Likes on reviews |
| `reviews` | Global testimonials |
| `subscribers` | Newsletter |
| `site_settings` | Global config |
| `page_visibility` | Page on/off |
| `page_views` | Analytics |

Key SQL features: `track_content_view()` RPC (3-day cooldown), `toggle_like()` RPC (atomic count updates), `toggle_repost()` RPC, `toggle_comment_like()` RPC, `update_comment_count()` trigger, `set_updated_at()` trigger on all tables.

### New Files
| File | Description |
|------|-------------|
| `src/components/shared/ImagePreviewModal.jsx` | Complete rebuild — zoom (scroll+pinch), pan, double-tap, swipe nav, keyboard nav, thumbnail strip, download button, mobile-first |
| `src/components/shared/Carousel.jsx` | New shared carousel for blogs+projects — auto-play, swipe, keyboard, thumbnail strip, click-to-preview, aspect ratio prop |
| `.creator/supabase/v2.5/supabase-schema-v2.5.2.sql` | Full new schema (see above) |
| `.creator/demo-data/blogs-demo-v2.5.2.csv` | 2 advanced demo blogs with full HTML content |
| `.creator/demo-data/posts-demo-v2.5.2.csv` | 2 advanced demo posts with markdown content + media |

### Rebuilt Files
| File | Changes |
|------|---------|
| `src/components/layout/Navbar.jsx` | `isHomePage = true` always — all pages use transparent/absolute navbar style |
| `src/components/layout/Layout.jsx` | Removed `pt-navbar` from `<main>` for all pages — each page manages its own top padding |
| `src/components/feed/FeedPage.jsx` | Header matches ProjectsPage style (no gradient banner). Sidebar: real user data from SITE_CONFIG, clickable tags (4 rows expandable, max 10 rows). Tags URL-connected. Feed header type tabs, sort select, search debounced 280ms. |
| `src/components/feed/feed.css` | Complete overhaul: reading progress bar below floating navbar, reaction popup, post markdown styles, proper media grid CSS, improved blog card, post card, TOC card, sidebar |
| `src/components/feed/BlogCard.jsx` | Auto-extract thumbnail from HTML `<img>` if no `thumbnail_url`. No thumbnail section if no image. Clickable tags → feed search. `timeAgo()` from upload time |
| `src/components/feed/BlogDetailPage.jsx` | Single `paddingTop: var(--navbar-h)` (no double spacing). Reading progress bar below float nav, same width+rounded. TOC: active section tracking with IntersectionObserver. Right sidebar: TOC + blog about (stats/tags) + share. Left column: no share. Images in content: click → ImagePreviewModal. OG image = first image in HTML content |
| `src/components/feed/PostCard.jsx` | Complete redesign. Markdown rendering (bold/italic/underline/links/headings). Reaction popup (hover 0.5s → emoji reactions). Three-dot menu (copy link, report). Header click → detail page. Profile pic/name → /about. See more/less (4-line clamp) |
| `src/components/feed/PostDetailPage.jsx` | Complete redesign. Left: media panel (sticky on desktop). Right: Facebook-style header (avatar, name, time, globe, three-dot). Then markdown content. Then tags. Then action bar (like+reactions, comment, save, share). Then CommentSection (auto-scroll if #comments hash). |
| `src/components/feed/MediaGrid.jsx` | Rebuilt. Video: single iframe with thumbnail+play (lazy load, autoplay on click). Images: 1/2/3/4/5+ layouts with correct Facebook-style grid. Max-height container. Shared ImagePreviewModal on click. |
| `src/services/supabase.js` | +`trackContentView()` using RPC with 3-day dedup fallback. +`getRepostsCount()`, `getUserRepostStatus()`, `toggleRepost()`. Updated `getPublishedBlogs/Posts/getBlogBySlug/getPostBySlug` to include all new columns. |

### Navbar Change Detail
- **Before**: `isHomePage = ['/', '/home', '/about'].includes(location.pathname)` — only home/about had transparent navbar
- **After**: `isHomePage = true` — ALL pages have transparent/absolute navbar (same style)
- **Impact**: Each page must handle its own top spacing via `paddingTop: 'var(--navbar-h)'` or `paddingTop: 'calc(var(--navbar-h) + X)'` in its first section
- **Pages already correct**: Projects, Home, About, Feed, BlogDetail, PostDetail
- **Pages to check**: Contact, any simple pages — add `paddingTop: 'var(--navbar-h)'` wrapper if needed

### Reading Progress Bar
- `position: fixed; top: 67px` (below float nav: 12px padding + 52px nav + 3px gap)
- Width: `min(calc(100% - 2.5rem), 1088px)` — matches floating navbar width
- Only `opacity: 1` when `scrollY > 420` (same threshold as float nav)
- Height: 3px, gradient fill (green → accent → purple)
- CSS class `reading-progress-container` + `visible` toggle

### View Deduplication
- `trackContentView(contentType, contentId, viewerKey, isAuth)` calls Supabase RPC
- `viewerKey` = Firebase UID (logged in) or `getDeviceHash()` (anonymous)
- RPC checks `views_tracking` table: if no row → insert + increment. If row exists + last_viewed > 3 days → update + increment. Otherwise → no-op.
- Falls back to legacy `increment*Views()` if RPC fails

### Post Card Image Grid Rules
| Count | Layout |
|-------|--------|
| 1 | Full width, 16:9 |
| 2 | 50/50 side-by-side, 2:1 ratio |
| 3 | First takes full left height, two right stack |
| 4 | 2×2 grid, 1:1 ratio |
| 5+ | 2×2 grid, 4th cell shows `+N` overlay |
| Max height | `min(360px, 60vw)` |

### PostDetailPage Layout
- **Desktop (>1024px)**: 2-col grid `1fr 380px` — left: media panel (sticky, full viewport height) / right: info panel (scrollable)
- **Mobile**: stacked — media panel (auto height, max 56vw) then info panel
- **Media panel**: sticky on desktop, contains `MediaGrid` with `inDetail=true` (ImagePreviewModal instead of navigate)
- **Info panel**: back button → author header (Facebook-style) → markdown content → tags → action bar → CommentSection

### Sample Data (2 blogs + 2 posts)
All inserted in `supabase-schema-v2.5.2.sql`:
- Blog 1: "Mastering React Hooks in 2026" — Web Dev, 7 min read, full HTML with code blocks
- Blog 2: "Supabase RLS: The Complete Security Guide" — Backend, 9 min read, full HTML
- Post 1: "My Development Workflow in 2026" — 3 images, markdown, tags: devsetup/vscode/workflow
- Post 2: "Dark Mode Design Tips That Actually Work" — YouTube video embed, markdown tips

### Architecture Notes
- `ImagePreviewModal` is now at `src/components/shared/ImagePreviewModal.jsx` — import from there everywhere
- Old `src/components/projects/ImagePreviewModal.jsx` should import from shared (or be deleted)
- `Carousel.jsx` at `src/components/shared/Carousel.jsx` — use for both blogs and projects
- `ProjectCarousel.jsx` in projects/ can now use the shared Carousel component
- `trackContentView` should replace all `incrementBlogViews` / `incrementPostViews` / `incrementProjectViews` calls

### Config Update
- `site.config.js` version: `'v2.5.2'`
