// Hero.jsx — v2.2.4
// Fixes: Material Symbols waving_hand, left content vertically centered,
//        image height = leftContentHeight*1.1 (ResizeObserver), no image clipping,
//        floating icons outside round frame on tablet/mobile, round frame ONLY at ≤900px,
//        removed portrait-orientation round frame rule
...
// Floating icons: only 4 shown on tablet, 0 on very small mobile
const FLOAT_ICONS = [
  { file:'html5',  label:'HTML5',  cls:'html',   dur:'3.9s', del:'0s',    sz:'46px' },
  { file:'css3',   label:'CSS3',   cls:'css',    dur:'4.4s', del:'0.65s', sz:'42px' },
  { file:'python', label:'Python', cls:'python', dur:'3.6s', del:'1.2s',  sz:'40px' },
  { file:'vscode', label:'VSCode', cls:'vscode', dur:'3.3s', del:'0.95s', sz:'44px' },
  { file:'design', label:'Design', cls:'design', dur:'4.8s', del:'0.35s', sz:'38px' },
]
...
export default function Hero({settings,settingsLoading}){
  ...
  // ResizeObserver: set right scene height = left content height * 1.1
  useEffect(()=>{
    if(!contentRef.current) return
    const obs=new ResizeObserver(([entry])=>{
      const h=entry.contentRect.height
      if(h>0) setSceneH(Math.round(h*1.1))
    })
...
/* Inner grid — takes full height */
.hinner{position:relative;z-index:5;display:grid;grid-template-columns:1.18fr 1fr;gap:clamp(1rem,3vw,2.5rem);align-items:center;width:100%;min-height:inherit;max-width:1280px;margin-inline:auto;padding-inline:1.75rem;padding-block:clamp(2rem,4vh,3.5rem)}
...
.hgreet-icon{color:#F59E0B;font-size:1.15rem;flex-shrink:0;line-height:1;display:inline-flex;align-items:center}
...
/* Right visual */
.hvisual{position:relative;z-index:5;display:flex;align-items:flex-start;justify-content:center;opacity:0;animation:hvis-in .85s cubic-bezier(.16,1,.3,1) .2s forwards}
@keyframes hvis-in{from{opacity:0;transform:translateX(22px) scale(.94)}to{opacity:1;transform:none}}
/* hscene: no fixed width constraint, overflow visible so icons show outside */
.hscene{position:relative;width:100%;overflow:visible;align-self:flex-start;margin-top:clamp(-20px,calc(-1vh - 5px),0px)}
/* hwrap: image wrapper */
.hwrap{position:relative;z-index:1;width:100%;height:100%;overflow:hidden}
.hphoto{width:100%;height:100%;object-fit:contain;object-position:top center;display:block;filter:drop-shadow(0 12px 40px rgba(0,0,0,.25))}
...
.hbot{position:absolute;bottom:0;left:-12%;right:-12%;z-index:4;pointer-events:none}
...
/* Floating icons — PC absolute positions relative to hscene */
...
/* PC icon positions — outside image area left and right */
.hicon-html{top:8%;left:-32px}
.hicon-css{top:52%;left:-28px}
.hicon-python{top:76%;left:-24px}
.hicon-vscode{top:5%;right:-30px}
.hicon-design{top:38%;right:-26px}
/* Scroll button */
...
/* TABLET ≤900px: columns stack, image becomes circle */
@media(max-width:900px){
  ...
  /* Circle frame on tablet */
  .hscene{width:clamp(175px,46vw,240px);margin:0 auto;align-self:auto}
  .hwrap{border-radius:50%;overflow:hidden;box-shadow:0 0 0 4px var(--bg-page),0 0 0 6px var(--border-color),0 0 30px rgba(37,99,235,.16);animation:hring-p 3s ease-in-out infinite;aspect-ratio:1}
  .hphoto{object-fit:cover;object-position:top center}
  .hglow{border-radius:50%}
  .hbot,.hgrad-1{display:none}
  /* Floating icons: outside circle border, close */
  .hicon{--sz:38px !important;border-radius:11px}
  .hicon-html{top:-12px !important;left:calc(50% - 23vw - 32px) !important;right:auto !important}
  .hicon-css{top:-12px !important;left:auto !important;right:calc(50% - 23vw - 32px) !important}
  .hicon-python{top:50% !important;left:calc(50% - 23vw - 34px) !important;right:auto !important;transform:translateY(-50%);animation-name:hicon-fy}
  .hicon-vscode{top:50% !important;left:auto !important;right:calc(50% - 23vw - 34px) !important;transform:translateY(-50%);animation-name:hicon-fy}
  .hicon-design{display:none !important}
...
@media(max-width:520px){
  ...
  .hscene{width:clamp(150px,55vw,190px)}
  .hicon{--sz:30px !important;padding:5px;border-radius:8px}
  .hicon-html{top:-10px !important;left:calc(50% - 27.5vw - 22px) !important}
  .hicon-css{top:-10px !important;right:calc(50% - 27.5vw - 22px) !important}
  .hicon-python{left:calc(50% - 27.5vw - 24px) !important}
  .hicon-vscode{right:calc(50% - 27.5vw - 24px) !important}
}
@media(min-width:1400px){.hscene{margin-top:clamp(-30px,-2vh,-10px)}}
@media(max-height:580px) and (min-width:521px){.hero{min-height:100dvh}}
`}</style>
      ...
      <section className="hero" id="hero" aria-label="Introduction">
        ...
        <div className="hinner">
          {/* LEFT CONTENT — with ref for ResizeObserver */}
          <div className="hcontent" ref={contentRef}>
            <div className="hgreet hup" style={{'--d':'0ms'}}>
              ...
              {/* Google Material Symbols waving_hand */}
            ...
            <div ref={statsRef} className="hstats hup" style={{'--d':'710ms'}} aria-label="Stats">
          ...
          {/* RIGHT VISUAL — height driven by left content */}
          <div className="hvisual" aria-hidden="true">
            <div className="hscene" ref={sceneRef}
                 style={sceneH > 0 ? {height: sceneH + 'px'} : {}}>
              <div className="hglow"/>
              <div className="hwrap" style={sceneH > 0 ? {height: sceneH + 'px'} : {}}>
              ...
              {FLOAT_ICONS.map(ic=>(
                <div key={ic.cls}
                     ...
                     style={{'--dur':ic.dur,'--del':ic.del,'--sz':ic.sz}}
