// Navbar.jsx — v2.2.4
// Key changes:
//  - max-width 1280px (matches body container-xl and footer nf-inner)
//  - Desktop search: no popup → toast message "Search coming soon"
//  - Mobile/tablet: search opens sidebar with beautiful focused search bar
//  - Sidebar search: full border changes on focus, clear button on typing
//  - Nav item hover: improved glass-pill effect on both top & floating nav
//  - Right icons: CSS :after tooltips already working, added minimal glass bg
//  - Mega menu glass effect on floating (already had .mega-floating class)
//  - Floating navbar: icons have glass bg matching float-nav style
//  - Home page: transparent top navbar; other pages: bg navbar
//  - About/Contact pages: cleared (placeholder)
...
const SEARCH_ITEMS = [
  ...
  { label: 'Projects', path: '/projects', icon: faCode, group: 'Page', keywords: 'portfolio work' },
  { label: 'Feed', path: '/feed', icon: faRss, group: 'Page', keywords: 'blogs posts updates' },
  { label: 'Contact', path: '/contact', icon: faEnvelope, group: 'Page', keywords: 'message email' },
  { label: 'My Profile', path: '/profile', icon: faAddressCard, group: 'Account', keywords: 'account user' },
  { label: 'Sign In', path: '/login', icon: faSignIn, group: 'Account', keywords: 'login auth' },
  { label: 'Sign Up', path: '/signup', icon: faArrowRight, group: 'Account', keywords: 'register' },
  { label: 'Privacy Policy', path: '/privacy-policy', icon: faShieldHalved, group: 'Legal', keywords: 'privacy' },
  { label: 'Cookies Policy', path: '/cookies-policy', icon: faCookie, group: 'Legal', keywords: 'cookie' },
  { label: 'Sitemap', path: '/sitemap.xml', icon: faGlobe, group: 'Utility', keywords: 'links xml', external: true },
]
...
function StatusDot({ mode = CURRENT_STATUS, size = 'md' }) {
  ...
  return (
    <span className={`absolute -bottom-0.5 -right-0.5 ${dim} rounded-full border-[var(--bg-page)] transition-colors`}
      style={{ background: s.color }} title={s.label} aria-label={`Status: ${s.label}`} />
  )
}
  ...
  return (
    <Link to="/" onClick={onClick}
      ...
      <div className={`relative ${logoSize} ${logoShape} flex items-center justify-center flex-shrink-0 overflow-visible`}>
        <img src={logoSrc} alt="Muhtasim logo"
      ...
      {showName && (
        <div className="flex flex-col leading-none">
          <span className="font-mono font-bold text-[16px] text-[var(--text-primary)] leading-none">{SITE_CONFIG.navName}</span>
...
// ── Theme Toggle ─────────────────────────────────────────────
function ThemeToggle({ size = 'md', className = '', floating = false }) {
  ...
  return (
    <button onClick={handleClick}
      className={`${sz} ${floating ? 'float-icon-btn' : ''} relative overflow-hidden flex items-center justify-center rounded-full border border-[var(--border-color)] bg-[var(--bg-surface-2)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors ${className}`}
      ...
      <AnimatePresence mode="wait" initial={false}>
        {dark
          ? <motion.span key="sun" initial={{opacity:0,rotate:-90,scale:0.5}} animate={{opacity:1,rotate:0,scale:1}} exit={{opacity:0,rotate:90,scale:0.5}} transition={{duration:0.18}}><FontAwesomeIcon icon={faSun} /></motion.span>
          : <motion.span key="moon" initial={{opacity:0,rotate:90,scale:0.5}} animate={{opacity:1,rotate:0,scale:1}} exit={{opacity:0,rotate:-90,scale:0.5}} transition={{duration:0.18}}><FontAwesomeIcon icon={faMoon} /></motion.span>
        }
      </AnimatePresence>
    </button>
  )
...
// ── Icon Button ───────────────────────────────────────────────
function IconBtn({ icon, onClick, label, badge, active, className = '', floating = false }) {
  const { ripples, createRipple } = useRipple()
  const handleClick = (e) => { createRipple(e); onClick?.(e) }
  return (
    <button onClick={handleClick}
      className={`relative overflow-hidden w-9 h-9 flex items-center justify-center rounded-full border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors ${floating ? 'float-icon-btn' : 'bg-[var(--bg-surface-2)]'} ${active ? 'bg-[var(--accent-light)] text-[var(--accent-primary)] border-[var(--accent-primary)]' : ''} ${className}`}
      aria-label={label} data-tooltip={label} data-ripple-managed="true">
      <RippleLayer ripples={ripples} color="rgba(59,130,246,0.2)" />
...
function SignInBtn({ className = '' }) {
  const { ripples, createRipple } = useRipple()
  return (
    <Link to="/login" onClick={createRipple} title="Sign in to your account"
...
function SocialMarquee({ compact = false }) {
  ...
  const onMouseDown = (e) => { dragRef.current = { ...dragRef.current, active: true, startX: e.pageX, offset: dragRef.current.currentOffset }; pause(); e.preventDefault() }
  const onMouseMove = (e) => {
    if (!dragRef.current.active) return
    dragRef.current.currentOffset = dragRef.current.offset + (e.pageX - dragRef.current.startX)
  ...
  const onTouchStart = (e) => { dragRef.current = { ...dragRef.current, active: true, startX: e.touches[0].pageX, offset: dragRef.current.currentOffset }; pause() }
  const onTouchMove = (e) => {
    if (!dragRef.current.active) return
    dragRef.current.currentOffset = dragRef.current.offset + (e.touches[0].pageX - dragRef.current.startX)
    if (trackRef.current) trackRef.current.style.transform = `translateX(${dragRef.current.currentOffset}px)`
  }
...
function MegaMenu({ onClose, isLoggedIn, floating = false }) {
  ...
  const copyUrl = async () => {
    try { await navigator.clipboard.writeText(currentUrl); toast.success('Copied', 'Current page URL copied.') }
    catch { toast.error('Copy failed', 'Could not copy this URL.') }
  }
  const shareNative = async () => {
    try {
      if (navigator.share) { await navigator.share({ title: SITE_CONFIG.siteName, text: shareText, url: currentUrl }); return }
      copyUrl()
    } catch (error) { if (error?.name !== 'AbortError') copyUrl() }
  }
          ...
          <div className={`rounded-2xl overflow-hidden ${floating ? 'mega-floating' : ''}`}
              ...
              {MEGA_COLS.map((col) => (
                <div key={col.label} className="px-3 py-3">
                  ...
                  <div className="space-y-0.5">
                    {col.items.map(item => (
                      item.external
                        ? <a key={item.path} href={item.path} target="_blank" rel="noopener noreferrer" onClick={onClose} className={itemClass(item.path)}>
                            ...
                            <span className="text-[13px] font-medium text-[var(--text-secondary)] group-hover:text-[var(--accent-primary)] transition-colors">{item.label}</span>
                          </a>
                        : <Link key={item.path} to={item.path} onClick={onClose} className={itemClass(item.path)}>
                            <FontAwesomeIcon icon={item.icon} className="w-3.5 text-center text-[var(--text-tertiary)] group-hover:text-[var(--accent-primary)] transition-colors flex-shrink-0 text-xs" />
                            <span className="text-[13px] font-medium text-[var(--text-secondary)] group-hover:text-[var(--accent-primary)] transition-colors">{item.label}</span>
                          </Link>
                    ))}
            ...
            <div className="mega-footer px-3 py-2 border-t border-[var(--border-color)] flex items-center gap-2">
              <button type="button" onClick={shareNative} className="mega-share-action" aria-label="Share this page" data-tooltip="Share this page">
                <FontAwesomeIcon icon={faShareNodes} className="text-[11px]" /><span>Share</span>
              </button>
              ...
              <span className="mega-version-pill">Web&nbsp;<strong>{SITE_CONFIG.version}</strong></span>
            </div>
          </div>
        </div>
      </motion.div>
...
// ── Nav Right Icons ───────────────────────────────────────────
function NavRight({ user, profile, isAdmin: isAdminProp, avatar, displayName, isLoggedIn, authLoading, unreadCount,
  notifOpen, setNotifOpen, userOpen, setUserOpen, megaOpen, setMegaOpen, onMenuOpen, onMobileSearch,
  onDesktopSearch, floating }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-shrink-0" data-nav-right>
      {/* Desktop: search → toast */}
      <IconBtn icon={faSearch} onClick={onDesktopSearch} label="Search  Ctrl+K"
        className="hidden lg:flex" floating={floating} />
      {/* Tablet/mobile: opens sidebar */}
      <IconBtn icon={faSearch} onClick={onMobileSearch} label="Search"
        className="lg:hidden" floating={floating} />
      ...
      <div className="notif-anchor relative">
        <IconBtn icon={faBell} onClick={() => { setNotifOpen(!notifOpen); setUserOpen(false); setMegaOpen(false) }}
          label="Notifications" badge={unreadCount} active={notifOpen} floating={floating} />
        <AnimatePresence>{notifOpen && <NotifPanel onClose={() => setNotifOpen(false)} />}</AnimatePresence>
      </div>

      <ThemeToggle floating={floating} />
      ...
      <div className="mega-anchor">
        <IconBtn icon={faTableCells} onClick={() => { setMegaOpen(!megaOpen); setNotifOpen(false); setUserOpen(false) }}
          label="All pages" active={megaOpen} floating={floating} />
      </div>

      <button onClick={(e) => { createMenuRipple(e); onMenuOpen() }}
        className={`lg:hidden relative overflow-hidden w-9 h-9 flex items-center justify-center rounded-full border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors ${floating ? 'float-icon-btn' : 'bg-[var(--bg-surface-2)]'}`}
...
export function Navbar() {
  ...
  useEffect(() => {
    setMobileOpen(false); setMegaOpen(false); setUserOpen(false); setNotifOpen(false); setSearchQuery('')
  }, [location.pathname])
  ...
  // Desktop search → toast notification
  const handleDesktopSearch = () => {
    toast.info('Search coming soon!', 'Full search engine is under development. Stay tuned! 🔍')
  ...
  const sharedProps = {
    ...
    onDesktopSearch: handleDesktopSearch,
  }

  const linkCls = (path) => (active) => {
    const isAct = path === '/' ? location.pathname === '/' : active
    return `nav-link-pill relative overflow-hidden flex items-center gap-1.5 h-9 px-3.5 rounded-full text-sm font-medium leading-none transition-all ${isAct ? 'nav-link-active' : ''}`
  }
  ...
  return (
    <>
      {/* ╔══ TOP NAVBAR ═══════════════════════════════════════╗ */}
      <nav
        className={`relative z-10 w-full ${isHomePage ? 'border-b border-transparent bg-transparent' : 'border-b border-[var(--navbar-border)] bg-[var(--navbar-bg)] backdrop-blur-md'}`}
        ...
        <div className="navbar-inner flex items-center h-full max-w-[1280px] mx-auto gap-5">
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center gap-0.5">
            {NAV_LINKS.map(link => (
              <NavLink key={link.path} to={link.path} end={link.path === '/'}
                ...
                title={link.title} data-click-fx="true">
          ...
          <div className="hidden lg:flex ml-auto">
            <NavRight {...sharedProps} floating={false} />
          </div>
          {/* Right tablet */}
          ...
          <div className="flex md:hidden items-center gap-1.5 ml-auto">
            {!authLoading && !isLoggedIn && (
              <Link to="/login" className="w-8 h-8 rounded-full bg-[var(--accent-primary)] flex items-center justify-center text-white flex-shrink-0 hover:bg-[var(--accent-hover)] transition-colors" aria-label="Sign in" data-tooltip="Sign In">
                <FontAwesomeIcon icon={faSignIn} className="text-xs" />
              </Link>
            )}
            <ThemeToggle size="sm" />
      ...
      <AnimatePresence>
        {floatVisible && (
          <motion.div variants={floatVariants} initial="hidden" animate="visible" exit="exit"
            ...
            <nav className="float-nav relative pointer-events-auto flex items-center gap-4 w-full max-w-[1280px] h-[52px] px-4 rounded-full">
                ...
                {NAV_LINKS.map(link => (
                  <NavLink key={link.path} to={link.path} end={link.path === '/'}
                    className={({ isActive }) => {
                      ...
                      return `float-nav-link nav-link-pill relative overflow-hidden flex items-center gap-1.5 h-9 px-3.5 rounded-full text-sm font-medium leading-none transition-all ${isAct ? 'float-nav-link-active' : ''}`
                    }}
                    title={link.title}>
                    <FontAwesomeIcon icon={link.icon} className="text-xs opacity-80" />
              ...
              <div className="hidden lg:flex ml-auto float-nav-right">
                <NavRight {...sharedProps} floating />
              </div>
              <div className="hidden md:flex lg:hidden items-center gap-1.5 ml-auto float-nav-right">
                {!authLoading && !isLoggedIn && <SignInBtn className="h-8 text-xs px-3 py-0" />}
                <ThemeToggle size="sm" floating />
                <IconBtn icon={faSearch} onClick={handleMobileSearch} label="Search" className="w-8 h-8 text-sm" floating />
                <button onClick={() => setMobileOpen(true)}
                  className="w-8 h-8 flex items-center justify-center rounded-full border border-[rgba(255,255,255,0.11)] float-icon-btn text-[rgba(226,232,240,0.8)]">
                  <FontAwesomeIcon icon={faBars} className="text-sm" />
                </button>
              </div>
              <div className="flex md:hidden items-center gap-1.5 ml-auto float-nav-right">
                {!authLoading && !isLoggedIn && (
                  <Link to="/login" className="w-8 h-8 rounded-full bg-[var(--accent-primary)] flex items-center justify-center text-white flex-shrink-0" aria-label="Sign in">
                    <FontAwesomeIcon icon={faSignIn} className="text-xs" />
                  </Link>
                )}
                <ThemeToggle size="sm" floating />
          ...
          <>
            ...
            <motion.aside key="sidebar" variants={sidebarVariants} initial="closed" animate="open" exit="closed"
              ...
              <div className="px-4 mt-3 flex-shrink-0">
                <div className="sidebar-search-wrap flex items-center gap-2.5 px-4 py-2.5 rounded-full border transition-all duration-200"
                  ...
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')}
                      className="w-5 h-5 rounded-full flex items-center justify-center bg-[var(--bg-surface-3)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors flex-shrink-0">
              ...
              <div className="flex-1 overflow-y-auto py-3 sidebar-scroll">
                {isSearching ? (
                  <div className="px-3 pt-2">
                      ...
                      <div className="space-y-0.5">
                        {sidebarResults.map(link => (
                          <NavLink key={link.path} to={link.path} onClick={() => setMobileOpen(false)}
                            className={({ isActive }) => `search-popup-item ${isActive ? 'is-sidebar-active' : ''}`}>
                    ...
                    <p className="text-[10px] text-[var(--text-tertiary)] text-center mt-4 px-2">
                      🔍 Full search engine coming soon!
                    </p>
                  </div>
                ) : (
                  <>
      ...
      {/* ── Styles ─────────────────────────────────────────────── */}
      <style>{`
        /* ── Navbar inner: max-width 1280px matches container-xl ─ */
        .navbar-inner {
        ...
        /* ── Top nav link pill hover effect ─────────────────────── */
        .nav-link-pill {
          color: var(--text-secondary);
          border: 1px solid transparent;
        }
        .nav-link-pill:hover:not(.nav-link-active):not(.float-nav-link-active) {
        ...
        /* ── Float nav icon buttons: glass style ─────────────────── */
        .float-icon-btn {
        ...
        [data-theme="light"] .float-icon-btn {
          background: rgba(15,23,42,0.06) !important;
          border-color: rgba(15,23,42,0.12) !important;
          color: rgba(30,41,59,0.75) !important;
          backdrop-filter: none;
        }
        [data-theme="light"] .float-icon-btn:hover {
        ...
        /* Top navbar mega always has glass too */
        ...
        /* Mega nav item hover: text + icon color change, not just text */
        .is-mega-active svg, .is-mega-active span { color: var(--accent-primary) !important; }

        /* ── Sidebar search: whole wrapper border changes on focus ── */
        .sidebar-search-wrap {
        ...
        /* ── Tooltip (CSS :after/:before) ───────────────────────── */
        [data-tooltip] { position: relative; }
        [data-tooltip]::after, [data-tooltip]::before {
          position: absolute; pointer-events: none; opacity: 0;
          transition: opacity .16s ease, transform .16s ease; z-index: 1020;
        }
        [data-tooltip]::after {
          content: attr(data-tooltip);
          bottom: -38px; left: 50%;
          transform: translateX(-50%) translateY(5px) scale(0.93);
          background: rgba(8,15,35,.97); color: #f1f5f9;
          border: 1px solid rgba(148,163,184,.18);
          box-shadow: 0 6px 20px rgba(0,0,0,.32), 0 2px 6px rgba(0,0,0,.2);
          backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
          font-size: 11.5px; line-height: 1; padding: 6px 11px;
          border-radius: 8px; white-space: nowrap; font-weight: 600; letter-spacing: 0.01em;
        }
        [data-tooltip]::before {
          content: ""; bottom: -10px; left: 50%;
        ...
        [data-theme="light"] [data-tooltip]::after { background: rgba(15,23,42,.97); border-color: rgba(100,116,139,.25); }
        [data-theme="light"] [data-tooltip]::before { background: rgba(15,23,42,.97); border-color: rgba(100,116,139,.25); }
        [data-tooltip]:hover::after { opacity: 1; transform: translateX(-50%) translateY(0) scale(1); }
        [data-tooltip]:hover::before { opacity: 1; transform: translateX(-50%) translateY(0) rotate(45deg); }
        [data-nav-right] [data-tooltip]::after { left: auto; right: 0; transform: translateY(5px) scale(0.93); }
        [data-nav-right] [data-tooltip]::before { left: auto; right: 12px; transform: translateY(5px) rotate(45deg); }
        [data-nav-right] [data-tooltip]:hover::after { transform: translateY(0) scale(1); }
        [data-nav-right] [data-tooltip]:hover::before { transform: translateY(0) rotate(45deg); }

        /* ── Search popup styles (sidebar) ──────────────────────── */
        .search-popup-item {
        ...
        .search-popup-item:hover, .search-popup-item.is-sidebar-active {
          background: var(--bg-surface-2); border-color: var(--border-color); color: var(--text-primary);
        }
        .search-popup-item-icon {
          width: 30px; height: 30px; border-radius: 8px; flex-shrink: 0;
          display: grid; place-items: center;
          background: var(--accent-light); color: var(--accent-primary); font-size: 12px;
        }
        .search-popup-item-label { flex: 1; min-width: 0; font-size: 13px; font-weight: 600; color: inherit; }
        .search-popup-item-badge {
          font-size: 10px; color: var(--text-tertiary); border: 1px solid var(--border-color);
          border-radius: 999px; padding: 2px 7px; line-height: 1; flex-shrink: 0;
        }
        .search-popup-item-arrow { font-size: 9px; color: var(--text-tertiary); opacity: 0; transition: opacity .15s, transform .15s; flex-shrink: 0; }
        ...
        .mega-url-copy { width: 30px; height: 30px; border-radius: 999px; display: inline-flex; align-items: center; justify-content: center; color: var(--text-tertiary); transition: all .2s ease; flex-shrink: 0; }
        .mega-url-copy:hover { color: var(--text-primary); background: var(--bg-surface-2); }
        .mega-share-action { height: 30px; padding: 0 12px; border-radius: 999px; display: inline-flex; align-items: center; gap: 6px; color: #fff; background: linear-gradient(135deg, var(--accent-primary) 0%, #6366f1 100%); font-size: 11.5px; font-weight: 600; transition: all .22s ease; flex-shrink: 0; box-shadow: 0 2px 8px rgba(59,130,246,.28); border: none; }
        .mega-share-action:hover { box-shadow: 0 4px 14px rgba(59,130,246,.38); transform: translateY(-1px); }
        .mega-url-field { height: 30px; min-width: 0; flex: 1; display: flex; align-items: center; gap: 4px; border: 1px solid var(--border-color); border-radius: 999px; background: var(--bg-surface-2); padding: 0 4px 0 12px; overflow: hidden; }
        .mega-url-field span { min-width: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 11px; color: var(--text-tertiary); font-family: var(--font-mono); }
        .mega-version-pill { height: 28px; padding: 0 10px; border-radius: 999px; display: inline-flex; align-items: center; font-size: 11px; color: var(--text-tertiary); background: var(--bg-surface-2); border: 1px solid var(--border-color); white-space: nowrap; flex-shrink: 0; }
        ...
        .mega-share-action:hover:active { transform: scale(.98) translateY(0); }

        @media (max-width: 640px) { .fixed.top-0.right-0.bottom-0 { width: 100vw !important; } }
      `}</style>
    </>
  )
}
