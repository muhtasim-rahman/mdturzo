// Skills.jsx — v2.2.3 (redesigned — vertical tabs + animated bars)
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-bold font-mono px-1.5 py-0.5 rounded-md" style={{color,background:`${color}15`}}>{p}%</span>
      </div>
      <div className="h-[7px] rounded-full bg-[var(--bg-surface-3)] overflow-hidden relative">
        <motion.div
          className="h-full rounded-full relative overflow-hidden"
          style={{background:`linear-gradient(90deg,${color}aa,${color})`}}
          initial={{width:0}}
          animate={{width: visible ? `${p}%` : 0}}
          transition={{duration:.75,delay:.05+i*.08,ease:[.16,1,.3,1]}}
        >
          <div className="absolute inset-0 opacity-30" style={{background:'linear-gradient(180deg,rgba(255,255,255,.4) 0%,transparent 100%)'}}/>
        </motion.div>
      </div>
    </div>
  )
...
function StatPill({ label, value, color, suffix, inView }) {
  ...
  return (
    <motion.div
      className="flex flex-col items-center gap-1.5 px-5 py-4 rounded-2xl border bg-[var(--bg-surface)] relative overflow-hidden"
      style={{borderColor:`${color}30`}}
      initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true,amount:.4}}
      transition={{duration:.4}}>
      <div className="absolute inset-x-0 bottom-0 h-0.5" style={{background:`linear-gradient(90deg,transparent,${color}60,transparent)`}}/>
      <span className="text-2xl font-display font-extrabold leading-none" style={{color}}>
        {count}{suffix ?? '+'}
      </span>
