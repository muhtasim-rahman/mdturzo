// AboutMini.jsx — v2.2.5
// Fixes:
//   - Read Full Story button: no hover:scale, effects stay within button
//   - Grid content properly centered (justify-items-center on single col)
//   - Light mode image overlay: reduced opacity to ~15%
  ...
  return (
    <section className="section section-alt" id="about-mini">
      <div className="container-xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center justify-items-center lg:justify-items-stretch">
          {/* Image */}
          <motion.div className="relative flex justify-center"
            ...
            <div className="relative w-72 sm:w-80 lg:w-96">
              ...
              <div className="relative rounded-2xl overflow-hidden aspect-[3/4] border border-[var(--border-strong)] bg-[var(--bg-surface-2)] shadow-[var(--shadow-xl)]">
                ...
                {/* Overlay — dark mode: full opacity; light mode: very subtle */}
                <div className="absolute bottom-0 left-0 right-0 h-1/2 pointer-events-none about-img-overlay"/>
                <div className="absolute bottom-4 left-4">
                  <p className="text-white font-bold font-display">{SITE_CONFIG.owner.displayName}</p>
                  <p className="text-white/60 text-xs mt-0.5">Age {age} · Bangladesh</p>
                </div>
              </div>
          ...
          <motion.div className="flex flex-col gap-5 w-full"
            ...
            <motion.div variants={up} className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              {/* Read Full Story — no hover:scale, effects stay inside */}
              <Link to="/about"
                className="group flex items-start gap-2.5 p-3 rounded-lg border transition-colors duration-200 active:opacity-80"
                style={{
      ...
      <style>{`
        /* Dark mode: prominent gradient overlay at bottom of image */
        [data-theme="dark"] .about-img-overlay {
          background: linear-gradient(to top, rgba(2,6,23,.85), transparent);
        }
        /* Light mode: very subtle — just ~15% opacity so it doesn't look dark/odd */
