...
function getSearchResults(query) {
  ...
  const matches = q
    ? SEARCH_ITEMS.filter(item => `${item.label} ${item.group} ${item.path} ${item.keywords}`.toLowerCase().includes(q))
    : SEARCH_ITEMS.filter(item => !item.external)

  // Keep unique entries by route path so sidebar never duplicates results.
  return Array.from(new Map(matches.map(item => [item.path, item])).values()).slice(0, 12)
}
  ...
  return (
    <AnimatePresence>
      <motion.div variants={megaVariants} initial="hidden" animate="visible" exit="exit"
        className="mega-panel absolute left-0 right-0 top-full z-[12000]">
        <div className="max-w-[1120px] mx-auto px-4 pt-2">
          <div className="rounded-2xl overflow-hidden"
            ...
            <div className="px-4 py-3 border-t border-[var(--border-color)] flex items-center justify-between gap-3">
              <div className="mega-footer-share">
                <button type="button" onClick={shareNative} className="mega-share-action" aria-label="Share current page">
                  <FontAwesomeIcon icon={faShareNodes} />
                  <span>Share</span>
                </button>
                <div className="mega-url-field" title={currentUrl}>
                  <span>{currentUrl}</span>
                  <button type="button" onClick={copyUrl} className="mega-url-copy" aria-label="Copy URL" data-tooltip="Copy URL" data-tooltip-side="right">
                    <FontAwesomeIcon icon={faCopy} />
                  </button>
                </div>
              </div>
              <span className="mega-version-pill">Web version <strong>{SITE_CONFIG.version}</strong></span>
            </div>
          </div>
        </div>
      </motion.div>
...
function DesktopSearchPopup({ open, query, setQuery, onClose, inputRef }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[410] hidden lg:flex items-center justify-center px-6 py-10" onMouseDown={onClose}>
      <div className="absolute inset-0 bg-black/35 backdrop-blur-[4px]" />
      <motion.div
        initial={{ opacity: 0, y: 18, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        exit={{ opacity: 0, y: 18, scale: 0.98 }}
        className="desktop-search-pop relative w-[min(680px,calc(100vw-48px))] rounded-[24px] border border-[var(--border-color)] bg-[var(--bg-surface)] shadow-[var(--shadow-xl)] overflow-hidden"
        ...
        <div className="p-3">
          <div className="px-2 pb-2 flex items-center justify-between">
            <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--text-tertiary)]">
              {hasQuery ? 'Search results' : 'Quick open'}
            </span>
          ...
          {results.length > 0 ? (
            ...
            <div className="desktop-search-empty">
              <div>
                <FontAwesomeIcon icon={faSearch} />
              </div>
              <p>No results found</p>
              <span>Try a page name like Projects, Feed, Contact, or Privacy.</span>
            </div>
          )}
        </div>
      </motion.div>
...
export function Navbar() {
  ...
  return (
    <>
      {/* ╔══ TOP NAVBAR ═══════════════════════════════════════╗ */}
      <nav className="relative z-[1000] w-full border-b border-[var(--navbar-border)] bg-[var(--navbar-bg)] backdrop-blur-md"
        style={{ height: 'var(--navbar-h)' }}>
        <div className="navbar-inner flex items-center h-full max-w-[1120px] mx-auto gap-5">
          <NavLogo />
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside key="sidebar" variants={sidebarVariants} initial="closed" animate="open" exit="closed"
              className="fixed top-0 right-0 bottom-0 w-[min(340px,88vw)] max-[479px]:w-screen bg-[var(--bg-surface)] border-l border-[var(--border-color)] z-[395] flex flex-col lg:hidden overflow-hidden">
              ...
              */}
              <div className="px-4 mt-3 flex-shrink-0">
                <div className="sidebar-search-field flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-[var(--bg-surface-2)] border border-[var(--border-color)] focus-within:border-[var(--accent-primary)] focus-within:shadow-[0_0_0_3px_rgba(59,130,246,0.1)] transition-all">
              ...
              <div className="flex-1 overflow-y-auto py-3 sidebar-scroll">
                {isSearching ? (
                  ...
                  <div className="px-5 py-8 text-center">
                    ...
                    <p className="text-sm font-medium text-[var(--text-primary)] mb-1">Results for "<span className="text-[var(--accent-primary)]">{searchQuery}</span>"</p>
                    <p className="text-xs text-[var(--text-tertiary)]">{sidebarResults.length ? `${sidebarResults.length} matching shortcut${sidebarResults.length > 1 ? 's' : ''}` : 'No results found'}</p>
                    <div className="mt-4 space-y-1 text-left">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-tertiary)] px-1 mb-2">Results</p>
                      {sidebarResults.length > 0 ? sidebarResults.map(link => (
                        ...
                        <div className="desktop-search-empty py-5">
                          <div>
                            <FontAwesomeIcon icon={faSearch} />
                          </div>
                          <p>No results found</p>
                          <span>Try Projects, Feed, Contact, or Privacy.</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex-shrink-0 border-t border-[var(--border-color)] p-4 space-y-2.5">
                <div className="flex items-center justify-between rounded-xl border border-[var(--border-color)] bg-[var(--bg-surface-2)] px-3 py-2">
