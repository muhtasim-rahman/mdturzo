...
export function Navbar() {
  ...
  // v2.2.9: close mega nav when user scrolls
  useEffect(() => {
    if (!megaOpen) return
    const onScrollClose = () => setMegaOpen(false)
  ...
  const megaTopOffset = floatVisible ? '72px' : 'var(--navbar-h)'

  return (
    <>
      {/* == TOP NAVBAR ====================================== */}
      <nav
        className={isHomePage ? 'absolute left-0 right-0 w-full' : 'relative w-full'}
        style={{
          height: 'var(--navbar-h)',
      ...
      */}
      <AnimatePresence>
        {megaOpen && (
          <>
            {/* Full-screen backdrop — clicking left/right empty space closes */}
            <div
              key="mega-backdrop"
