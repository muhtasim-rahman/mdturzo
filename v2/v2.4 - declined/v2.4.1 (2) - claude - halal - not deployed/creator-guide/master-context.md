...
## v2.4.1 — Projects Overhaul (May 2026)

### What Changed

#### New Files
- `src/components/shared/ImageViewer.jsx` — Fullscreen image popup portal. Exports `useImageViewer` hook + `ImageViewer` component + `attachImageViewerToDOM` helper. Used in ProjectDetail when clicking any image in prose-content or the hero thumbnail.

#### Modified Files
- `src/components/projects/ProjectCard.jsx` — **Complete redesign.** New GridCard (thumbnail → tags → title → desc → footer) and ListCard (accent-bar | thumb | content | links). Also exports `CAT_COLORS` (now includes `PWA: #6366F1`, `Learning: #84CC16`, `Institutional: #0EA5E9`) and `NoProjectsPlaceholder`. No more `.proj-card` class — all Tailwind + Framer Motion.
- `src/pages/Projects.jsx` — New sort dropdown (latest/oldest/liked/viewed/A→Z), URL sync (`?sort=`), scrollable category pills with left/right arrows, responsive pagination (desktop=12 / tablet=8 / mobile=6 via `usePerPage` hook matching CSS breakpoints >1024 / >580).
- `src/pages/ProjectDetail.jsx` — **Full redesign.** Two-column layout (PC: left content + share + comments, right sticky sidebar with links+meta+related). Mobile: related ABOVE comments. New AdvancedShareSection (Twitter/X, LinkedIn, WhatsApp, Telegram, Facebook + Copy + Native Share with pre-filled messages). ImageViewer integration for all prose-content images + hero thumbnail. JSON-LD structured data. Full OG meta tags including `og:image:width/height` and `twitter:card`.
- `src/components/home/RecentProjects.jsx` — Updated to use new card design. Added `EmptyPlaceholder` shown when no featured projects in DB. Fixed static fallback to include `accent` colors.
- `src/services/supabase.js` — `getPublishedProjects` and `getFeaturedProjects` select queries now include `accent, tech_stack, year_built, complexity, sort_order`.
- `src/index.css` — `.prose-content img` updated to add `cursor: zoom-in` and hover effect.

#### New Creator Guide Files
- `creator-guide/supabase-schema-v2.4.1.sql` — Full migration script. Adds 15 new columns (`accent`, `tech_stack`, `year_built`, `complexity`, `project_type`, `platform`, `team_size`, `duration`, `og_image_url`, `screenshots`, `changelog`, `manual_related`, `sort_order`, `extra_links`, `long_description`). **Also fixes the `increment_project_views` RPC 404 bug.**
- `creator-guide/Projects-table-programmer.turzo-2.4.1.csv` — Rich CSV with all 19 projects including all new fields. Import into Supabase.

### Bug Fixed
- `increment_project_views` RPC was returning 404 because the function was not created in Supabase. The SQL migration now includes `CREATE OR REPLACE FUNCTION` to fix this.

### New DB Columns (projects table)
| Column | Type | Notes |
|---|---|---|
| `accent` | text | Hex color override (e.g. `#3B82F6`) |
| `tech_stack` | text[] | Separate from tags (e.g. `{React,Firebase}`) |
| `year_built` | integer | Year project started |
| `complexity` | text | `beginner\|intermediate\|advanced\|expert` |
| `project_type` | text | `PWA`, `Library`, `Website`, etc |
| `platform` | text | `web`, `mobile`, `desktop` |
| `team_size` | integer | 1 = solo |
| `duration` | text | Human-readable (`3 weeks`, `6 months`) |
| `og_image_url` | text | Custom OG image (falls back to thumbnail_url) |
| `screenshots` | text[] | Additional screenshot URLs |
| `changelog` | jsonb | `[{version, date, changes}]` |
| `manual_related` | text[] | Override auto-related slugs |
| `sort_order` | integer | Manual sort priority |
| `extra_links` | jsonb | `[{label, url, icon}]` |
| `long_description` | text | Plain text for SEO |
...
CAT_COLORS = {
  ...
  'Tool': '#F97316', 'Portfolio': '#8B5CF6', 'Design': '#F43F5E',
  'PWA': '#6366F1',        // ← NEW (was missing, causing gray fallback)
  'Learning': '#84CC16',   // ← NEW
  'Institutional': '#0EA5E9', // ← NEW
  'default': '#64748B',
}
```
...
### ImageViewer Usage
```jsx
// In any page that needs image viewer:
import { useImageViewer, ImageViewer, attachImageViewerToDOM } from '../components/shared/ImageViewer.jsx'

const viewer = useImageViewer()
// Attach to prose-content DOM after render:
useEffect(() => attachImageViewerToDOM(contentRef, title, viewer), [project])
// Render:
<ImageViewer {...viewer} />
// Manual open:
viewer.open([{ url: 'https://...', alt: 'description' }], 0, 'Title')
```

### Manual Steps for Turzo After Deployment
1. **Run `supabase-schema-v2.4.1.sql`** in Supabase SQL Editor
2. **Import `Projects-table-programmer.turzo-2.4.1.csv`** into projects table (use Supabase Table Editor → Import CSV)
3. Add thumbnail images for each project and update `thumbnail_url` in Supabase
4. Set `og_image_url` for important projects (should be 1200×630px)
5. Add `tech_stack` arrays for each project (data is in the CSV)
