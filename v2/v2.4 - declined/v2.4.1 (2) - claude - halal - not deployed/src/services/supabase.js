...
export async function getPublishedProjects({ limit, offset = 0, category, tag } = {}) {
  let query = supabase
    .from('projects')
    .select('id, slug, title, short_description, thumbnail_url, github_link, live_link, pdf_link, tags, category, accent, tech_stack, year_built, complexity, sort_order, status, is_featured, featured_order, views_count, likes_count, dislikes_count, created_at, updated_at')
    .eq('status', 'published')
...
export async function getFeaturedProjects() {
  const { data, error } = await supabase
    .from('projects')
    .select('id, slug, title, short_description, thumbnail_url, github_link, live_link, tags, category, accent, tech_stack, is_featured, featured_order, views_count, likes_count')
    .eq('status', 'published')
    .eq('visibility', 'public')
    .eq('is_featured', true)
