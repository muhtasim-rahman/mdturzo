// ProjectDetail.jsx — v2.4.3
// Full redesign:
//   - Hero section: ProjectCarousel (screenshots[] + thumbnail)
//   - PC: 2-col layout (content | sticky sidebar)
//   - Mobile/Tablet: sidebar compact block sits ABOVE tech stack, BELOW title/stats
//   - Related projects: horizontal scrollable row (RelatedProjectsRow)
//     → on PC placed at bottom of left column, ABOVE comments
//     → on mobile same
//   - Tech Stack: redesigned — grouped with visual icon headers
//   - Overall: more polished, airy, minimal but rich
...
import RelatedProjectsRow from '../components/shared/RelatedProjectsRow.jsx'
...
import ProjectCarousel from '../components/shared/ProjectCarousel.jsx'
...
// Safely parse JSONB arrays (Supabase returns real JS arrays, but defensive)
function asArray(v) {
  if (!v) return []
            ...
            <div key={key} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {items.map(item => (
                    <span key={item}
                      className={`text-[10px] font-semibold px-2 py-0.5 rounded-lg border whitespace-nowrap ${
                        accent
...
// ── Compact Sidebar Meta (for mobile placement) ───────────────────
function SidebarMeta({ project, compact = false }) {
  ...
  return (
    <div className={`bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-2xl overflow-hidden ${compact ? '' : ''}`}>
      <div className="h-0.5 bg-gradient-to-r from-[var(--accent-primary)] via-[var(--accent-hover)] to-transparent" />
      <div className={`p-4 space-y-4 ${compact ? 'space-y-3' : ''}`}>
...
function ProjectDetailContent() {
  ...
  // Clicks on <img> inside prose content
  const handleContentClick = (e) => {
    ...
    const imgs = Array.from(contentRef.current.querySelectorAll('img')).map(img => ({
      url: img.src, alt: img.alt || project?.title
    }))
      ...
      <div className="container-xl py-6 lg:py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] xl:grid-cols-[1fr_296px] gap-8">
          ...
          <div className="min-w-0 space-y-8">
            ...
            {/* ── Sidebar: MOBILE ONLY (sits here, above tech stack) ── */}
            ...
            {/* ── Mobile share panel (above related) ── */}
            <div className="block lg:hidden">
