...
export async function getPublishedProjects({ limit, offset = 0, category, tag } = {}) {
  let query = supabase
    .from('projects')
    .select('id, slug, title, short_name, tagline, short_description, detailed_description, thumbnail_url, github_link, live_link, pdf_link, custom_link, demo_link, tags, category, subcategory, type, status, visibility, is_featured, featured_order, development_status, views_count, likes_count, dislikes_count, created_at, updated_at, platform, languages, tech_stack, key_features, dependencies, version, complexity_level, project_timeline, is_open_source, notes')
    .eq('status', 'published')
...
export async function getFeaturedProjects() {
  const { data, error } = await supabase
    .from('projects')
    .select('id, slug, title, short_description, thumbnail_url, github_link, live_link, tags, category, is_featured, featured_order, views_count, likes_count')
    .eq('status', 'published')
    .eq('visibility', 'public')
    .eq('is_featured', true)
