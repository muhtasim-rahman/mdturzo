...
## v2.4.0 — Projects Page: Full Build (2026-05-31)

### New
- **Projects page (`/projects`)** — Full build from placeholder
  - Hero section with title, badge, animated project count
  - Sticky controls bar: debounced search, category filter chips, grid/list toggle
  - URL-synced filters (`?q=...&cat=...`) via `useSearchParams`
  - Grid view (3→2→1 col) and List view (row layout with thumbnail)
  - Load More pagination (9 items per page)
  - Skeleton loading, empty state, error state with retry
  - Active filter pills with individual/clear-all dismissal
- **Project detail page (`/projects/:slug`)** — Full build from placeholder
  - Full SEO: Helmet with OG tags, canonical, JSON-LD SoftwareApplication schema
  - Breadcrumb navigation
  - Hero thumbnail with gradient glow
  - Two-column layout: content + sidebar
  - TipTap HTML rendering with full prose styles
  - Interaction bar: LikeDislike + ShareButtons + ReportButton
  - CommentSection (login + email verify required, 10/day limit)
  - Sidebar: Project Info, Links, Tags, Back button
  - Related projects (3 cards, same category)
  - Auto view increment (session deduped)
- **New shared components:**
  - `Breadcrumb.jsx` — reusable breadcrumb with home icon
  - `LikeDislike.jsx` — like/dislike with optimistic UI, auth check
  - `ShareButtons.jsx` — 5 platforms + copy link popup
  - `ReportButton.jsx` — report modal with reason chips
  - `CommentSection.jsx` — full comment list + submit form

### Changed
- **`RecentProjects.jsx`** (home page) — removed all hardcoded fallback data; now 100% dynamic from Supabase; shows empty state if no featured projects; error state with retry
- **`supabase.js`** — added many helpers: getProjectCategories, countPublishedProjects, incrementProjectViews, getRelatedProjects, getUserVote, toggleLike, submitComment, checkCommentRateLimit, submitReport; fixed getFeaturedProjects uses `is_featured`; added search to getPublishedProjects
- **`formatters.js`** — added `formatDistanceToNow`, `formatDateShort`

### Database
- New column: `projects.accent_color TEXT DEFAULT '#3B82F6'`
- New column: `feed.accent_color TEXT DEFAULT '#3B82F6'`
- New RPC: `increment_project_views(UUID)`
- New RPC: `increment_feed_views(UUID)` (prep for v2.5.0)
- Run `creator-guide/supabase-v2.4.0.sql`
