...
// NAVBAR — v2.2.6
// FIXES applied:
//   • nav-items click/active scale effect (CSS + :active)
//   • floating navbar overflow fixed: center nav flex-1 min-w-0; icons flex-shrink-0
//   • floating mega nav breaks out to full viewport width (position:fixed wrapper)
//   • nav-icon-btn border-radius 9999px (fully round) — done in index.css
//   • nav-icon-btn overflow:clip — tooltips now visible (::after escapes clip boundary)
//   • float-nav-right nav-icon-btn border-radius ensured via CSS
// ============================================================
...
// ── Mega Menu ─────────────────────────────────────────────────
// isFloat: when true, parent positions it via position:fixed — no extra style needed inside
      ...
      <AnimatePresence>
        {floatVisible && (
          <motion.div
            ...
            {/* FIX v2.2.6: float nav inner — w-full, overflow:visible for mega/tooltips */}
            <nav className="float-nav nf-inner relative pointer-events-auto flex items-center gap-4 w-full h-[52px] px-4 rounded-full" style={{overflow:'visible'}}>
              <NavLogo size="sm" rounded />

              {/* Center nav — FIX: flex-1 + min-w-0 + overflow:hidden prevents overflow */}
                ...
                {NAV_LINKS.map(link => (
                  <NavLink key={link.path} to={link.path} end={link.path === '/'}
                    className={({ isActive }) => {
                      ...
                      return `relative overflow-hidden flex items-center gap-1.5 h-9 px-3.5 rounded-full text-sm font-medium leading-none transition-all float-nav-link flex-shrink-0 ${isAct ? 'float-nav-link-active' : ''}`
                    }}
                    title={link.title}>
                    <FontAwesomeIcon icon={link.icon} className="text-xs opacity-80" />
              ...
              {/* Right — FIX: flex-shrink-0 prevents icons from being squeezed */}
              ...
              {/* FIX v2.2.6: Float-nav mega breaks out to full viewport width using position:fixed */}
              {megaOpen && (
