// RecentProjects.jsx — v2.2.6
// FIXES:
//   • Card hover border = card.color (via onMouseEnter/Leave state, not always --accent-primary)
//   • Only title <Link> is clickable, NOT the full card (removed full-card hover click area)
//   • Title color changes on title hover only (not entire card)
...
function ProjectCard({ p, i }) {
  ...
  // v2.2.6 FIX: hover border uses card's own color, not always --accent-primary
  ...
  return (
    <motion.div
      ...
      {/* Thumbnail */}
      <div className="relative h-40 bg-[var(--bg-surface-2)] overflow-hidden flex-shrink-0 group">
        {p.thumbnail_url
        ...
        {/* GitHub + live links — always clickable, stop propagation */}
      ...
      {/* Content — v2.2.6 FIX: only title Link is clickable, not full card */}
      <div className="p-4 flex flex-col gap-2.5 flex-1">
        ...
        {/* v2.2.6 FIX: title Link — only this element is clickable + hover color */}
        <Link to={`/projects/${p.slug}`}
          className="font-display font-bold text-[var(--text-primary)] leading-snug line-clamp-2 text-sm hover:text-[var(--accent-primary)] transition-colors duration-200 w-fit max-w-full"
        ...
        <div className="pt-2 border-t border-[var(--border-color)]">
          {/* v2.2.6: "View details" row — Link is only the arrow+text, not full card */}
          <Link to={`/projects/${p.slug}`}
            className="flex items-center justify-between text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--accent-primary)] transition-colors duration-200 group/link">
