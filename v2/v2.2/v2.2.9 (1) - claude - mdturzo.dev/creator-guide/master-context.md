...
## v2.2.9 — Margin/Alignment Fix, GitHub Redesign, Hero Polish, Project Cards Redesign, Navbar Fixes

### Summary
Complete polish pass: margin/alignment consistency, GitHub section redesign, hero text/color fixes, minimal project cards, navbar tooltip + mega nav behavior, setup section update with real data, global click effects.

### Changed files

- `src/config/site.config.js` — version bumped to `v2.2.9`

- `package.json` — version bumped to `2.2.9`

- `src/components/home/Hero.jsx`:
  - `"Muhtasim Rahman"` unified in single `<span class="hname-main">` (white/text-primary); `(Turzo)` in `.hname-turzo-wrap` with `color:var(--accent-primary)`
  - Large screens: `(Turzo)` is `position:absolute; right:0; top:100%` — sits below-right of name line without affecting layout
  - Mobile/tablet (≤899px): `(Turzo)` becomes `position:static; display:block` below name
  - `min-height:100dvh` + `display:flex; align-items:center` only on `@media(min-width:900px)` — mobile/tablet stays `height:auto`
  - `himg-frame::after` gradient: `bottom:-5px` (was `bottom:0`) — prevents zoom gap
  - Hero inner uses `max-width:1120px` + `padding-inline:clamp(1rem,4vw,1.75rem)` matching `container-xl` exactly
  - Mobile xs (≤479px): `.hbtn-dl-text` hidden, `.hcta` has `flex-wrap:nowrap` — both buttons on same row
  - `Assalamu Alaikum` greeting: `.hgreet-salam` dark mode=`#F59E0B`, light mode=`var(--accent-primary)`; `.hgreet-icon` same pattern
  - `--` dashes in bio/greeting replaced with `—` em dash

- `src/components/home/AboutMini.jsx`:
  - `?` bullet characters in FACTS array values replaced with `·` (U+00B7 middle dot): SSC-26 · SGSC, Bengali · English · Hindi, Islam · Discipline · Quality
  - Age/location text: `Age {age} · Bangladesh`
  - Version header bumped to v2.2.9

- `src/components/home/GithubStats.jsx` — FULL REDESIGN v2:
  - Profile card: LEFT = avatar + name (link) + `@username` (clickable link) + `•` join date + bio + location; RIGHT = 5 compact vertical stat cards (Repos/Stars/Followers/Forks + GitHub redirect card with external icon)
  - Stat cards are `.gh-pcard` — 90px wide column on desktop, wrapping row on mobile
  - 5th card `.gh-pcard-redirect` has gradient bg, GitHub icon, links to profile
  - Two-col image panels: `border:none`, dark=`var(--bg-surface-2)` bg, light=`#ffffff` bg; `StatImagePanel` component with skeleton loading + onLoad/onError handling
  - Trophies section REMOVED entirely
  - API rate-limit: `retryRef` setTimeout — 3 retries × 5s, then 60s wait; manual "Retry Now" resets count
  - Repos grid: desktop 6, tablet 4 (`:nth-child(n+5){display:none}`), mobile 3 (`:nth-child(n+4){display:none}`)
  - `ProfileSkeleton` component for loading state

- `src/components/home/RecentProjects.jsx` — MINIMAL CARD REDESIGN:
  - Top `3px` accent line appears on hover (gradient from `--c` to transparent)
  - Header row: 36px icon box (thumbnail or folder icon) + category pill + external links (GitHub/live)
  - Title (`.proj-card-title`) + description (2-line clamp) + tag pills (mono font, bg-surface-2)
  - Bottom CTA row with "View details" + arrow; arrow slides `3px` right on hover
  - `active:scale(0.97)` click effect with fast `0.08s` transition
  - Full-card Link overlay at z-index:1; external links at z-index:2

- `src/components/home/Process.jsx` — SETUP SECTION UPDATE:
  - TOOLSET updated to Turzo's actual tools: VS Code, Git/GitHub, HTML+CSS, Firebase, Tailwind CSS, Graphic Design, Video Editing, PWA/Mobile
  - PRINCIPLES updated: clean code, mobile-first, halal/ethical, self-learning
  - "Currently learning" row added at bottom-right: TypeScript · Next.js · Node.js · React
  - Light mode: tool cards use `background:#fff`, principles card uses `background:#fff`, editor stats use `background:#fff`
  - Editor mockup CODE_LINES updated with Turzo-themed comments
  - Hosting stat updated: "Firebase / GitHub Pages"

- `src/components/home/Services.jsx`:
  - `--` dashes in description text replaced with `—` em dashes

- `src/components/layout/Navbar.jsx`:
  - `[data-nb-right] [data-nb-tip]::after`: tooltip now `left:50%; transform:translateX(-50%)` — centered below button (was `right:0` aligned to edge)
  - `onScroll` handler: when `scrollY > FLOAT_THRESHOLD`, also calls `setMegaOpen(false)`
  - Mega overlay div: added `bottom:0` to cover full viewport; `onClick` handler closes mega when clicking outside `.mega-glass-inner`
  - Version header bumped to v2.2.9

- `src/index.css`:
  - Appended v2.2.9 section: global click effects for `.card:active` (`scale(0.98)`) and `[data-click-fx]:active` (`scale(0.95)`)
  - `@keyframes marquee-scroll` added (was previously only in Navbar `<style>` block — now in global CSS for consistency)
  - `#about-mini .container-xl` and `#hero .hero-inner` get explicit `margin-left:auto; margin-right:auto` for centering consistency

### Important notes for next AI (v2.3.0+)
- Hero: `(Turzo)` is `position:absolute` on large screens in `.hname-line` (which has `position:relative`). Do NOT remove `position:relative` from `.hname-line`.
- GitHub: Trophies section is permanently removed. Do not re-add.
- GitHub: `retryRef` is a `useRef` storing the setTimeout ID — cleared on unmount via `return () => clearTimeout(retryRef.current)`.
- Project cards: `.proj-card-overlay` is a `<Link>` at z-index:1; external link buttons are z-index:2.
- Process/Setup section: uses Turzo's actual tools. Keep accurate. `learning` row uses `·` separator.
- Navbar mega overlay: outer div has `bottom:0` and click handler. The `mega-glass-inner` class is the inner panel — clicking it doesn't close.
- `marquee-scroll` keyframe is now in global `index.css` AND in Navbar's inline style. Both should be kept in sync if changed.
