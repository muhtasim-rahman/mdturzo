...
export function Navbar() {
  ...
  useEffect(() => {
    // v2.2.9: close mega nav on scroll too (when float nav appears)
    const onScroll = () => {
      const past = window.scrollY > FLOAT_THRESHOLD
      setFloatVisible(past)
      if (past) setMegaOpen(false)
    }
      ...
      */}
      <AnimatePresence>
        {megaOpen && (
          <div
            key="mega-overlay"
            style={{ position:'fixed', top: megaTopOffset, left:0, right:0, bottom:0, zIndex:10000 }}
            onClick={(e) => { if (!e.target.closest('.mega-glass-inner') && !e.target.closest('.mega-share-btn') && !e.target.closest('.mega-url-pill')) setMegaOpen(false) }}>
      ...
      <style>{`
        ...
        /* v2.2.9: tooltip centered below each button, not right-aligned */
