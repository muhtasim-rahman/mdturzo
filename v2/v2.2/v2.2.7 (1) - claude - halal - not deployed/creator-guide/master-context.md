...
#### 1. Scroll Behavior
- Removed `scroll-snap-type: y proximity` from `html` in `index.css` — was causing unwanted auto-alignment
- Removed `scroll-snap-align: start` from `.section`
- Reduced `scroll-padding-top` to `calc(var(--navbar-h) + 8px)` for tighter anchor positioning
- Kept `scroll-behavior: smooth` for soft native scrolling

#### 2. Navbar — Complete Redesign (`Navbar.jsx`)
- **Home page**: `nb-top--home` class makes top navbar fully transparent — hero bg visible through
- **Non-home pages**: `--navbar-bg` glass with blur
- **z-index**: Raised all z-index vars (`--z-sticky: 300`, `--z-mega: 1000`) — mega nav never goes under page content (404 fix)
- **Mega menu**: Uses `position: fixed` + `z-index: var(--z-mega)` — always above all page content
- **Tooltips**: Icon buttons use `overflow: clip` (not `hidden`) — `::after` pseudo-tooltip escapes the clip boundary
- **Click effects**: `:active { transform: scale(.88) }` on all `.nb-icon` buttons
- **Float navbar**: `flex-1 min-w-0 overflow:hidden` on center links + `flex-shrink-0` on right icons — no overflow
- **Float right icons**: `.nb-float-right` class overrides icon button bg/border for glass pill context
- **Mega menu positioning**: `navbarHeight` prop passed to `<MegaMenu>` — calculated via ResizeObserver on nav element
- **Mobile sidebar**: Full slide-in sidebar with search, nav links, user card, social row

#### 3. Hero — 12+ Breakpoints, No Floating Icons (`Hero.jsx`)
- **ResizeObserver removed**: Was causing infinite height feedback loop (image height → grid reflow → left content height change → observer triggers → repeat)
- **Floating tech icons removed entirely**: Cleaner, more minimal look
- **Pure CSS sizing**: `h-img-wrap` uses `height: clamp(340px, calc(100dvh - var(--navbar-h) - 4rem), 680px)` — bounded, never grows infinitely
- **12+ breakpoints**: 1920px, 1440px, 1280px, 1024px, 900px, 768px, 640px, 480px, 414px, 375px, ≤374px, landscape phones (max-height:500px)
- **Portrait override**: Image column goes above content on portrait phones
- **Minimal visual**: Subtle dot-grid, rising particles, orbs, bottom gradient — no floating icons

#### 4. Featured Projects (`RecentProjects.jsx`)
- **Full card clickable**: Overlay `<Link>` positioned `absolute inset-0 z-index:1` — entire card navigates to detail page
- **External links (GitHub/Live)**: `z-index: 2` + `e.stopPropagation()` — clickable above the overlay
- **Title**: No color change on hover — static `var(--text-primary)` always
- **Card footer**: Redesigned with card's `accent` color via `--c` CSS var — colored dot, label, "View details" cta
- **Fallback data**: Added `accent` field to fallback project entries
- **Admin note**: When adding projects in Admin Panel, set `accent` color (hex) to control the card's theme color — visible in card footer border, tag badges, and hover border

#### 5. Skills Redesign (`Skills.jsx`)
- **Left column**: 2×2 stat cards (3+ Dev, 6+ Design, 16+ Projects, 5+ Languages) with colored left-accent border + icon
- **Left column**: Bio text + 3 bullet points with glowing colored dots
- **Right column**: Tab buttons (Skills / Tools / Learning) + colorful gradient progress bars (unique gradient per skill)
- **Bar colors**: HTML=orange/red, JS=yellow, React=cyan, Python=blue, Tailwind=sky, Firebase=amber, etc.
- **Shimmer animation**: `::after` with translateX(-100%→100%) sweep on each bar

#### 6. Journey — New Section (`Journey.jsx`)
- **Location**: Between Skills and Services in home page order
- **Background**: Plain (not section-alt)
- **Design**: Alternating left-right vertical timeline on desktop, single-column on mobile
- **Milestones**: 2019 (first code) → 2020 (design) → 2021 (first real project) → 2022 (React) → 2023 (freelance) → 2024 (16+ projects)
- **Interactive**: Card hover lifts + border glows with milestone color

#### 7. GitHub Activity (`GithubStats.jsx`)
- **FIXED**: Streak stats now uses `streak-stats.demolab.com` (DenverCoder1) — replaces defunct `github-readme-streak-stats.herokuapp.com`
- **Added**: GitHub Profile Trophies via `github-profile-trophy.vercel.app`
- **Added**: Achievement badges section — 6 badges built from live repo data (Early Bird, Milestone, Star Gazer, Polyglot, Shipped 16+, Open Source)
- **Fallback**: If any image fails to load, a custom fallback card renders with icon + stats
- **URL pattern** for streak: `https://streak-stats.demolab.com/?user=USERNAME&hide_border=true&theme=transparent&...`

#### 8. CTA Section (`CTA.jsx`)
- **Removed**: "Available for freelance & collaboration" pill badge
- **Added**: Animated star icon with rotate/scale animation
- **Added**: Feature pills row (Fast response, Clean code, On-time delivery, Pixel-perfect design)
- **Added**: LinkedIn button alongside GitHub
- **Redesigned**: Card-style centered panel with gradient glass border + backdrop blur
- **Gradient heading**: "amazing together" uses blue→indigo→purple gradient text

#### 9. Home Section Order (`Home.jsx`)
```
Hero → AboutMini (alt) → RecentProjects → Skills (alt) → Journey → Services (alt) → Testimonials → GithubStats (alt) → CTA
```
- BlogMini removed from home page (was between GithubStats and CTA)

#### 10. z-index Map (updated)
```
--z-dropdown: 200
--z-sticky:   300   ← navbar, float pill
--z-mega:     1000  ← mega menu (fixed, always on top)
--z-modal:    1400
--z-toast:    1500
--z-progress: 1600
```

### Admin Panel — Project Accent Colors
When adding or editing a project in the Admin Panel, set the `accent` field (hex color string, e.g. `#3B82F6`) to control:
- Card hover border color
- Card footer background + border tint
- Footer label color + dot glow
- Tag badge background/color
- Category badge tint

Suggested accent colors by category:
- Web App: `#3B82F6` (blue)
- Utility: `#10B981` (green)
- Education: `#F59E0B` (amber)
- UI Component: `#EC4899` (pink)
- Dev Tool: `#A855F7` (purple)
- Islamic: `#06B6D4` (cyan)
