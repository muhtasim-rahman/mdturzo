...
// NAVBAR -- v2.2.8
// FIXES:
//   * Double search icon removed -- NavRight has ONE search only (desktop-only component)
//   * Hamburger removed from NavRight -- it was conflicting with nb-icon-btn display:inline-flex
//     Hamburgers remain in the tablet/mobile sections outside NavRight
//   * Mega nav double-render FIXED: MegaMenu rendered ONCE outside both navbars
//     Uses floatVisible to determine correct top position
//   * Mega nav outside-click FIXED: transparent backdrop overlay added behind MegaMenu
//   * CSS tooltips kept via data-nb-tip (already working); [data-nb-right] for right-anchor
// ============================================================
...
// -- Right controls cluster (DESKTOP ONLY -- no hamburger, ONE search)
function NavRight({ user, profile, isAdmin: isAdminProp, avatar, displayName, isLoggedIn, authLoading, unreadCount, openSearch, notifOpen, setNotifOpen, userOpen, setUserOpen, megaOpen, setMegaOpen }) {
  return (
    <div className="flex items-center gap-1.5 flex-shrink-0" data-nb-right>
      ...
      {/* NO hamburger here -- hamburger only in tablet/mobile sections */}
    </div>
  )
}
...
export function Navbar() {
  ...
  // Mega nav top offset based on which navbar is active
  const megaTop = floatVisible ? '72px' : 'var(--navbar-h)'
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {floatVisible && (
          <motion.div
            ...
            className="fixed top-0 left-0 right-0 flex justify-center px-4 pt-3 pointer-events-none"
            style={{zIndex: 200}}>
      ...
      {/* == MEGA NAV -- rendered ONCE here, outside both navbars == */}
      <AnimatePresence>
        {megaOpen && (
          <>
            ...
            <div
              key="mega-panel"
              style={{position:'fixed', top: megaTop, left:0, right:0, zIndex:10000}}
            >
      ...
      <style>{`
        ...
        /* Right-anchored tooltips for right-side cluster */
        ...
        .mega-item--active svg, .mega-item--active span {
          color: var(--accent-primary) !important;
        }
        ...
        .mega-share-btn {
          ...
          flex-shrink: 0; box-shadow: 0 2px 8px rgba(59,130,246,0.28); border: none;
        }
        .mega-share-btn:hover {
          background: linear-gradient(135deg, var(--accent-hover) 0%, #4f46e5 100%);
          box-shadow: 0 4px 14px rgba(59,130,246,0.38); transform: translateY(-1px);
        }
        .mega-share-btn:active { transform: scale(0.95); }
        .mega-url-pill {
          height: 30px; min-width: 0; flex: 1; display: flex; align-items: center;
          gap: 4px; border: 1px solid var(--border-color); border-radius: 999px;
