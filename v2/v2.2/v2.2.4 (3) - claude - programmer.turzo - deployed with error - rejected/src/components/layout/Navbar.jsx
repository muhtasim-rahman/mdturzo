...
// Search popup removed in v2.2.4 (planned for future version)
// Mobile sidebar search bar kept for design purposes only
...
function MegaMenu({ onClose, isLoggedIn, floating = false }) {
  ...
  const itemClass = (path) => {
    ...
    return `relative overflow-hidden flex items-center gap-2.5 px-2.5 py-2 rounded-lg border transition-all duration-150 group ${active ? 'is-mega-active bg-[var(--accent-light)] text-[var(--accent-primary)] border-[rgba(59,130,246,0.2)]' : 'border-transparent hover:bg-[var(--bg-surface-3)] hover:border-[var(--border-color)]'}`
  }
  return (
    <AnimatePresence>
...
// ── Main Navbar Export ────────────────────────────────────────
// DesktopSearchPopup removed in v2.2.4
...
export function Navbar() {
  ...
  // Detect home page for transparent navbar
  // NOTE: To make any page use transparent top navbar, add logic here or use a data attribute.
  // Currently only '/' and '/home' use transparent mode.
  ...
  // Desktop search: show toast (full search planned for future version)
  const handleDesktopSearch = () => {
    toast.info('Search coming soon', 'Full search will be available in a future update.')
  }
            ...
            <motion.aside key="sidebar" variants={sidebarVariants} initial="closed" animate="open" exit="closed"
              ...
              */}
              <div className="px-4 mt-3 flex-shrink-0">
                <div className="sidebar-search-field flex items-center gap-2.5 px-4 py-2.5 rounded-full border transition-all duration-200"
                  ...
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')}
                      className="w-5 h-5 flex items-center justify-center rounded-full bg-[var(--bg-surface-3)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors flex-shrink-0">
              ...
              <div className="flex-1 overflow-y-auto py-3 sidebar-scroll">
                {isSearching ? (
                  /* Sidebar search: coming soon placeholder */
                  <div className="flex flex-col items-center gap-3 px-4 py-10 text-center">
                    <div className="w-12 h-12 rounded-2xl bg-[var(--accent-light)] border border-[var(--border-color)] flex items-center justify-center text-[var(--accent-primary)]">
                      <FontAwesomeIcon icon={faSearch} className="text-lg"/>
                    </div>
                    <p className="text-sm font-semibold text-[var(--text-primary)]">Search coming soon</p>
                    <span className="text-xs text-[var(--text-tertiary)] leading-relaxed max-w-[200px]">
                      Full search will be available in a future update.
                    </span>
                  </div>
                ) : (
                  <>
