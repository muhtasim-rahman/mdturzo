...
//   * Large screen: "(Turzo)" absolute positioned below name, right-aligned, primary color
//   * "Muhtasim Rahman" single span, both white — no accent split
//   * "(Turzo)" is primary color
//   * Hero image gradient: bottom:-5px to prevent gap on zoom
//   * Replaced "--" dashes with "—" em dashes
//   * Mobile: Download CV = icon only, both CTA buttons in same row
//   * Light mode: "Assalamu Alaikum" color fixed to accent-primary
//   * Large screen: max-height set
...
/* -- Main inner grid ------------------------------------ */
/* v2.2.9: aligned to 1120px to match navbar + container-xl */
.hero-inner{
...
@media(min-width:900px){
  .hero-inner{ min-height:min(100dvh, 960px); }
}
...
/* v2.2.9: salam text uses accent color in light mode too */
...
.hname{
  ...
  letter-spacing:-.03em;font-family:var(--font-display);color:var(--text-primary);
  position:relative;
}
...
/* v2.2.9: Muhtasim Rahman both white — no accent split */
.hname-main{color:#FFFFFF}
[data-theme=light] .hname-main{color:var(--text-primary)}

/* v2.2.9: (Turzo) — primary color always */
/* Large screen: absolute below name, right-aligned */
.hname-turzo{
...
/* v2.2.9: on large screen, add top margin to not overlap (Turzo) absolute */
@media(min-width:900px){
  .hrole{ margin-top:.5em; }
}
...
.hbtn-p:active{transform:scale(.96)!important}
...
.hbtn-o:active{transform:scale(.96)!important}
...
/* v2.2.9: mobile — hide "Download" text, show only icon */
@media(max-width:639px){
  .hbtn-dl-text{ display:none; }
  .hbtn{ padding:.58rem .9rem; }
}
...
.hsocial:active{transform:scale(.93)!important}
...
/* v2.2.9: gradient starts -5px (extends below image edge) to prevent zoom gap */
.himg-frame::after{
  content:'';position:absolute;z-index:2;pointer-events:none;
  bottom:-5px;left:0;right:0;height:calc(38% + 5px);
...
@media(max-width:899px){
  .hero{min-height:100dvh;height:auto;max-height:none}
  .hero-inner{
        ...
        <div className="hero-inner">
          ...
          <div className="hcontent">
            <div className="hgreet hup" style={{'--d':'0ms'}}>
              ...
              {/* v2.2.9: em dash instead of -- */}
              <span className="hgreet-rest">I am —</span>
            </div>

            {/* v2.2.9: "Muhtasim Rahman" single span, white. "(Turzo)" absolute below on lg, primary color */}
