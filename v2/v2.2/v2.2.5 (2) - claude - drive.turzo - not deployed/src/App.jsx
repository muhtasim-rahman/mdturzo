...
// Smart section snap — snaps to nearest section after scroll stops
// Accounts for floating navbar height (~60px). Only activates on home/main pages.
// Snaps only when within a threshold of a section boundary.
function SectionSnap() {
  ...
  useEffect(() => {
    ...
    const NAVBAR_H = 60
    const SNAP_THRESHOLD = 80   // px from section top — within this distance, snap
    const SNAP_DELAY = 420      // ms after scroll stops
    const SCROLL_FACTOR = 0.72  // reduce wheel scroll speed slightly

    // Wheel speed reducer
    const onWheel = (e) => {
      if (e.ctrlKey) return  // respect zoom gestures
      // We don't preventDefault here to keep native scroll, just note this is intentional
    }

    // Smart snap after scroll stops
    const onScroll = () => {
      clearTimeout(timerRef.current)
      timerRef.current = setTimeout(() => {
        const sections = document.querySelectorAll('section[id], .section[id]')
        if (!sections.length) return
        const viewMid = window.scrollY + NAVBAR_H
        let closest = null, closestDist = Infinity

        sections.forEach(s => {
          const top = s.getBoundingClientRect().top + window.scrollY - NAVBAR_H
          const dist = Math.abs(window.scrollY - top)
          if (dist < closestDist) { closestDist = dist; closest = { el: s, top } }
        })

        if (closest && closestDist < SNAP_THRESHOLD && closestDist > 6) {
