...
// Hero.jsx — v2.2.5
// Changes:
//   - Material Symbols waving_hand replaces custom SVG icon
//   - Left content vertically centered via align-items:center on hinner grid
//   - ResizeObserver: image height = leftContentHeight * 1.1
//   - Round frame ONLY at <=900px (portrait orientation rule removed)
//   - Floating icons just outside circular frame border on tablet/mobile
...
export default function Hero({settings,settingsLoading}){
  ...
  // v2.2.5: Track stacked layout breakpoint (<=900px)
  useEffect(()=>{
    const check = () => setIsStacked(window.innerWidth <= 900)
    check()
    window.addEventListener('resize', check, {passive:true})
    return () => window.removeEventListener('resize', check)
  },[])

  // v2.2.5: ResizeObserver on left content — drive image height
  useEffect(()=>{
    if(!leftRef.current) return
    const ro = new ResizeObserver(entries => {
      for(const entry of entries){
        const h = entry.contentRect.height
        if(h > 50) setLeftHeight(h)
      }
    })
  ...
  // Image container height: 10% taller than left content (PC only)
  const imgHeight = (!isStacked && leftHeight) ? `${leftHeight * 1.1}px` : 'auto'

  return(
    <>
<style>{`
/* ── Hero base ── */
.hero{position:relative;height:100dvh;max-height:760px;min-height:580px;display:flex;align-items:center;overflow:hidden}
...
/* v2.2.5: hinner uses layout tokens + align-items:center */
.hinner{position:relative;z-index:5;display:grid;grid-template-columns:1.18fr 1fr;gap:clamp(1rem,3vw,2.5rem);align-items:center;width:100%;height:100%;max-width:var(--layout-max-w,1120px);margin-inline:auto;padding-inline:var(--layout-px,clamp(1rem,4vw,2rem));padding-block:clamp(2rem,4vh,3rem)}
.hcontent{display:flex;flex-direction:column;justify-content:center;gap:clamp(.65rem,1.6vh,1.1rem);position:relative;z-index:6;align-self:center}
...
.hgreet-icon{color:#F59E0B;font-size:1.3em;vertical-align:middle;flex-shrink:0;position:relative;top:1px;line-height:1;font-style:normal}
...
/* v2.2.5: hvisual — image container, no fixed width constraint */
.hvisual{position:relative;z-index:5;display:flex;align-items:center;justify-content:center;opacity:0;animation:hvis-in .85s cubic-bezier(.16,1,.3,1) .2s forwards}
@keyframes hvis-in{from{opacity:0;transform:translateX(22px) scale(.94)}to{opacity:1;transform:none}}
/* hscene: width fit-content on PC, no max-width clipping */
.hscene{position:relative;width:fit-content;flex-shrink:0;align-self:center}
...
.hwrap{position:relative;z-index:1;width:clamp(220px,22vw,310px);border-radius:16px;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.25)}
[data-theme=dark] .hwrap{box-shadow:0 12px 40px rgba(0,0,0,.5)}
.hphoto{width:100%;height:100%;object-fit:contain;object-position:top center;display:block}
...
[data-theme=light] .hbot-fade{background:linear-gradient(to top,var(--bg-page) 0%,var(--bg-page) 8%,rgba(238,242,247,.92) 34%,rgba(238,242,247,.44) 56%,transparent 100%)}
...
[data-theme=light] .hgrad-1{background:linear-gradient(to top,var(--bg-page) 0%,rgba(238,242,247,.75) 22%,rgba(238,242,247,.3) 46%,transparent 100%)}
...
[data-theme=light] .hgrad-2{background:linear-gradient(to top,rgba(238,242,247,.15) 0%,rgba(238,242,247,.07) 24%,transparent 48%)}

/* v2.2.5: <=900px stacked layout — round frame + icon positions just outside border */
@media(max-width:900px){
  ...
  /* Round frame activates ONLY at <=900px (stacked layout) */
  .hwrap{width:clamp(175px,46vw,240px) !important;height:clamp(175px,46vw,240px) !important;border-radius:50% !important;overflow:hidden;box-shadow:0 0 0 4px var(--bg-page),0 0 0 6px var(--border-color),0 0 30px rgba(37,99,235,.16);animation:hring-p 3s ease-in-out infinite}
  .hglow{border-radius:50%}
  .hphoto{object-position:top center}
  .hbot,.hgrad-1{display:none}
  .hscroll-btn{display:none}
  /* v2.2.5: Floating icons — just outside circle border on tablet */
  .hicon{--sz:38px !important;border-radius:11px}
  /* Frame radius = ~50% of width. For 240px frame: radius=120px. Icons sit at circle edge. */
  /* Using calc(50% - radius - iconHalf) logic relative to hscene center */
  .hicon-html{top:-14px !important;left:calc(50% - 100px) !important;right:auto !important}
  .hicon-css{top:50% !important;left:-14px !important;right:auto !important;transform:translateY(-50%);animation-name:hicon-fy}
  .hicon-python{bottom:-14px !important;top:auto !important;left:calc(50% - 100px) !important;right:auto !important}
  .hicon-vscode{top:-14px !important;left:auto !important;right:calc(50% - 100px) !important}
  .hicon-design{top:50% !important;right:-14px !important;left:auto !important;transform:translateY(-50%);animation-name:hicon-fy;display:flex}
  /* hscene must wrap properly for icon positioning */
  .hscene{width:fit-content !important;height:auto !important;position:relative}
}
...
@media(max-width:520px){
  ...
  .hwrap{width:clamp(150px,44vw,185px) !important;height:clamp(150px,44vw,185px) !important}
  /* Icons just outside smaller circle */
  .hicon{--sz:32px !important;padding:6px;border-radius:9px}
  .hicon-html{top:-12px !important;left:calc(50% - 80px) !important}
  .hicon-css{left:-12px !important}
  .hicon-python{bottom:-12px !important;left:calc(50% - 80px) !important}
  .hicon-vscode{top:-12px !important;right:calc(50% - 80px) !important}
  .hicon-design{right:-12px !important}
}
@media(min-width:1400px){.hwrap{width:clamp(280px,18vw,330px)}}
@media(min-height:880px) and (min-width:901px){.hero{max-height:760px}}
@media(max-height:580px) and (min-width:521px){.hero{height:auto;max-height:none;min-height:100dvh}}
`}</style>
      ...
      <section className="hero" id="hero" aria-label="Introduction">
        ...
        <div className="hinner">
          {/* LEFT CONTENT — vertically centered via hcontent align-self:center */}
          <div className="hcontent" ref={leftRef}>
            <div className="hgreet hup" style={{'--d':'0ms'}}>
              <span className="hgreet-salam">Assalamu Alaikum</span>
              {/* v2.2.5: Google Material Symbols waving_hand icon */}
              <span className="material-symbols-outlined hgreet-icon" aria-hidden="true">waving_hand</span>
          ...
          {/* RIGHT VISUAL — image height driven by ResizeObserver on left content */}
          <div className="hvisual" aria-hidden="true">
            <div className="hscene">
              <div className="hglow"/>
              {/* v2.2.5: hwrap height = leftHeight * 1.1, no fixed max-width, object-fit:contain */}
              <div className="hwrap" style={!isStacked && imgHeight !== 'auto' ? {height: imgHeight} : {}}>
                <img src="/hero.webp" alt="Muhtasim Rahman" className="hphoto"
                     loading="eager" fetchPriority="high"/>
              </div>

              {/* Futuristic bottom bar — PC only */}
              {!isStacked && (
                <div className="hbot">
