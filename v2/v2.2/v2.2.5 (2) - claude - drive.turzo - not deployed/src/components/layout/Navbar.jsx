...
// NAVBAR — v2.2.5
// Changes from v2.1.3:
//   - Desktop search popup REMOVED — clicking search shows "Search coming soon" toast
//   - Ctrl+K global shortcut → toast instead of popup
//   - Nav link hover: distinct pill highlight (accent tint bg + border)
//   - Mega menu glass always applied (not only when floating)
//   - Right-side icon tooltips: glass style, always visible
//   - Floating navbar right buttons: minimal glass bg
//   - navbar-inner uses --layout-max-w / --layout-px (unified with container-xl)
//   - Mobile sidebar search kept fully functional
...
function getSidebarResults(query) {
  const q = query.trim().toLowerCase()
  if (!q) return SEARCH_ITEMS.slice(0, 6)
  return SEARCH_ITEMS
    .filter(i => `${i.label} ${i.group} ${i.path} ${i.keywords}`.toLowerCase().includes(q))
    .slice(0, 8)
}
...
// ── Mega Menu ─────────────────────────────────────────────────
// v2.2.5: glass always applied (mega-glass class), not just when floating
function MegaMenu({ onClose, isLoggedIn }) {
  ...
  const copyUrl = async () => {
    try { await navigator.clipboard.writeText(currentUrl); toast.success('Copied', 'Page URL copied to clipboard.') }
    catch { toast.error('Copy failed', 'Could not copy the URL.') }
  }
  const shareNative = async () => {
    try {
      if (navigator.share) { await navigator.share({ title: SITE_CONFIG.siteName, url: currentUrl }); return }
      copyUrl()
  ...
  return (
    <AnimatePresence>
      <motion.div variants={megaVariants} initial="hidden" animate="visible" exit="exit"
        ...
        <div style={{maxWidth:'var(--layout-max-w,1120px)',margin:'0 auto',paddingInline:'var(--layout-px,clamp(1rem,4vw,2rem))',paddingTop:'8px'}}>
          {/* v2.2.5: mega-glass always on */}
          <div className="mega-glass rounded-2xl overflow-hidden">
            ...
            <div className="mega-footer px-3 py-2 border-t border-[var(--border-color)] flex items-center gap-2">
              <button type="button" onClick={shareNative} className="mega-share-action" aria-label="Share this page">
...
// ── Nav Right Icons ───────────────────────────────────────────
function NavRight({ user, profile, isAdmin: isAdminProp, avatar, displayName, isLoggedIn, authLoading,
  unreadCount, notifOpen, setNotifOpen, userOpen, setUserOpen, megaOpen, setMegaOpen, onMenuOpen, onMobileSearch, onDesktopSearch }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-shrink-0" data-nav-right>
      {/* v2.2.5: Desktop search → toast "coming soon" (no popup) */}
      <IconBtn icon={faSearch} onClick={onDesktopSearch} label="Search  Ctrl+K" className="hidden lg:flex" tooltipSide="right" />
      {/* Tablet/mobile search → opens sidebar with auto-focus */}
...
export function Navbar() {
  ...
  // Only home page gets transparent top bar
  ...
  // v2.2.5: Ctrl+K → toast (no popup)
  useEffect(() => {
    const h = (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
        e.preventDefault()
        toast.info('Search coming soon', 'Full site search is on the way! 🔍')
      }
    }
  ...
  // v2.2.5: Desktop search → toast
  const handleDesktopSearch = () => {
    toast.info('Search coming soon', 'Full site search is on the way! 🔍')
  }

  // Mobile search: open sidebar + auto-focus input
  ...
  // v2.2.5: pill hover style for top nav links
  const linkCls = (path) => (active) => {
    const isAct = path === '/' ? location.pathname === '/' : active
    return `nav-pill-link relative overflow-hidden flex items-center gap-1.5 h-9 px-3.5 rounded-full text-sm font-medium leading-none transition-all ${isAct ? 'nav-pill-link-active' : ''}`
  }

  const isSearching = searchQuery.trim().length > 0
  const sidebarResults = getSidebarResults(searchQuery)

  return (
    <>
      {/* ╔══ TOP NAVBAR ═══════════════════════════════════════╗ */}
      <nav
        className={`relative z-10 w-full ${isHomePage
          ? 'border-b border-transparent bg-transparent'
          : 'border-b border-[var(--navbar-border)] bg-[var(--navbar-bg)] backdrop-blur-md'}`}
        style={{ height: 'var(--navbar-h)' }}>
        {/* v2.2.5: navbar-inner uses --layout-max-w and --layout-px */}
        <div className="navbar-inner flex items-center h-full mx-auto gap-5"
          style={{maxWidth:'var(--layout-max-w,1120px)',paddingInline:'var(--layout-px,clamp(1rem,4vw,2rem))'}}>
          <NavLogo />
          ...
          <div className="hidden md:flex lg:hidden items-center gap-1.5 ml-auto">
            ...
            <button onClick={() => setMobileOpen(true)}
              className="w-8 h-8 flex items-center justify-center rounded-full border border-[var(--border-color)] bg-[var(--bg-surface-2)] text-[var(--text-secondary)]" aria-label="Menu">
              <FontAwesomeIcon icon={faBars} />
            </button>
          </div>
          {/* Right — mobile */}
      ...
      <AnimatePresence>
        {floatVisible && (
          <motion.div
            ...
            <nav className="float-nav relative pointer-events-auto flex items-center gap-4 w-full h-[52px] px-4 rounded-full"
              style={{maxWidth:'var(--layout-max-w,1120px)'}}>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside key="sidebar" variants={sidebarVariants} initial="closed" animate="open" exit="closed"
              ...
              {/* Search field — v2.2.5: focus ring glow */}
                      ...
                      <div className="space-y-0.5">
                        {sidebarResults.map(link => (
                          <NavLink key={link.path} to={link.path} onClick={() => setMobileOpen(false)}
                            className={({ isActive }) => `search-result-item ${isActive ? 'search-result-item-active' : ''}`}>
                            <span className="sri-icon"><FontAwesomeIcon icon={link.icon} /></span>
                            <span className="sri-label">{link.label}</span>
                            <span className="sri-badge">{link.group}</span>
                            <FontAwesomeIcon icon={faChevronRight} className="sri-arrow" />
                          </NavLink>
                        ))}
                      </div>
                    ) : (
      ...
      {/* ── All Styles ─────────────────────────────────────────── */}
      <style>{`
        /* v2.2.5: Top nav pill hover (distinct accent tint) */
        .nav-pill-link {
        ...
        .nav-pill-link:hover {
          ...
          border-color: rgba(var(--accent-primary-rgb, 59,130,246), 0.18);
          color: var(--text-primary);
        }
        .nav-pill-link-active {
          background: var(--accent-light) !important;
          border-color: rgba(var(--accent-primary-rgb, 59,130,246), 0.25) !important;
          color: var(--accent-primary) !important;
        }
        [data-theme="light"] .nav-pill-link:hover {
          background: rgba(var(--accent-primary-rgb, 37,99,235), 0.07);
          border-color: rgba(var(--accent-primary-rgb, 37,99,235), 0.16);
          color: var(--text-primary);
        }

        /* v2.2.5: Float nav link pill */
        .float-nav-link {
        ...
        /* v2.2.5: Float-nav right buttons glass */
        .float-nav-right button:not([class*="bg-[var(--accent"]):not([class*="bg-red"]),
        ...
        .float-nav-right button:not([class*="bg-[var(--accent"]):not([class*="bg-red"]):hover,
        .float-nav-right a:not([class*="bg-[var(--accent"]):not([class*="bg-red"]):hover {
          background: rgba(255,255,255,0.14) !important;
          border-color: rgba(255,255,255,0.2) !important;
          color: #f1f5f9 !important;
        }
        [data-theme="light"] .float-nav-right button:not([class*="bg-[var(--accent"]):not([class*="bg-red"]),
        ...
        [data-theme="light"] .float-nav-right button:not([class*="bg-[var(--accent"]):not([class*="bg-red"]):hover,
        ...
        /* v2.2.5: Mega menu — glass always on */
        .mega-glass {
          background: rgba(8,15,38,0.86);
          border: 1px solid rgba(148,163,184,0.16);
          box-shadow: var(--shadow-xl);
          backdrop-filter: blur(28px) saturate(180%);
          -webkit-backdrop-filter: blur(28px) saturate(180%);
        }
        [data-theme="light"] .mega-glass {
          background: rgba(240,245,251,0.94);
          border-color: rgba(203,213,225,0.85);
          backdrop-filter: blur(22px) saturate(160%);
          -webkit-backdrop-filter: blur(22px) saturate(160%);
        }
        .is-mega-active svg, .is-mega-active span {
          color: var(--accent-primary) !important;
        }

        /* v2.2.5: Tooltip — glass style, always visible on hover */
        [data-tooltip] { position: relative; }
        [data-tooltip]::after, [data-tooltip]::before {
          position: absolute; pointer-events: none;
          opacity: 0; transition: opacity .16s ease, transform .16s ease; z-index: 1020;
        }
        [data-tooltip]::after {
        ...
        [data-tooltip-side="right"]::after {
          left: auto; right: 0; transform: translateY(5px) scale(0.93);
        }
        [data-nav-right] [data-tooltip]::before,
        [data-tooltip-side="right"]::before {
          left: auto; right: 12px; transform: translateY(5px) rotate(45deg);
        }
        [data-nav-right] [data-tooltip]:hover::after,
        [data-tooltip-side="right"]:hover::after { transform: translateY(0) scale(1); }
        [data-nav-right] [data-tooltip]:hover::before,
        [data-tooltip-side="right"]:hover::before { transform: translateY(0) rotate(45deg); }

        /* Sidebar search focus glow */
        .sidebar-search-field:focus-within {
          border-color: var(--accent-primary) !important;
          box-shadow: 0 0 0 3px rgba(59,130,246,.2), 0 0 14px rgba(59,130,246,.07) !important;
          background: var(--bg-surface) !important;
        }

        /* Sidebar search result items */
        .search-result-item {
          display: flex; align-items: center; gap: 10px;
          padding: 9px 10px; border-radius: 10px;
          border: 1px solid transparent; text-decoration: none;
          transition: all .16s ease; color: var(--text-secondary);
        }
        ...
        .sri-label { flex: 1; min-width: 0; font-size: 13px; font-weight: 600; color: inherit; }
        .sri-badge {
          font-size: 10px; color: var(--text-tertiary);
          border: 1px solid var(--border-color); border-radius: 999px;
          padding: 2px 7px; line-height: 1; flex-shrink: 0;
        }
        .sri-arrow { font-size: 9px; color: var(--text-tertiary); opacity: 0; flex-shrink: 0; transition: opacity .15s ease; }
        .search-result-item:hover .sri-arrow { opacity: 1; }
        ...
        .mega-share-action {
          ...
          color: #fff; background: linear-gradient(135deg,var(--accent-primary) 0%,#6366f1 100%);
          font-size: 11px; font-weight: 600; transition: all .22s ease; flex-shrink: 0;
        ...
        .mega-url-field {
          height: 28px; min-width: 0; flex: 1;
          display: flex; align-items: center; gap: 4px;
          border: 1px solid var(--border-color); border-radius: 999px;
          background: var(--bg-surface-2); padding: 0 3px 0 11px; overflow: hidden;
        }
        .mega-url-field span {
        ...
        .mega-share-action:active { transform: scale(.97); }
