...
// NAVBAR — v2.2.5
// Changes:
//   - Desktop search popup + Ctrl+K + SEARCH_ITEMS fully removed
//   - Desktop search icon → toast.info("Search coming soon")
//   - Mobile/tablet search → opens sidebar + focuses search input
//   - Sidebar search: container border+glow on focus, × clear btn while typing
//   - Top navbar transparent bg only on '/' and '/home'
//   - Floating navbar always keeps glass bg
//   - Navbar inner, floating inner: max-width 1120px, clamp(1rem,4vw,1.75rem) padding
//   - Nav link hover: glass pill (accent tint) on top + floating nav
//   - Mega menu panel: always glass (backdrop-filter) — not conditional on float
//   - Mega menu item hover: accent tint border
//   - 4 right icon buttons: CSS ::after tooltips + minimal glass bg
// ============================================================
...
function ThemeToggle({ size = 'md', className = '' }) {
  ...
  return (
    <button onClick={handleClick}
      className={`nav-icon-btn ${sz} relative overflow-hidden flex items-center justify-center rounded-lg transition-colors ${className}`}
...
function IconBtn({ icon, onClick, label, badge, active, className = '' }) {
  ...
  return (
    <button onClick={handleClick}
      className={`nav-icon-btn relative overflow-hidden w-9 h-9 flex items-center justify-center rounded-lg transition-colors ${active ? 'nav-icon-btn-active' : ''} ${className}`}
      aria-label={label}
      data-tooltip={label}
      data-ripple-managed="true">
      <RippleLayer ripples={ripples} color="rgba(59,130,246,0.2)" />
...
// ── Mega Menu ─────────────────────────────────────────────────
// Glass effect is always applied — not conditional on floating state
  ...
  return (
    <AnimatePresence>
      <motion.div variants={megaVariants} initial="hidden" animate="visible" exit="exit"
        ...
        <div className="mega-panel-inner max-w-[1120px] mx-auto px-[clamp(1rem,4vw,1.75rem)] pt-2"
          style={{ paddingInline: 'clamp(1rem, 4vw, 1.75rem)' }}>
          <div className="mega-glass rounded-2xl overflow-hidden">
                    ...
                    {col.items.map(item => (
                      item.external
                        ? <a key={item.path} href={item.path} target="_blank" rel="noopener noreferrer"
                            onClick={onClose} className={itemCls(item.path)}>
                        ...
                        : <Link key={item.path} to={item.path} onClick={onClose} className={itemCls(item.path)}>
                            <FontAwesomeIcon icon={item.icon} className="w-3.5 text-center text-[var(--text-tertiary)] group-hover:text-[var(--accent-primary)] transition-colors flex-shrink-0 text-xs" />
                            <span className="text-[13px] font-medium text-[var(--text-secondary)] group-hover:text-[var(--accent-primary)] transition-colors leading-tight">{item.label}</span>
                          </Link>
                    ))}
            ...
            <div className="mega-footer px-3 py-2 border-t border-[var(--border-color)] flex items-center gap-2">
              <button type="button" onClick={shareNative} className="mega-share-action" aria-label="Share" data-tooltip="Share this page">
...
export function Navbar() {
  ...
  // Desktop search icon → toast
  const handleDesktopSearch = () => {
    toast.info('Search coming soon! 🔍', 'Full search will be available in a future version.')
    setMegaOpen(false); setNotifOpen(false); setUserOpen(false)
  }

  // Mobile/tablet search → open sidebar + focus input
  ...
  // Nav link class — top navbar glass pill hover
  const linkCls = (path) => (active) => {
    const isAct = path === '/' ? location.pathname === '/' : active
    return `nav-link relative overflow-hidden flex items-center gap-1.5 h-9 px-3.5 rounded-lg text-sm font-medium leading-none transition-all ${isAct ? 'nav-link-active' : ''}`
  }

  // ── Right icons section ──────────────────────────────────────
  const RightIcons = ({ compact = false, inFloat = false }) => {
    const sz = compact ? 'w-8 h-8 text-sm' : 'w-9 h-9 text-base'
    const cl = inFloat ? 'float-nav-right' : ''
    return (
      <div className={`flex items-center gap-1.5 flex-shrink-0 ${cl}`}>
        {/* Search — desktop shows toast, mobile opens sidebar */}
        <IconBtn icon={faSearch} onClick={handleDesktopSearch} label="Search"
          className={`hidden lg:flex ${sz}`} />
        <IconBtn icon={faSearch} onClick={handleMobileSearch} label="Search"
          className={`lg:hidden ${sz}`} />

        {/* Notifications */}
        <div className="notif-anchor relative">
          <IconBtn icon={faBell}
            onClick={() => { setNotifOpen(!notifOpen); setUserOpen(false); setMegaOpen(false) }}
            label="Notifications" badge={unreadCount} active={notifOpen} className={sz} />
        ...
        {/* Auth */}
        {authLoading ? <div className={`${sz} rounded-full sk`} /> : isLoggedIn ? (
          <div className="user-anchor relative">
            <button onClick={() => { setUserOpen(!userOpen); setNotifOpen(false); setMegaOpen(false) }}
              className={`${compact ? 'w-8 h-8' : 'w-9 h-9'} rounded-full overflow-hidden border-2 transition-colors ${userOpen ? 'border-[var(--accent-primary)]' : 'border-[var(--border-color)] hover:border-[var(--border-strong)]'}`}>
              {avatar ? <img src={avatar} alt={displayName} className="w-full h-full object-cover" />
        ...
        ) : <SignInBtn className={compact ? 'h-8 text-xs px-3 py-0' : ''} />}

        {/* More / Mega */}
        <div className="mega-anchor">
          <IconBtn icon={faTableCells}
            onClick={() => { setMegaOpen(!megaOpen); setNotifOpen(false); setUserOpen(false) }}
            label="More" active={megaOpen} className={sz} />
        </div>

        {/* Hamburger (mobile/tablet) */}
        <button onClick={() => setMobileOpen(true)}
          className={`lg:hidden nav-icon-btn ${compact ? 'w-8 h-8 text-sm' : 'w-9 h-9 text-base'} flex items-center justify-center rounded-lg transition-colors`}
          aria-label="Menu" data-tooltip="Menu">
    ...
    <>
      ...
      <nav
        ...
        style={{
          ...
          borderBottom: isHomePage ? '1px solid transparent' : '1px solid var(--navbar-border)',
                ...
                {NAV_LINKS.map(link => (
                  <NavLink key={link.path} to={link.path} end={link.path === '/'}
                    className={({ isActive }) => {
                      ...
                      return `float-nav-link relative overflow-hidden flex items-center gap-1.5 h-9 px-3.5 rounded-lg text-sm font-medium leading-none transition-all ${isAct ? 'float-nav-link-active' : ''}`
                    }}
                    title={link.title}>
                    <FontAwesomeIcon icon={link.icon} className="text-xs opacity-80" />
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside key="sidebar" variants={sidebarVariants} initial="closed" animate="open" exit="closed"
              ...
              {/* Search bar — beautiful design, no function yet */}
              <div className="px-4 mt-3 flex-shrink-0">
                <div className={`sidebar-search-wrap flex items-center gap-2.5 px-4 py-2.5 rounded-xl border transition-all ${isSearching ? 'sidebar-search-typing' : ''}`}>
                  ...
                  {isSearching && (
                    <button
                      onClick={() => { setSearchQuery(''); sidebarSearchRef.current?.focus() }}
                      className="w-5 h-5 rounded-full flex items-center justify-center text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-2)] transition-colors flex-shrink-0">
                      <FontAwesomeIcon icon={faXmark} className="text-[10px]" />
                    </button>
                  )}
                </div>
                {isSearching && (
                  <p className="text-xs text-[var(--text-tertiary)] mt-2 px-1 text-center">
      ...
      {/* ── Navbar Styles ─────────────────────────────────────── */}
      <style>{`
        /* ── Shared inner container — matches body .container-xl exactly ── */
        .navbar-inner {
          max-width: 1120px;
          margin-inline: auto;
        ...
        /* ── Right icon buttons: minimal glass bg + tooltips ── */
        .nav-icon-btn {
          color: var(--text-secondary);
        ...
        /* ── CSS ::after tooltips on all nav icon buttons ──── */
        ...
        [data-theme="light"] .nav-icon-btn::after {
        ...
        [data-theme="light"] .float-nav {
          background: rgba(237, 242, 248, 0.90);
          border: 1px solid rgba(203, 213, 225, 0.82);
          box-shadow: 0 6px 24px rgba(15,23,42,0.10), inset 0 1px 0 rgba(255,255,255,0.9);
          backdrop-filter: blur(20px) saturate(150%);
          -webkit-backdrop-filter: blur(20px) saturate(150%);
        }

        /* ── Float nav links: glass pill hover ───────────────── */
        .float-nav-link {
        ...
        /* ── Float nav right buttons — override to glass ─────── */
        ...
        /* ── Mega menu: always glass ─────────────────────────── */
        .mega-glass {
          background: rgba(8, 15, 38, 0.84);
        ...
        [data-theme="light"] .mega-glass {
          ...
          border: 1px solid rgba(190, 204, 224, 0.88);
        ...
        .mega-item-active span, .mega-item-active svg {
          color: var(--accent-primary) !important;
        }
        ...
        /* ── Sidebar search: full container border+glow on focus ── */
        .sidebar-search-wrap {
        ...
        .sidebar-search-wrap:focus-within {
          border-color: var(--accent-primary) !important;
          box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.18), 0 0 12px rgba(59, 130, 246, 0.08);
          background: var(--bg-surface);
        }
        .sidebar-search-wrap input { caret-color: var(--accent-primary); }
        ...
        /* ── Legacy tooltip (non-icon-btn elements) ──────────── */
        [data-tooltip]:not(.nav-icon-btn) { position: relative; }
        [data-tooltip]:not(.nav-icon-btn)::after {
          content: attr(data-tooltip);
          position: absolute;
          bottom: -38px;
          left: 50%;
        ...
        [data-tooltip]:not(.nav-icon-btn):hover::after {
          opacity: 1;
        ...
        .mega-share-action:hover:active { transform: scale(0.97) translateY(0); }
