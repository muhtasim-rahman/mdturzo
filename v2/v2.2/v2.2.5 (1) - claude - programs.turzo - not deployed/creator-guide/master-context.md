...
## v2.2.5 — Layout Unification, Navbar Polish, Cookie Fix, Search Refactor, Light Mode

**Summary:**
v2.2.5 is a polish and infrastructure release. The primary goal was eliminating horizontal alignment drift across Navbar, body sections, and Footer by unifying them on a single layout token (`--layout-max: 1120px` + `clamp(1rem,4vw,1.75rem)` padding). The navbar was overhauled: desktop search popup fully removed (replaced with toast), sidebar search redesigned with container-focus glow and a clear button. Cookie banner moved to Layout.jsx and made fully persistent via `localStorage`. Top navbar is now transparent only on home page; the floating navbar always keeps glass. Nav link hover redesigned as glass pill with accent tint. Mega menu always has backdrop-filter glass. Right icon buttons have CSS `::after` tooltips. Light mode is warmer. Section snap auto-scroll removed entirely per user request — users scroll freely. `SectionSnap` component deleted from `App.jsx`.

**Changed files:**
- `index.html` — added Material Symbols variable font link (waving_hand)
- `src/App.jsx` — `SectionSnap` component deleted; scroll is fully free
- `src/index.css` — `--layout-max: 1120px`, `--layout-pad: clamp(1rem,4vw,1.75rem)` tokens; `.container-xl` uses these; light mode bg-page `#F0F4F8`, surface `#F7F9FC`, borders slightly darker; `scroll-behavior: auto` (not smooth); `.material-symbols-outlined` CSS; section `scroll-margin-top: 80px` only (no snap)
- `src/config/site.config.js` — version `v2.2.5`
- `package.json` — version `2.2.5`
- `src/components/layout/Layout.jsx` — imports and renders `<CookieBanner>` globally (self-filters to home via `useLocation`); imports `useSiteSettings` for `enabled` prop
- `src/components/ui/CookieBanner.jsx` — localStorage key `mdturzo_cookie_consent`; `checkedRef` prevents async flicker; home-only via `HOME_PATHS = ['/', '/home']`; fully functional accept/decline persistence
- `src/components/layout/Navbar.jsx` — full rewrite: desktop search popup + `SEARCH_ITEMS` + `getSearchResults` + `Ctrl+K` + `DesktopSearchPopup` component all deleted; desktop search icon → `toast.info`; mobile search → sidebar + `sidebarSearchRef.focus()`; sidebar search: `sidebar-search-wrap` container gets border+glow on `:focus-within`, × clear btn when typing; top navbar transparent bg only when `HOME_PATHS.includes(pathname)`; floating nav always keeps glass; nav-link hover: glass pill `.nav-link` class with accent rgba; mega menu `.mega-glass` always applied (not conditional on float); mega item hover: `.mega-item` with accent border; 4 icon buttons use `.nav-icon-btn` class with glass bg + CSS `::after` tooltip; `RightIcons` extracted as inner component to avoid duplication
- `src/components/layout/Footer.jsx` — `nf-inner` max-width 1120px (was 1280px); `nf-footer` padding-inline uses clamp; `nf-sc-wrap` padding-inline uses clamp; `nf-sc` max-width 1120px + margin:auto for center; Let's Collaborate `<Link>` has `data-click-fx="true"`; 1440px breakpoint sets both padding-inline to 0
- `src/components/home/AboutMini.jsx` — Read Full Story button: no `hover:scale`, only color/arrow effects; `justify-items-center lg:justify-items-stretch` on grid for proper centering; light mode image overlay ~15% opacity via `.about-img-overlay` + CSS class
- `src/components/home/Hero.jsx` — `WaveIcon` SVG component replaced with `<span className="material-symbols-outlined hgreet-icon-ms">waving_hand</span>`
- `src/pages/Home.jsx` — `CookieBanner` import and usage removed (now in Layout)
- `src/pages/About.jsx` — content cleared; "Coming in v2.3.0" placeholder card
- `src/pages/Contact.jsx` — content cleared; "Coming in v2.6.0" placeholder card

**Architecture decisions:**
- **Cookie banner in Layout:** Mounted globally so it works on any route if ever needed; self-filters via `useLocation()` inside the component — cleaner than passing route props from Home. `checkedRef` guards against double-show when `enabled` prop arrives asynchronously from Supabase.
- **`isHomePage` transparent navbar:** `HOME_PATHS.includes(location.pathname)` array check — easy to extend. Floating nav ignores this and always keeps glass (users have scrolled, context is clear).
- **Desktop search removal:** Fully deleted — no dead code, no conditional. Desktop click → `toast.info`. Mobile → sidebar + focus. Sidebar search has UI only (no results) — do NOT re-add desktop popup; search engine is a future milestone.
- **Layout token:** `--layout-max: 1120px` and `--layout-pad: clamp(1rem,4vw,1.75rem)` in `:root` CSS. Navbar inner uses `max-width: 1120px; margin-inline: auto; padding-inline: clamp(...)`. Footer uses same. `.container-xl` uses same. All three are pixel-aligned at every viewport.
- **Section snap removed:** `SectionSnap` component deleted from App.jsx entirely. `scroll-behavior: auto` in CSS. Sections only have `scroll-margin-top: 80px` for anchor link offset. No JS scroll intervention.
- **Mega glass always on:** `.mega-glass` CSS class always on the panel div — not toggled by float state. Previously `.mega-floating` was conditionally added; now glass is the default for both.

**Deferred to v2.2.6:**
- Hero section full rewrite (ResizeObserver height, round frame at ≤900px only, floating icons outside circular frame, all content center on mobile)
- Skills & Experience section redesign (stat card polish, sidebar alignment, shimmer progress bars)
- GitHub Activity fully dynamic (GitHub public API, streak stats, repo cards, language breakdown)
- RecentProjects/Services card hover color = card.color
- CTA section redesign (centered, dot-grid texture, hero-sit.webp removed)

**Important notes for v2.2.6 AI:**
- localStorage key for cookies is `mdturzo_cookie_consent` — do not rename
- Search sidebar has no function — do not re-add desktop search popup
- Navbar transparent home uses `HOME_PATHS = ['/', '/home']` array in Navbar.jsx
- Footer `nf-inner` is now 1120px max — match this in any new footer sections
- `scroll-behavior: auto` is intentional — do not revert to smooth
- GitHub API calls are unauthenticated — 60 req/hr rate limit; batch wisely
- `hero-sit.webp` is still in `public/` until v2.2.6 CTA removes it
