...
## v2.2.4 — Hero Overhaul · Navbar Fixes · GitHub Live · CTA Redesign · Polish Sprint

**Summary:**
Major polish and fix release. Hero section fully rewritten with correct image sizing logic, Google Material Symbols waving_hand icon, round frame only on tablet/mobile, floating icons always outside frame. Navbar alignment fixed to match body/footer max-width (1280px). Search popup removed — toast shown instead on desktop click. Mega menu always has glass effect (top + floating). Cookie banner repeated-show bug fixed. CTA redesigned as compact centered textured section (hero-sit.webp removed). About/Contact pages cleared (content deferred to v2.3.0/v2.6.0). GithubStats rewritten with live GitHub API data. Skills section background and card designs improved. Light mode colors deepened to reduce washed-out feel. Stay Connected banner centered. Section alternation fixed across all home sections.

**Changed files:**
- `index.html` — Added Material Symbols Outlined font link (waving_hand icon)
- `src/components/home/Hero.jsx` — Full rewrite v2.2.4: Material Symbols waving_hand; image height tracks left-content height via ResizeObserver (+10%); parent div has no width restriction; round frame ONLY on tablet (≤1024px); floating icons always outside round-frame border on all devices; left content vertically centered; tablet/mobile all centered
- `src/components/home/Skills.jsx` — section-alt bg; StatPill redesigned with accent border-bottom gradient; SkillBar improved with shimmer gradient; uses section-alt
- `src/components/home/AboutMini.jsx` — "More" button: removed hover scale; image frame: light-mode gradient fixed (light gray-blue instead of dark); section changed to plain
- `src/components/home/RecentProjects.jsx` — section-alt; card hover border = card category color; card title NOT colored on full-card hover (only on title hover)
- `src/components/home/Services.jsx` — plain section (alternating with RecentProjects); card hover border = service color
- `src/components/home/GithubStats.jsx` — full rewrite: all data via GitHub public API (repos, stars, followers, following); dynamic repo cards with language colors, star/fork counts, last-updated tooltips; Contribution Streak via streak-stats.demolab.com (live); no more static data or written text
- `src/components/home/CTA.jsx` — full redesign: compact centered layout, no image, dot-grid texture bg, badge with pulse, service pills grid, no hero-sit.webp dependency
- `src/components/ui/CookieBanner.jsx` — Fixed repeated-show bug: uses a ref to track if already shown/checked; only shows once per browser session if not answered
- `src/components/layout/Navbar.jsx` — Removed DesktopSearchPopup and all popup-related code/state; desktop search click → toast('Search coming soon'); mega menu always mega-glass effect; all navbar/floating/mega max-width synced to 1280px; Ctrl+K → toast
- `src/components/layout/Footer.jsx` — Stay Connected: added mx-auto + border-radius top to center banner correctly; Let's Collaborate card: data-click-fx added for proper click effect
- `src/index.css` — Light mode: bg-page #F0F4F8, text-primary deeper, borders stronger, navbar bg more visible — reduces washed-out look
- `src/pages/About.jsx` — Content cleared; placeholder "coming in v2.3.0" shown
- `src/pages/Contact.jsx` — Content cleared; placeholder "coming in v2.6.0" shown
- `src/config/site.config.js` — version v2.2.4
- `package.json` — version 2.2.4
- `public/hero-sit.webp` — REMOVED (no longer used in any component)

**Architecture notes & decisions:**
- Hero image height: `ResizeObserver` on `.hcontent` div → image height = contentHeight × 1.10. Falls back to `max-height:88vh` while content hasn't mounted yet.
- Round frame: CSS classes `hscene-pc` / `hscene-round` toggled by `@media(max-width:1024px)`. Round frame icons: 5 absolute-positioned divs relative to `.hround-wrap`, placed outside the frame boundary (negative or slightly overlapping border).
- GithubStats: uses `https://api.github.com/users/muhtasim-rahman` and `/repos` public endpoints. No auth required. Falls back to error state gracefully.
- Cookie banner: `checkedRef` prevents `useEffect` from re-running even when `enabled` prop changes after initial settings load.
- Mega menu glass: `.mega-glass` class always applied (was `.mega-floating` only when floating). Both top and floating navbar mega menus now get backdrop-filter blur.
- Section bg order (v2.2.4): Hero(none) → Skills(alt) → AboutMini(plain) → RecentProjects(alt) → Services(plain) → Testimonials(plain) → GithubStats(alt) → BlogMini(plain) → CTA(own texture) → Footer
- About page (v2.3.0) and Contact page (v2.6.0): cleared to placeholder. The old v2.2.0 content had issues that will be rebuilt from scratch in respective versions.
- Light mode: colors shifted to have more contrast (bg-page #F0F4F8 vs #F8FAFC, text more navy, borders stronger). This reduces the overly bright/washed look without breaking dark mode.

**Deferred:**
- About page full build → v2.3.0
- Contact page full build → v2.6.0
- GitHub stats: language breakdown panel removed (was static data); repo language colors from GitHub API `language` field are used in repo cards
- Search engine + sidebar search function → future version; desktop shows toast, sidebar has working UI design only
