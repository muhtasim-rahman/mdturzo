...
function ProjectCard({p,i}){
  ...
  return(
    <motion.div className="card group flex flex-col overflow-hidden transition-all duration-300"
      ...
      <div className="p-4 flex flex-col gap-2.5 flex-1">
        ...
        <h3 className="font-display font-bold text-[var(--text-primary)] leading-snug line-clamp-2 text-sm">{p.title}</h3>
        ...
        <div className="pt-2 border-t border-[var(--border-color)]">
          <Link to={`/projects/${p.slug}`} className="flex items-center justify-between text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--accent-primary)] transition-colors duration-200 group/link">
            <span className="hover:text-[var(--accent-primary)] transition-colors duration-200" style={{'--card-color':color}} onMouseEnter={e=>e.currentTarget.style.color=color} onMouseLeave={e=>e.currentTarget.style.color=''}>{p.title.length > 0 ? 'View details' : 'View'}</span>
...
export default function RecentProjects(){
  ...
  useEffect(()=>{
    getFeaturedProjects()
      .then(d=>{ setProjects(d?.length ? d : FALLBACK); setFallback(!d?.length) })
      .catch(()=>{ setProjects(FALLBACK); setFallback(true) })
    ...
    <section className="section section-alt" id="projects-mini">
      <div className="container-xl">
        ...
        <div className="proj-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {loading
            ? Array.from({length:6},(_,i)=><SkeletonCard key={i} className={i>=3?'proj-hide-mob':''}/>)
            : projects.map((p,i)=><ProjectCard key={p.id} p={p} i={i} idx={i}/>)
          }
        </div>
        <style>{`
