...
// FIX: Card hover border = card's category color (not accent-primary)
// FIX: Only the project title is a clickable Link (not "View details" CTA)
  ...
  return (
    ...
    >
      ...
      <div className="relative h-40 bg-[var(--bg-surface-2)] overflow-hidden flex-shrink-0">
        ...
        <div className="absolute top-3 right-3 flex gap-1.5 opacity-0 hover:opacity-100 transition-opacity duration-200"
      ...
      <div className="p-4 flex flex-col gap-2.5 flex-1">
        ...
        {/* Title — ONLY the title is a clickable Link */}
        <h3 className="font-display font-bold leading-snug line-clamp-2 text-sm">
          <Link
            to={`/projects/${p.slug}`}
            className="text-[var(--text-primary)] hover:text-[var(--accent-primary)] transition-colors duration-200"
        ...
        {/* Bottom accent line in card color */}
        <div className="pt-2 mt-auto border-t border-[var(--border-color)]">
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: color }} />
            <span className="text-[11px] text-[var(--text-tertiary)] font-medium">{p.category}</span>
          </div>
        </div>
      </div>
    </motion.div>
