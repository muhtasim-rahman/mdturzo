...
export async function getFeaturedProjects() {
  ...
  // Prompt schema uses `featured`; earlier local code used `is_featured`.
  // Try the prompt schema first, then fall back for older databases.
  const { data, error } = await baseQuery().eq('featured', true)
  if (!error) return data

  const fallback = await baseQuery().eq('is_featured', true)
