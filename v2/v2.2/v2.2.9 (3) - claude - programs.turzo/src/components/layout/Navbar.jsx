...
// NAVBAR -- v2.2.9
// FIXES:
//   * Removed HTML `title` attr from all icon buttons (was showing
//     both native browser tooltip AND CSS tooltip simultaneously)
//   * CSS tooltip: now centers perfectly below button (left:50% translateX(-50%))
//   * Mega nav: hides when page scrolls past the navbar area
//   * Mega nav: clicking empty space (left/right of panel) closes it
// ============================================================
...
export function Navbar() {
  ...
  useEffect(() => {
    // Close mega nav on scroll (hides behind navbar when scrolling)
    const onScroll = () => {
      setFloatVisible(window.scrollY > FLOAT_THRESHOLD)
      if (window.scrollY > FLOAT_THRESHOLD + 60) setMegaOpen(false)
    }
  ...
  return (
    <>
      ...
      {/* == MEGA MENU -- single render, outside both navbars ===
          Clicking the overlay (empty left/right space) closes the menu.
      */}
      <AnimatePresence>
        {megaOpen && (
          <div
            ...
            onClick={(e) => {
              // Close if clicking the backdrop (not the panel itself)
              if (!e.target.closest('.mega-panel-wrap') && !e.target.closest('.mega-glass-inner')) {
                setMegaOpen(false)
              }
            }}>
      ...
      <style>{`
        ...
        /* == TOOLTIP -- data-nb-tip (centered below button) */
        ...
        /* Right-side cluster: tooltip still centers on button, not offset */
