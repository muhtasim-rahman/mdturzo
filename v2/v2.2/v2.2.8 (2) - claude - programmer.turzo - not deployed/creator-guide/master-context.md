...
**Summary:**
Final polish release. Fixes all remaining visual/UX bugs reported by user after v2.2.7 review. Hero image frame decoration removed on desktop, hero height slightly increased, "Available for hire" badge removed. Name layout fixed for tablet/mobile. Navbar double-search-icon, double-menu-icon, double mega-menu, and outside-click bugs all fixed. Skills stat cards compacted. "How I Work" replaced with clean "My Setup & Workflow" section (code editor mockup + tool grid). GitHub stats section fully redesigned: profile card → streak + stats row → trophies → languages → repos.

**Changed files:**

- `src/components/ui/Ripple.jsx` — RippleLayer now wraps ripples in its own `overflow:hidden` span so parent button can use `overflow:visible` for CSS tooltip ::after pseudo-elements
- `src/components/home/Hero.jsx` — CHANGES:
  - `.himg-frame` on desktop: `background:transparent`, `border:none`, `box-shadow:none`, `::before`/`::after` set to `display:none` — no frame decoration, image floats
  - `.himg-box` height increased by ~35px across all breakpoints
  - "Available for hire" badge removed entirely
  - Tablet (640-899px): `.hname` single line, `white-space:nowrap`, `.hname-line` inline, `(Turzo)` `.hname-nick` inline
  - Mobile (<640px): `.hname-nick` `display:block`, smaller font, own line below
  - Round frame (≤899px): `background:color-mix(in srgb, var(--accent-primary) 14%, var(--bg-surface-2))` + matching border — alt accent-tinted bg added
- `src/components/layout/Navbar.jsx` — FIXES:
  - `NavRight` component: search icon and `lg:hidden` menu button REMOVED — NavRight is desktop-only cluster, mobile/tablet have their own icon buttons inline
  - Mega menu: moved OUTSIDE both `<nav>` elements into a single standalone `<AnimatePresence>` block at root level — eliminates double-render when both top + float navbars are in DOM
  - Outside-click: mega panel handler checks `'.mega-anchor'` AND `'.mega-panel-wrap'` — click inside mega no longer closes it; outside click reliably closes
  - `nb-icon-btn` CSS: `overflow: visible` (was `overflow: hidden`) so `[data-nb-tip]::after` tooltip pseudo-elements are not clipped
  - `[data-nb-tip]::after` tooltip: now actually visible due to above fix; `position:absolute; top:calc(100%+9px)` below button; `z-index:10001`
  - `[data-nb-right]` modifier: right-aligned tooltips for right-side icon cluster
  - `title={label}` added to all `IconBtn` and `ThemeToggle` as native tooltip fallback
- `src/components/home/Skills.jsx` — StatCard redesign: compact layout (`padding .65rem .8rem`, radius `10px`, border-left `2.5px`), `sk2-stat-top` row with icon + number side-by-side, number font `1.28rem` (was `1.65rem`), label `uppercase .68rem tertiary`, no corner glow `::after`, subtle hover only
- `src/components/home/Process.jsx` — COMPLETE REWRITE: "How I Work" 6-step process → "My Setup & Workflow" section. Left: 2×4 tool grid (compact cards with colored left-border + icon) + principles list. Right: VS Code-style editor mockup (fake JSX code, window chrome, line numbers, status bar) + 2-stat row. Minimal color, fully responsive (stacks at 860px).
- `src/components/home/GithubStats.jsx` — FULL REDESIGN:
  - GitHub Profile card: avatar with gradient ring, name/login/bio/location/website/join-year, 4 stat pills, "View Profile" button, accent top-line decoration
  - Streak + Stats row: `gh-img-grid` (2-col, stacks at 640px): LEFT = awesome-github-stats level card (`https://awesome-github-stats.azurewebsites.net/user-stats/muhtasim-rahman?cardType=level&fontFamily=42dot%20Sans`) with theme param; RIGHT = github-readme-stats (existing, with theme-aware text color)
  - Both image cards: theme-aware URL params via `useThemeStore().isDark()`
  - GitHub Trophies: `github-profile-trophy.vercel.app` with `darkhub`/`flat` theme, `row=2&column=6`
  - Top Languages: unchanged from v2.2.7
  - Top Repositories: unchanged from v2.2.7
  - Removed: ContribStats custom card, AchievementBadge section (replaced by trophies image)
- `src/config/site.config.js` — version bumped to `v2.2.8`
- `package.json` — version bumped to `2.2.8`

**Architecture decisions:**

- **Single mega menu render:** Both top and floating navbars were previously rendering their own `<MegaMenu>` inside their own `<AnimatePresence>`. When both were in the DOM (scroll position >420px), clicking the mega icon opened TWO panels stacked. Fix: `megaOpen` state + `setMegaOpen` shared at Navbar root level; single `<AnimatePresence><MegaMenu /></AnimatePresence>` block rendered after both `<nav>` elements. `megaTopOffset` switches between `72px` (float nav bottom) and `var(--navbar-h)` depending on `floatVisible`.
- **Overflow:visible for tooltips:** CSS `::after` pseudo-elements on `.nb-icon-btn` were being clipped by `overflow:hidden`. Changed to `overflow:visible`. RippleLayer now handles its own clipping via an inner `<span style="overflow:hidden">` wrapper.
- **Hero frame removal:** `.himg-frame::before` and `::after` set to `display:none` globally; `background/border/box-shadow` all transparent/none. On ≤899px round frame re-applies its own bg + border directly on `.himg-frame`. This ensures no leakage of desktop styles into mobile round frame.
- **Streak image:** Uses `awesome-github-stats.azurewebsites.net` (provided by user, from brunobritodev/awesome-github-stats). Theme param is `theme=dark` or `theme=default` based on `isDark()` from `useThemeStore`.
- **Process section:** Renamed in spirit but still exported as `Process` and imported as `Process` in `Home.jsx` — no import changes needed.

**Important notes for next AI (v2.2.9+):**

- Navbar mega menu is now a SINGLE render outside both `<nav>` blocks. Do NOT move it back inside either navbar. The `megaTopOffset` variable controls its vertical position.
- `nb-icon-btn` MUST keep `overflow:visible` for tooltips to show. Ripple clipping is handled by `RippleLayer`'s inner `<span>`.
- Hero `.himg-frame`: desktop has NO decoration (no bg, no border, no shadow, no pseudo-elements). Round frame on ≤899px re-applies everything directly. Do not add back frame styles.
- GithubStats: streak uses awesome-github-stats (NOT herokuapp — that is permanently broken).
- `Process.jsx` is the "My Setup & Workflow" section — do not revert to "How I Work" step list.
- Skills stat cards are now compact (no corner glow, smaller numbers, label is uppercase tertiary). Do not make them larger again.
