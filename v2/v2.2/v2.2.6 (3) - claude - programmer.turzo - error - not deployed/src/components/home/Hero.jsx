// Hero.jsx — v2.2.6
// TASK 1 fixes:
//   1a: left content vertically centered (flex col + justify-content:center)
//   1b: ≤900px stacked, both columns centered (text-align:center, margin-inline:auto)
//   1c: round frame ONLY at ≤900px width — REMOVED orientation/height media queries
//   1d: image height = leftContentHeight × 1.10 via ResizeObserver
//   1e: image parent has no width constraint
//   1f: floating icons positioned outside circular frame border on mobile
//   ALL existing animations, particles, orbs, socials, stats preserved 100%
...
const FLOAT_ICONS = [
  { file:'html5',  label:'HTML5',  cls:'html',   dur:'3.9s', del:'0s',    pcTop:'12%',  pcSide:{left:'-28px'}, sz:'48px' },
  ...
  { file:'vscode', label:'VSCode', cls:'vscode', dur:'3.3s', del:'0.95s', pcTop:'8%',   pcSide:{right:'-27px'},sz:'46px' },
...
export default function Hero({settings,settingsLoading}){
  ...
  const leftRef=useRef(null)         // Task 1d: ResizeObserver target
  const [inView,setInView]=useState(false)
  const [imgHeight,setImgHeight]=useState(null)  // Task 1d: dynamic image height
  const [isDesktop,setIsDesktop]=useState(window.innerWidth>900) // Task 1c: width-only
  ...
  // Task 1d: ResizeObserver on left content
  useEffect(()=>{
    if(!leftRef.current) return
    const ro=new ResizeObserver(([entry])=>{
      setImgHeight(entry.contentRect.height * 1.10)
    })
    ro.observe(leftRef.current)
    return()=>ro.disconnect()
  },[])

  // Task 1c: width-only breakpoint (not orientation/height)
  useEffect(()=>{
...
/* Task 1a: inner grid — left content vertically centered */
.hinner{position:relative;z-index:5;display:grid;grid-template-columns:1.18fr 1fr;gap:clamp(1rem,3vw,2.5rem);align-items:center;width:100%;height:100%;max-width:1120px;margin-inline:auto;padding-inline:clamp(1rem,4vw,1.75rem);padding-block:clamp(2rem,4vh,3rem)}

/* Task 1a: left content — flex col, justify:center */
...
/* Task 1e: hvisual — no width constraint, just flex center */
.hvisual{position:relative;z-index:5;display:flex;align-items:flex-start;justify-content:center;height:100%;opacity:0;animation:hvis-in .85s cubic-bezier(.16,1,.3,1) .2s forwards}
@keyframes hvis-in{from{opacity:0;transform:translateX(22px) scale(.94)}to{opacity:1;transform:none}}

/* Task 1e: hscene — position:relative only, NO max-width/width constraint */
.hscene{position:relative;width:100%;height:var(--hscene-h,auto);flex-shrink:0;align-self:flex-start;margin-top:clamp(-30px,calc(-2vh - 10px),-5px)}
...
/* Task 1f: floating icons on PC — current positions relative to hscene edges */
...
/* ── Task 1c: Round frame ONLY at ≤900px WIDTH — no orientation/height media queries ── */
@media(max-width:900px){
  .hero{height:auto;max-height:none;min-height:100dvh}
  /* Task 1b: stacked, both centered */
  ...
  .hvisual{order:1;height:auto;justify-content:center;align-items:center;animation:ifdef-in .85s cubic-bezier(.16,1,.3,1) .2s forwards}
  .hbio{max-width:76%;text-align:center}
  .hcta,.hsocials,.hstats{justify-content:center}
  .hgreet{align-self:center}
  /* Task 1c: circular frame, fixed size */
  .hscene{width:clamp(175px,46vw,240px);height:clamp(175px,46vw,240px) !important;max-width:none;margin-top:0;align-self:auto}
  /* Task 1c: round frame triggered here — width only */
  ...
  /* Task 1f: icons outside circular frame
     Frame is clamp(175,46vw,240)px, so radius = half = ~120px max
     Icons positioned at frame edge + small gap
     Using calc(50% - frame_half - icon_half - gap) style offsets relative to .hscene */
  .hicon{--sz:36px !important;border-radius:10px}
  /* HTML: top-left outside circle */
  .hicon-html{top:-14px !important;left:calc(50% - 46vw / 2 - 18px) !important;right:auto !important}
  /* CSS: top-right outside circle */
  .hicon-css{top:-14px !important;left:auto !important;right:calc(50% - 46vw / 2 - 18px) !important}
  /* Python: mid-left outside */
  .hicon-python{top:50% !important;left:calc(50% - 46vw / 2 - 20px) !important;right:auto !important;transform:translateY(-50%);animation-name:hicon-fy}
  /* VSCode: mid-right outside */
  .hicon-vscode{top:50% !important;left:auto !important;right:calc(50% - 46vw / 2 - 20px) !important;transform:translateY(-50%);animation-name:hicon-fy}
...
@media(max-width:520px){
  ...
  .hscene{width:clamp(150px,44vw,185px);height:clamp(150px,44vw,185px) !important}
  .hicon{--sz:30px !important;padding:6px;border-radius:8px}
  .hicon-html{top:-12px !important;left:calc(50% - 44vw / 2 - 15px) !important}
  .hicon-css{top:-12px !important;right:calc(50% - 44vw / 2 - 15px) !important}
  .hicon-python{left:calc(50% - 44vw / 2 - 18px) !important}
  .hicon-vscode{right:calc(50% - 44vw / 2 - 18px) !important}
}

@media(min-width:1400px){.hscene{height:clamp(500px,min(78vh,680px),700px) !important}}
...
/* NOTE: NO orientation:portrait media query — round frame is width-only (Task 1c) */
`}</style>
      ...
      <section className="hero" id="hero" aria-label="Introduction">
        ...
        <div className="hinner">
          {/* LEFT CONTENT — Task 1d: ref for ResizeObserver */}
          ...
          {/* RIGHT VISUAL — Task 1d: height driven by left content height */}
          <div className="hvisual" aria-hidden="true">
            {/* Task 1e: hscene has no fixed width — uses CSS var for height from ResizeObserver */}
            <div className="hscene"
              style={isDesktop && imgHeight ? {'--hscene-h':`${imgHeight}px`} : {}}>
              <div className="hglow"/>
              <div className="hwrap">
              ...
              {/* Floating icons — Task 1f: positioned just outside frame on mobile via CSS */}
              {FLOAT_ICONS.map(ic=>(
                <div key={ic.cls}
        ...
        {/* Scroll down button — scrolls by 88vh, no SectionSnap interference */}
        <button className="hscroll-btn"
