...
// TASK 3a: card hover border = that card's category color (not always --accent-primary)
// TASK 3b: only card title is clickable (navigates to detail) — rest of card is not a link
...
// Task 3a: hover border = card's own category color
// Task 3b: only title is a Link — card div itself is not clickable
function ProjectCard({ p, i }) {
  const color = CAT_COLORS[p.category] ?? CAT_COLORS.default
  return (
    <motion.div
      className="proj-card card flex flex-col overflow-hidden"
      ...
      <div className="relative h-40 bg-[var(--bg-surface-2)] overflow-hidden flex-shrink-0">
        ...
        {/* External link buttons on hover */}
        <div className="proj-card-actions absolute top-3 right-3 flex gap-1.5">
          {p.github_link && (
      ...
      <div className="p-4 flex flex-col gap-2.5 flex-1">
        ...
        {/* Task 3b: ONLY title is a Link — rest of card body is plain text */}
        <Link
          to={`/projects/${p.slug}`}
          className="proj-card-title font-display font-bold text-[var(--text-primary)] leading-snug
            hover:text-[var(--accent-primary)] transition-colors duration-200 line-clamp-2 text-sm
            focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:rounded-sm"
        ...
        <div className="pt-2 border-t border-[var(--border-color)]">
          {/* Informational link row — uses Link for the icon arrow */}
          <Link
            to={`/projects/${p.slug}`}
      ...
      <div className="container-xl">
        ...
        <style>{`
          ...
          .proj-card:hover {
            ...
            box-shadow: 0 8px 28px rgba(0,0,0,.18), 0 0 0 1px var(--card-color, transparent);
          ...
          /* Mobile: hide 4th+ cards */
          @media(max-width:639px) {
            .proj-grid > *:nth-child(n+4) { display:none; }
          }
          /* Tablet: hide 5th+ cards */
          @media(min-width:640px) and (max-width:1023px) {
            .proj-grid > *:nth-child(n+5) { display:none; }
          }
        `}</style>
