...
// Fully persistent cookie consent — shows once per browser, never again after accept/decline.
// Controlled from Layout.jsx via enabled prop (home page only).
...
export default function CookieBanner({ enabled = true }) {
  ...
  // checkedRef prevents re-running when enabled flips (navigating back to home)
  const checkedRef = useRef(false)

  useEffect(() => {
    // If already decided in a previous session — never show again
    ...
    if (!stored) {
      // Small delay so hero content renders first
      const t = setTimeout(() => setVisible(true), 1600)
      return () => clearTimeout(t)
    }
    // Already answered — stay hidden permanently
  }, [enabled])
