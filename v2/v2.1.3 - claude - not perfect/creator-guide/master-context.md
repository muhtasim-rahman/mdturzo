...
## v2.1.3 — Footer Redesign (v9) + Navbar Polish

**Footer (সম্পূর্ণ নতুন — footer-v9-claude-deepseek.html same-to-same):**
- Font: Sora + DM Serif Display (Google Fonts, `@import` in style tag)
- Stay Connected banner: blue gradient (135deg, blue-700→blue-500→sky-400), rounded-top (28px), full-width above footer shell
  - Left: DM Serif Display heading "Stay Connected with My Work"
  - Right: email form (glass pill) + animated subscriber count + success message
  - Mobile: submit btn → round icon-only; layout stacks to column
- Footer main grid: `2fr 1fr 1fr 1.6fr` — Brand | Explore | Legal | Get in Touch
- Brand col: T-logo (gradient) + status dot (bottom-right) + name + handle + desc + 8 social icons (inline SVG) + location
- Social icons: 28×28 rounded squares (GH, LI, X, IG, YT, FB, Threads, TikTok)
- Explore: Home, About, Projects, Feed, Contact — React Router `<Link to=...>`
- Legal: Privacy Policy, Cookies Policy, Terms of Use, Sitemap (external)
- Contact col: "Let's Collaborate" card (→ /contact) + email card (mailto:)
- Bottom bar: copyright + version badge + scroll-to-top (desktop) + border-straddle btn (mobile)
- Responsive: 4-col → 3-col (tablet ≤1024) → 2-col (mobile ≤600) → 1-col (≤300px)
- Dark/light: `--nf-*` CSS variable prefix (avoids conflicts with global vars)

**Footer — Firebase subscriber count:**
- `onSubscriberCount()` reads RTDB real-time count
- `subscribeEmail()` writes subscription
- RTDB schema unchanged from v2.1.2 — full Firebase integration deferred to later version

**Footer + Navbar — Status dot (4 modes):**
- Modes: `active | busy | away | offline`
- Currently hard-coded to `'active'` via `CURRENT_STATUS` constant in both files
- **FIREBASE TODO (future version):** Read `/status/presenceMode` from Firebase RTDB to control status in real-time. Store in Zustand or local state.
- Colors: active=#22c55e, busy=#ef4444, away=#f59e0b, offline=#6b7280
- Hover: HTML `title` attribute shows mode label ("Active" etc)
- Footer logo dot + Navbar T-avatar dot (top navbar, floating navbar, sidebar) — all synced

**Navbar (v2.1.3 changes):**
- Mega menu redesign: 3-col (Pages | Account | Legal), no color-coded backgrounds
  - Branding header row: T-logo + navName + tagline + "Hire Me" CTA (→ /contact)
  - Clean icon+label rows, hover bg only
  - Footer row: siteURL + version badge
- Mobile/tablet search: clicking search icon → opens sidebar + auto-focuses search input + keyboard activates
  - Typing → nav sections hide → search placeholder shown
  - Escape clears query
  - **FUTURE:** Full search engine (page/content indexing) in a later version
- Floating navbar glass: `blur(28px) saturate(200%)`, multi-layer inset shadows (dark), white glow (light)
- All icon buttons: `data-tooltip` CSS tooltip (::after pseudo-element)
- Footer ↔ Navbar nav links always aligned (same NAV_LINKS constant, same Explore links in Footer)

**Files changed:**
- `src/components/layout/Footer.jsx` — complete rewrite
- `src/components/layout/Navbar.jsx` — mega menu + search + status dot + glass effect
- `creator-guide/master-context.md` — this update

**site.config.js version:** bump to `v2.1.3`
