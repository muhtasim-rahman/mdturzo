...
## v2.2.8 — Final Polish Pass

**Commit:** `fix(v2.2.8): hero frame, navbar bugs, skills compact, showcase section, github redesign`

**Changes by file:**

- `src/config/site.config.js` — version bumped to `v2.2.8`
- `src/components/home/Hero.jsx` — MULTIPLE FIXES:
  - **Large screen `.himg-frame`**: `background`, `border`, `box-shadow`, `::before` (top-bar) fully removed. Image floats with transparent frame, no visual decoration. Bottom-blend `::after` gradient kept.
  - **`.himg-box` height**: increased ~35px on all large-screen breakpoints (2xl: 655→655px, xl: 595px, lg: 495px, md-lg: 425px).
  - **`Available for hire` badge**: completely removed — no badge, no wrapper element.
  - **Name responsiveness redesigned**:
    - Desktop (≥900px): 2-line layout unchanged, `(Turzo)` inline via `.hname-nick`
    - Tablet (480-899px): flex row, single line `Muhtasim Rahman (Turzo)` via `flex-wrap:nowrap`
    - Mobile (<480px): wraps to 2 lines — `Muhtasim Rahman` on top, `(Turzo)` as `display:block; width:100%` centered below, smaller font
  - **Tablet/mobile circular frame**: purple/indigo alt-color ring via `box-shadow` — animating pulse `hring-p` / `hring-p-l` (dark/light variants)
- `src/components/layout/Navbar.jsx` — MULTIPLE BUG FIXES:
  - **Double search icon**: `NavRight` component now renders exactly ONE search `IconBtn`. Hamburger/menu button REMOVED from `NavRight` — hamburger only appears in the tablet/mobile sections outside `NavRight`.
  - **Mega nav double-render**: `MegaMenu` rendered ONCE outside both navbar elements as `position:fixed` with `top: megaTop` (switches between `72px` float height vs `var(--navbar-h)` based on `floatVisible`).
  - **Mega nav outside-click**: transparent full-screen backdrop (`position:fixed; inset:0; zIndex:9999`) added behind the panel. Click closes mega nav instantly.
  - **CSS tooltips**: all `nb-icon-btn` elements retain `data-nb-tip` attribute for CSS-only tooltips via `::after` pseudo-element. Right-anchored variant via `[data-nb-right]`.
  - Float nav right-side buttons wrapped in `.float-nav-right` class for visual polish.
- `src/components/home/Skills.jsx` — STAT CARDS REDESIGNED:
  - Cards now use horizontal layout (icon + label left, number right) instead of stacked vertical
  - Padding reduced: `0.6rem 0.75rem` (was `0.9rem 1rem`)
  - Icon reduced: 22px (was 26px), 6px border-radius (was 7px)
  - `::after` corner glow decoration REMOVED entirely
  - `hover: translateY(-2px)` (was -3px)
  - Number size: `1.22rem` (was `1.65rem`) — compact
  - No `subLabel` field any more
- `src/components/home/Process.jsx` — REPLACED "How I Work" section:
  - Old 6-step workflow grid + value props REMOVED entirely
  - New section: **"What I Deliver"** — screenshot/mockup showcase
  - `BrowserMockup`: styled HTML browser chrome with a mini faithful rendition of the portfolio hero section inside it (mini navbar, name, typewriter cursor, CTA buttons, stats, avatar)
  - `EditorMockup`: VS Code-style dark editor panel with syntax-highlighted JSX code snippet (`Hero.jsx` excerpt)
  - Grid: 2-col on desktop (browser 1.45fr : editor 1fr), stacks on ≤860px
  - Below grid: 4-col highlight chips (Performance First, Clean Codebase, Pixel-Perfect UI, Dark+Light Mode) with accent dots
  - CTA button linking to `/projects`
  - No section-alt (plain bg)
- `src/components/home/GithubStats.jsx` — PROFILE + STATS REDESIGN:
  - **ProfileCard** (full redesign): cover gradient bg (72px purple/indigo gradient strip), avatar with ring, name + handle + bio + meta row (location, blog, joined date), visit button, stat pills below a divider. Replaces old horizontal flex `gh-profile-row`.
  - **StatsImageRow**: new 2-col grid — LEFT = `streak-stats.demolab.com` streak image (theme-aware dark/light URL param), RIGHT = `awesome-github-stats.azurewebsites.net` level card (theme-aware). Both have fallback states and bottom label.
  - Old `ContribStats` component REMOVED (it was replacing the broken streak — now proper streak image used).
  - `useThemeStore` imported to detect dark/light for image URL theme params.
  - Streak URL: `https://streak-stats.demolab.com?user=muhtasim-rahman&theme=dark|default&hide_border=true&background=transparent`
  - Awesome stats URL: `https://awesome-github-stats.azurewebsites.net/user-stats/muhtasim-rahman?cardType=level&fontFamily=42dot%20Sans&preferLogin=false&theme=dark|light`

**Architecture notes for v2.2.9+:**
- Hero image frame: large screen has NO frame styling — pure transparent. Only mobile gets the ring. Do not add border/shadow back.
- Mega nav: single render location is the JSX block after the floating navbar `</AnimatePresence>`. Keep it there. The `megaTop` variable switches top offset correctly.
- Process.jsx now exports a "What I Deliver" showcase — not a workflow. If user wants workflow back, create a new component.
- GithubStats streak: uses `demolab.com` which is reliable. If it breaks, swap to `github-readme-streak-stats.vercel.app` with same params.
- `useThemeStore` in GithubStats: call `isDark()` as a function (it returns boolean). Pass the result as `dark` prop.
