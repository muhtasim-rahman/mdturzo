// Hero.jsx -- v2.2.9
// CHANGES:
//   * "Muhtasim Rahman" in single white span (no accent split)
//   * "(Turzo)" primary color, absolute positioned below-right of name on lg+
//   * Hero section: large screen explicit min-height (100dvh)
//   * Hero bottom gradient starts 5px below frame (bottom:-5px)
//   * Double dashes replaced with em-dash ( — )
//   * Small screen: "Download" text hidden, both CTA buttons in one row
//   * Light mode: waving_hand + salam color improved
//   * Hero inner max-width aligned with container-xl (1120px)
//   * Right side image gets slight right margin tweak
...
.hgreet-salam{
  font-family:'Georgia',serif;font-style:italic;font-weight:600;font-size:.88rem;
  color:#D97706;
}
[data-theme=light] .hgreet-salam{color:#1D4ED8;}
.hgreet-rest{color:var(--text-primary);font-family:var(--font-mono);font-weight:500;font-size:.82rem;opacity:.9}
.hgreet-wave{
  font-size:1.1em;vertical-align:middle;line-height:1;display:inline-flex;align-items:center;
  color:#F59E0B;
}
[data-theme=light] .hgreet-wave{color:#D97706;}

/* name */
.hname{
  font-size:clamp(2.2rem,4.5vw,4.2rem);font-weight:800;line-height:1.02;
  letter-spacing:-.03em;font-family:var(--font-display);
  color:var(--text-primary);
  position:relative;
}
.hname-line{display:block;overflow:hidden;line-height:1.15;position:relative;}
...
/* v2.2.9: Muhtasim Rahman all white (no accent split) */
.hname-main{color:var(--text-primary);}

/* (Turzo) — primary color, absolute on large screen */
.hname-turzo{
  font-size:.38em;font-weight:500;
  color:var(--accent-primary);
  letter-spacing:.02em;
  font-family:var(--font-mono);
  display:block;
  margin-top:.15em;
  opacity:0;animation:hfade .5s ease 1.1s forwards;
}
...
/* v2.2.9: hide "Download" text on mobile, keep icon */
.hbtn-dl-text{display:inline}
...
/* v2.2.9: gradient starts 5px below image (bottom:-5px) so no gap when zoomed */
.himg-frame::after{
  content:'';position:absolute;z-index:2;pointer-events:none;
  bottom:-5px;left:0;right:0;height:40%;
  background:linear-gradient(to top,var(--bg-page) 0%,var(--bg-page) 12%,rgba(2,6,23,.7) 34%,transparent 100%);
}
[data-theme=light] .himg-frame::after{
  background:linear-gradient(to top,var(--bg-page) 0%,var(--bg-page) 10%,rgba(240,244,248,.8) 32%,transparent 100%);
}
...
@media(min-width:1536px){
  .hero-inner{grid-template-columns:1.1fr 1fr;max-width:1280px}
...
@media(max-width:899px){
  ...
  .hvisual{order:1;height:auto;justify-content:center}
  ...
  /* v2.2.9: (Turzo) block on mobile */
  .hname-turzo{display:block;font-size:.42em;margin-top:.08em;text-align:center;}
}
...
@media(max-width:479px){
  ...
  /* v2.2.9: hide "Download" text, keep icon, both btns same row */
  .hbtn-dl-text{display:none}
  .hcta{flex-wrap:nowrap;gap:.5rem}
  .hbtn{padding:.58rem .9rem;font-size:.8rem}
}
      ...
      <section className="hero" id="hero" aria-label="Introduction">
        ...
        <div className="hero-inner">
          ...
          <div className="hcontent">
            {/* v2.2.9: Assalamu Alaikum with better light-mode colors */}
            ...
            {/* v2.2.9: Name — Muhtasim Rahman in one white span; (Turzo) primary below */}
            <h1 className="hname" aria-label="Muhtasim Rahman (Turzo)">
              <span className="hname-line">
                <span className="hname-in" style={{'--d':'70ms'}}>
                  <span className="hname-main">Muhtasim Rahman</span>
                </span>
              </span>
              <span className="hname-turzo">(Turzo)</span>
            </h1>
            ...
            {/* v2.2.9: no flex-wrap so both stay in one row; on xs "Download" text hidden */}
              ...
              <span className="hbtn-wrap">
                {cvEnabled && cvUrl && cvUrl !== '#'
                  ? <a href={cvUrl} target="_blank" rel="noopener noreferrer" className="hbtn hbtn-o">
                      ...
                      <span className="hbtn-dl-text"> Download CV</span>
                    </a>
                  : <span className="hbtn hbtn-o" style={{opacity:.7,cursor:'default'}}>
                      <FontAwesomeIcon icon={faDownload} aria-hidden="true"/>
                      <span className="hbtn-dl-text"> CV Soon</span>
                    </span>
                }
                <span className="hbtn-tt">{cvEnabled && cvUrl && cvUrl !== '#' ? 'Download my resume as PDF' : 'CV will be uploaded soon'}</span>
              </span>
            </div>
