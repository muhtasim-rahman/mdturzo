// AboutMini.jsx -- v2.2.9
// FIXES:
//   - Bullet separator ? -> proper unicode bullet character
//   - Light mode image overlay opacity correction
//   - About section right-margin alignment fix
...
// v2.2.9: use unicode bullet \u2022 instead of raw • (fixes ? rendering)
const BULL = '\u2022'

const FACTS = [
  { icon:faLocationDot,   color:'#3B82F6', label:'Location', value:'Nilphamari, Bangladesh'                            },
  { icon:faGraduationCap, color:'#10B981', label:'School',   value:`SSC-26 ${BULL} SGSC`                               },
  { icon:faLaptopCode,    color:'#F59E0B', label:'Goal',     value:'CSE Engineer & Developer'                          },
  { icon:faLanguage,      color:'#EC4899', label:'Languages',value:`Bengali ${BULL} English ${BULL} Hindi`             },
  { icon:faHeart,         color:'#A855F7', label:'Values',   value:`Islam ${BULL} Discipline ${BULL} Quality`          },
]
    ...
    <section className="section section-alt" id="about-mini">
            ...
            <div className="relative w-72 sm:w-80 lg:w-96">
              ...
              <div className="relative rounded-2xl overflow-hidden aspect-[3/4] border border-[var(--border-strong)] bg-[var(--bg-surface-2)] shadow-[var(--shadow-xl)]">
                ...
                <div className="absolute bottom-4 left-4">
                  ...
                  <p className="text-white/60 text-xs mt-0.5">Age {age} {BULL} Bangladesh</p>
                </div>
              </div>
          ...
          <motion.div className="flex flex-col gap-5"
            ...
            <motion.div variants={up} className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {FACTS.map(({icon,color,label,value})=>(
                <div key={label} className="flex items-start gap-2.5 p-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-color)] hover:border-[var(--accent-primary)] transition-colors duration-200 active:scale-[.98]">
      ...
      <style>{`
        ...
        [data-theme="light"] .about-img-overlay {
          background: linear-gradient(to top, rgba(248,250,252,0.20), transparent);
        }
      `}</style>
    </section>
  )
