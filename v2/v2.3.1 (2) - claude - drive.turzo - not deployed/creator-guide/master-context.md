...
## v2.3.1 — About Page redesign (2026-05-25)

### Version History (recent)

| Version | Summary |
|---|---|
| v2.2.9 | Hero polish, navbar, GitHub redesign, journey timeline, skills compact |
| v2.3.0 | Full About page — hero, story, education timeline, skills (tabbed), languages, values, goals, CTA |
| **v2.3.1** | **About page full redesign; shared CTA; Layout fix for /about top padding; listed unused home components** |
...
**Section order:**
1. **Hero** — Minimal, clean. Right: `hero-back.webp` with exact same bottom gradient as home hero (`himg-frame::after`). Left: breadcrumb, eyebrow, h1 name/tagline, bio, quick-fact chips, CTA buttons, social icons. No "Available for Work" badge. No scroll hint. Mobile/tablet: image stacks above left content, no rounded/frame design.
2. **About Me Details** — Quote card (left), 3 story cards (right), 4 experience stat cards (spans both columns). `ab-` classnames.
3. **Education Timeline** — Alternating left/right on desktop (copy-3 style, 3-col grid `[card | dot | card]`). Left-aligned on mobile. Center line (desktop) / left line (mobile) fills progressively on scroll via `lineProgress` state driven by `window.scroll` listener. `TLItem` uses framer-motion `useInView` per-item entrance.
4. **Skills & Expertise** — Tabbed: Programming / Design / Video / Tools. Progress bars animated via `useInView` + staggered setTimeout (same pattern as home Skills). Layout from copy-4 (`1fr 280px` side panel on desktop).
5. **Language Proficiency** — 2×2 grid. Each card: `flagcdn.com` flag img, language name, level badge (colored), animated progress bar (`whileInView`). No npm flag library — uses free CDN img.
6. **Values & Personality** — 6-card grid (3 cols desktop). Hobbies & Interests chip row inside this section.
7. **Goals & Plans** — 3-col grid. Click ripple effect via `RippleCard` component (custom, no library). No progress bars. Cards highlight on hover with `--goal-color` CSS var.
8. **Find Me Online** — 5-col responsive grid (10 platforms: GitHub, LinkedIn, Facebook, Instagram, YouTube, X, Telegram, TikTok, Threads, Email). Each card: icon, label, handle, external link arrow on hover.
9. **CTA** — Shared `<CTA />` component from `components/home/CTA.jsx`.

**Key implementation details:**
- All styles in `<style>` tag at bottom. Class prefix: `ab-`.
- Age: `calculateAge()` from `site.config.js`.
- Flag images: `https://flagcdn.com/32x24/{cc}.png` (zero dependency, fast CDN).
- Timeline line: `lineProgress` (0–1) drives `height` of `.ab-tl-line-fill`. Scroll listener, passive event.
- Ripple: `RippleCard` wrapper tracks click coordinates, renders `.ab-ripple` span with CSS `scale` animation.
- Hero image gradient: `.ab-h-imggrad` — exact same as `.himg-frame::after` in `Hero.jsx`.

#### `src/components/home/CTA.jsx` — Redesign + Shared

Completely redesigned CTA section. Now used in **both** `Home.jsx` and `About.jsx`.

- "Open to opportunities" green pulse badge
- Large gradient-text heading
- Two buttons: "Get in touch" → /contact, "View Projects" → /projects
- Dot-grid bg texture + two ambient orbs
- Footer note: "Fast response · Clean code · On-time delivery"

#### `src/components/layout/Layout.jsx` — Fix

Changed `isHome` check from `['/', '/home']` to `['/', '/home', '/about']`.

**Why:** The About hero section already has `padding-top: calc(var(--navbar-h) + ...)`. Adding `pt-navbar` (68px) on top of that created double-offset causing excess whitespace.


#### File Map (v2.3.1 changes)
```
src/
  pages/
  ...
  components/
    home/
      CTA.jsx           ← v2.3.1 (redesigned + now shared)
      ...
      Stats.jsx         ← v2.2.8 (BUILT BUT NOT SHOWN — see below)
      Services.jsx      ← v2.2.8 (unchanged)
      Testimonials.jsx  ← v2.2.8 (unchanged)
      BlogMini.jsx      ← v2.2.x (BUILT BUT NOT SHOWN — see below)
      RecentProjectsOrginal.jsx ← (BUILT BUT NOT SHOWN — see below)
    layout/
      Layout.jsx        ← v2.3.1 (isHome now includes /about)
  config/
    site.config.js      ← v2.3.1 (version bump)
```

#### Home components BUILT but NOT shown on homepage

| Component | File | Why not shown |
|---|---|---|
| **Stats** | `Stats.jsx` | Not imported in `Home.jsx`. Displays 4 animated stat counters. |
| **BlogMini** | `BlogMini.jsx` | Explicitly removed from Home.jsx (comment: "BlogMini REMOVED from homepage — not in spec"). Renders mini blog post cards. |
| **RecentProjectsOrginal** | `RecentProjectsOrginal.jsx` | An older/alternate version of the projects section. Replaced by `RecentProjects.jsx`. |

#### Flag CDN (no install needed)
```
https://flagcdn.com/32x24/{country-code}.png
https://flagcdn.com/64x48/{country-code}.png  (2x retina)
Codes: bd=Bangladesh, gb=UK, in=India, pk=Pakistan
```
...
```
v2.3.1 — About page redesign + shared CTA + Layout fix

1. ABOUT (full redesign): minimal hero with hero-back.webp (same gradient as home); about-details section with quote + story cards + exp stats; education timeline with scroll-animated center/left line (lineProgress state); skills tabbed panel (Dev/Design/Video/Tools) with animated bars + side info card; language proficiency with flagcdn.com country flags + animated bars; values & personality 6-card grid with hobbies inside; goals & plans 3-col with ripple click effect; find-me-online 5-col social grid (10 platforms); shared CTA component
2. CTA (shared): redesigned, now imported by both Home and About; green availability badge, gradient heading, dot-grid bg
3. LAYOUT: /about added to isHome list — removes extra pt-navbar top padding from About page
4. CONFIG: version bumped to v2.3.1 in site.config.js and package.json
5. BONUS: Home components built but not shown on homepage — Stats.jsx, BlogMini.jsx, RecentProjectsOrginal.jsx
```
