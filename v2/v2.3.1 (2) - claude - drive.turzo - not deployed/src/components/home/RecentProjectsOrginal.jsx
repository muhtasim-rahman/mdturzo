...
// CHANGES:
//   * Full project card is now clickable (Link wraps whole card)
//   * Title hover: color does NOT change (removed hover:text-[accent] from title)
//   * Card hover: card border animates to card's accent color (existing behavior kept)
//   * Card footer: styled with card's theme color -- tinted bg, colored text
//   * Admin context note: accent color from project.accent_color or CAT_COLORS[category]
...
function ProjectCard({ p, i }) {
  // accent_color from admin panel takes priority, otherwise fall back to category color
  const color = p.accent_color || CAT_COLORS[p.category] || CAT_COLORS.default
  const [hovered, setHovered] = useState(false)

  return (
    // Full card is a Link -- clickable anywhere
    <motion.div
      className="rp-card card flex flex-col overflow-hidden transition-all duration-300"
      style={{ borderColor: hovered ? color : undefined, '--card-color': color }}
      ...
      {/* Thumbnail -- clickable */}
      <Link to={`/projects/${p.slug}`} className="block relative h-40 bg-[var(--bg-surface-2)] overflow-hidden flex-shrink-0 group">
        {p.thumbnail_url
      ...
      {/* Card body */}
      <Link to={`/projects/${p.slug}`} className="flex flex-col gap-2.5 flex-1 p-4 group/body">
        {p.tags?.length > 0 && (
        ...
        {/* Title -- color stays constant, no hover color change */}
      ...
      {/* Card footer -- themed with card's accent color */}
      <Link
        to={`/projects/${p.slug}`}
        className="rp-card-footer flex items-center justify-between px-4 py-2.5 transition-all duration-200"
        style={{
          background: hovered ? `${color}14` : `${color}08`,
          borderTop: `1px solid ${color}22`,
        }}>
        <span className="text-xs font-semibold" style={{ color }}>View details</span>
        <FontAwesomeIcon
          icon={faArrowRight}
          className="text-[10px] transition-transform duration-200"
          style={{ color, transform: hovered ? 'translateX(3px)' : 'none' }}
        />
      </Link>
    </motion.div>
  )
      ...
      <div className="container-xl">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
          ...
          <motion.div initial={{ opacity:0, x:20 }} whileInView={{ opacity:1, x:0 }}
            ...
            <Link to="/projects"
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--accent-primary)] transition-colors duration-200 group">
        ...
        <motion.div className="flex justify-center mt-10"
          ...
          <Link to="/projects"
            className="inline-flex items-center gap-2 px-7 py-3 rounded-xl font-semibold text-sm border border-[var(--border-strong)] text-[var(--text-secondary)] hover:border-[var(--accent-primary)] hover:text-[var(--accent-primary)] transition-all duration-200 active:scale-[.97] group">
            View All Projects
