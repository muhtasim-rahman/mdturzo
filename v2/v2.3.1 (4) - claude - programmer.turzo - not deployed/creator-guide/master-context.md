...
|---|---|
| v2.3.0 | Full About page built — hero, story, education timeline, skills (tabbed), languages, values, goals, CTA |
| **v2.3.1** | **About page full redesign + shared SiteCTA + Layout double-padding fix** |

### v2.3.1 Change Log Summary

#### New File: `src/components/shared/SiteCTA.jsx`
Shared CTA used on both Home and About pages. Replaces `home/CTA.jsx` (still kept as backup).
- Dot-grid bg animation + radial glow
- Badge with green pulse dot
- Gradient heading (accent → indigo → purple)
- Primary ("Get In Touch") + Secondary ("See My Work") buttons
- Footer note line

#### Modified: `src/pages/About.jsx` — Full Redesign

**Section 1 — Hero**
- Minimal 2-col: left content + right `hero-back.webp`
- No floating cards, no scroll hint, no Available-for-Work badge
- Right: same gradient as home hero (linear-gradient to top, page bg → transparent)
- Mobile/tablet: image stacks on top, NO round/frame — plain rectangle
- Left: breadcrumb, "About Me" label, Name + (Turzo) mono block, role, bio, chips (location/school/age/religion), social row, 2 CTA buttons

**Section 2 — Info / My Story**
- 2-col: blockquote (from about.md) + 3 exp-stat cards + 3 story cards (Early Spark / Learning in Progress / What's Next)

**Section 3 — Education Timeline**
- PC: centered, cards alternate left-right (copy-3 style)
- Mobile: left line, all cards on right
- Animated line: `useScroll` + `useTransform` scaleY 0→1 as user scrolls through section
- 7 entries: St.Geroza → Tulshiram → Lions → SGSC(2021-25) → SGSC SSC-26(current) → HSC(upcoming) → CSE(dream)
- Map links on SGSC entries

**Section 4 — Skills & Expertise**
- 4 tabs: Programming / Design / Video / Tools
- Programming: 7 bars with gradient fill + glow, home-style animation (inView trigger)
- AnimatePresence for tab transitions
- Design: 3-col grid cards; Video: list with color dots; Tools: 4-col grid

**Section 5 — Language Proficiency**
- Flag icons via CDN (`flag-icons` v7.2.3, dynamically injected)
- No npm package needed — `fi fi-{code}` CSS classes
- 4 bars: Bengali(bd) / English(gb) / Hindi(in) / Urdu(pk)
- Colored glow on fill bars

**Section 6 — Values & Personality**
- 5 minimal value cards (Islam First / Discipline / Useful Knowledge / Honesty / Perfection)
- Hobbies & Interests chips embedded below inside same section (8 items)

**Section 7 — Goals & Plans**
- Copy-3 layout (3-col grid)
- Cards: thin top accent bar + icon header + checklist
- Full card click effect (active:scale .97), hover lift
- No progress bars

**Section 8 — Find Me Online**
- New design: grid of social cards
- 10 platforms: GitHub, LinkedIn, Facebook, Instagram, YouTube, X/Twitter, Telegram, TikTok, Threads, Email
- Each: left colored accent bar, icon box, name + handle, arrow
- 2→3→5 col responsive grid

**Section 9 — SiteCTA** (shared component)

#### Modified: `src/components/layout/Layout.jsx`
`SELF_SPACED = ['/', '/home', '/about']` — pages that manage their own navbar offset.
About was getting double top spacing (pt-navbar from Layout + calc(navbar-h + 4rem) in hero).

#### Modified: `src/pages/Home.jsx`
Uses `SiteCTA` from `shared/` instead of `home/CTA.jsx`.

#### Modified: `src/config/site.config.js`
Version: `v2.3.1`
...
src/
  pages/
    About.jsx              ← v2.3.1 REDESIGN
    Home.jsx               ← v2.3.1 (SiteCTA swap)
  components/
    home/
      Hero.jsx             ← v2.2.9 (unchanged)
      AboutMini.jsx        ← v2.2.9 (unchanged)
      ...
      CTA.jsx              ← v2.2.8 (KEPT — not deleted, not used on home)
      Stats.jsx            ← v2.2.0 (unused — see note)
      BlogMini.jsx         ← v2.2.0 (unused — removed from homepage per spec)
      RecentProjectsOrginal.jsx ← backup copy (unused)
    shared/
      SiteCTA.jsx          ← v2.3.1 NEW — used on Home + About
      AdminQuickActions.jsx← (unchanged)
      VisibilityGuard.jsx  ← (unchanged)
    layout/
      Layout.jsx           ← v2.3.1 (SELF_SPACED fix)
      Navbar.jsx           ← v2.2.9 (unchanged)
      Footer.jsx           ← v2.2.8 (unchanged)
  config/
...
### Home Components Built But NOT Shown on Home Page

| File | Status | Reason |
|---|---|---|
| `Stats.jsx` | Built, unused | Animated stat counters — available but not in current Home spec |
| `BlogMini.jsx` | Built, unused | Blog preview — intentionally removed from homepage per spec |
| `CTA.jsx` | Built, replaced | Replaced by shared `SiteCTA.jsx` — kept as reference |
| `RecentProjectsOrginal.jsx` | Built, backup | Original version of RecentProjects — superseded |

### Design Tokens (from index.css)
- `--navbar-h: 68px`
- `--font-display` — display/heading font
- `--font-mono` — mono font
- `--accent-primary` — main blue accent
- `--accent-hover` — darker accent for hover states
- `--accent-light` — light accent background
- `--bg-page` — main page background
- `--bg-surface` — card/surface bg
- `--bg-surface-2` / `--bg-surface-3` — elevated surfaces
