...
export async function getPublishedPosts({ limit, offset = 0, category, tag } = {}) {
  let query = supabase
    .from('posts')
    .select(`
      id, slug, title, description, embed_url, platform, thumbnail_url,
      tags, category, status, visibility,
      views_count, likes_count, dislikes_count, comments_count,
      created_at, updated_at
    `)
    .eq('status', 'published')
...
export async function getRelatedPosts(currentSlug, category, tags = [], limit = 3) {
  let { data: catData } = await supabase
    .from('posts')
    .select('id, slug, title, description, embed_url, platform, thumbnail_url, views_count')
    .eq('status', 'published')
    .eq('visibility', 'public')
    .eq('category', category)
    .neq('slug', currentSlug)
