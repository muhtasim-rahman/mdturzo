...
// ── v2.5.2: trackContentView (deduplicated, 3-day cooldown) ──
export async function trackContentView(contentType, contentId, viewerKey, isAuthenticated = false) {
  if (!contentId || !viewerKey) return
  const { data, error } = await supabase
    .rpc('track_content_view', {
      p_content_type: contentType,
      p_content_id:   String(contentId),
      p_viewer_key:   String(viewerKey),
      p_is_auth:      isAuthenticated,
    })
  if (error) {
    console.warn('[trackContentView] RPC failed, using legacy:', error.message)
    if (contentType === 'blog')    await incrementBlogViews(contentId)
    if (contentType === 'post')    await incrementPostViews(contentId)
    if (contentType === 'project') await incrementProjectViews(contentId)
  }
  return data
}

// ── v2.5.2: getRepostsCount ────────────────────────────────────
export async function getRepostsCount(contentType, contentId) {
...
// ── v2.5.2: getUserRepostStatus ───────────────────────────────
export async function getUserRepostStatus(contentType, contentId, userId) {
  if (!userId) return false
...
// ── v2.5.2: toggleRepost ──────────────────────────────────────
export async function toggleRepost(contentType, contentId, userId, note = null) {
  if (!userId) throw new Error('Must be logged in to repost')
  const { data, error } = await supabase
    .rpc('toggle_repost', {
      p_content_type: contentType,
      p_content_id:   String(contentId),
