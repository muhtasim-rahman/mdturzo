...
## v2.2.0 — Home Page Overhaul + About + Contact
...
- `src/config/site.config.js` — version bumped to v2.2.0
...
**Supabase dependencies (v2.2.0 new):**
...
## v2.2.3 — Hero Full Rework + Alignment + Nav Fixes

**New public asset needed:**
- `public/muhtasim-about.webp` — used in AboutMini section (place in public folder)

**Modified files:**
- `src/components/home/Hero.jsx` — complete rewrite: salam chip (monospace no-badge), name Muhtasim(accent)/Rahman(Turzo), slower typing, View Projects + Download CV btns, social hover effects, accent + stats, dual gradient layers (z:3 image, z:6 full), 760px PC max-height, ring-pulse circle on tablet/mobile, unconstrained image width, SVG 2D hologram ring bottom, floating icons staggered left/right
- `src/components/home/Skills.jsx` — count-up runs once only (useRef flag), no star ratings, clean % bars
- `src/components/home/AboutMini.jsx` — uses muhtasim-about.webp
- `src/components/layout/Navbar.jsx` — max-width 1280px; padding clamp(1rem,4vw,2rem); z-index 9999 on both navbars; mega panel z-index 99999; mega panel backdrop-filter blur
- `src/components/layout/Footer.jsx` — max-width 1280px
- `src/pages/Home.jsx` — passes settings to Skills; alternating section-alt order fixed
- `src/config/site.config.js` — v2.2.3
- `package.json` — 2.2.3

**Section alternating order (after hero):**
Skills=alt → About=plain → Services=alt → Projects=plain → Reviews=alt → Github=plain → Blog=alt → CTA=plain
