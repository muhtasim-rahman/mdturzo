...
## v2.2.6 — Scroll Snap, Navbar Polish, Hero Height Fix, CTA Redesign

> (Context from v2.2.6 build — refer to v2.2.6 zip for full details)
...
## v2.2.9 — Home page polish (2026-05-24)
...
## v2.3.1 — About page redesin (2026-05-25)
...
| v2.2.9 | Alignment/margin fixes, Hero (Turzo) absolute, GitHub full redesign, navbar tooltip + mega-close, project cards redesign, My Setup update, click effects, bullet fixes |
| v2.3.0 | Full About page built — hero, story, education timeline, skills (tabbed), languages, values, goals, CTA |
| **v2.3.1** | **About page full redesign, SharedCTA component, Layout fix (about margin), home stats info** |

### v2.3.1 Change Log

#### Layout (`src/components/layout/Layout.jsx`)
- **Fix:** About page now excluded from `pt-navbar` class (was adding unwanted margin above hero)
- `noNavPadPages` array now includes `'/', '/home', '/about'`
- Both Home & About hero sections manage their own `padding-top: calc(var(--navbar-h) + ...)`

#### About Page (`src/pages/About.jsx`) — Full Redesign

Complete replacement of v2.3.0 About page. All CSS uses `ab31-` prefix. Sections:

**Section 1 — Hero (Redesigned)**
- Minimal, different from home hero — left text content, right `hero-back.webp` image
- Right side: same image + gradient style as home hero (`himg-frame::after` pattern — linear gradient from page bg)
- Tablet: no rounded/frame styles on image
- Mobile: image sits on top (full width), no border/frame
- Left content: breadcrumb nav → label → name → subtitle → bio → info chips → CTA buttons + social icons
- No "Available for Work" badge, no scroll icon
- Info chips: Location, School, Religion, Goal (minimal mono-font pills)
- CTAs: "Hire Me" (primary) + "Download CV" (outline, conditional on settings)
- Social icons: GitHub, LinkedIn, Facebook, Telegram, Email (compact icon buttons)
- Background: subtle dot-grid texture + 2 animated orbs (same style as home)

**Section 2 — Info Details (New section after hero)**
- 2-col grid: left = blockquote (self-written bio from about.md), right = 3 story cards
- Story cards: Early Spark / Learning in Progress / What's Next
- Below grid: full-width experience stats bar (4 stats: Web Dev yrs, Design yrs, Projects, Video yrs)
- Stats bar uses 1px border grid aesthetic (no cards — unified strip)

**Section 3 — Education Timeline**
- PC: center-line left-right alternating layout (copy-3 inspired)
- Mobile: left-line single-column
- Animated line: scroll-driven, top portion fills with accent→purple gradient, rest stays muted
- Animation: per-item `useInView` fadeIn from left/right on PC, from left on mobile
- Current entry (SSC-26) highlighted with accent border + glow dot
- Future entries (HSC, BSc) styled with dashed border + reduced opacity
- `faSchool`, `faGraduationCap`, `faTrophy`, `faBook`, `faRocket` icons per entry

**Section 4 — Skills & Expertise**
- 4 tabs: Programming | Design | Video | Tools
- **Programming tab:** `SkillBar` components with home-style animation (visible state + stagger delay), note text + percentage, glow shadow on fill
- **Design tab:** Grid of skill cards (icon + label), note box on right
- **Video tab:** Checklist items, note box on right
- **Tools tab:** 6 tool cards in auto-fill grid
- Tab switcher resets `skillVisible` for re-animation on tab change
- Styles: `ab31-skill-*` classes

**Section 5 — Language Proficiency**
- 2-col layout: left = section header + description, right = 4 language bars
- Country flags via `flag-icons` CDN (`fi fi-{code}` classes): bd, gb, in, pk
- Each bar: flag + name on left, level badge on right, animated fill with dot endpoint
- Progress bar glow dot at end of fill
- Bars animate `whileInView` with stagger delay

**Section 6 — Values & Personality**
- 6 value cards in auto-fit grid: Islam First, Perfection, Honesty, Discipline, Beneficial Knowledge, Growth Mindset
- Each: accent-colored icon box + title + description — minimal, 1-accent-color only
- Hover: subtle lift + border accent
- **Hobbies & Interests inside this section** (below values grid) — chip row: 8 items

**Section 7 — Goals & Plans**
- 3-column layout (auto-fit)
- `GoalCard` component (separate from map to respect React hooks rules)
- Click ripple effect on each card (wave animation from click point)
- Each: icon box + period + subtitle + checklist items
- No progress bars

**Section 8 — Find Me Online (New)**
- Full social grid: GitHub, LinkedIn, Facebook, Instagram, YouTube, Telegram, X, TikTok, Email (9 items)
- Each card: icon box (accent on hover with per-platform color via `--hc` CSS var) + label + handle + description
- Arrow icon, hover: translateX + color
- `color-mix()` for tinted shadow on hover

**Section 9 — SharedCTA**
- Uses `SharedCTA` component (shared with Home)

**SharedCTA (`src/components/shared/SharedCTA.jsx`) — New**
- Replaces `CTA.jsx` on Home page and the old About CTA
- Used on both Home and About pages
- Centered layout: icon → heading → sub-text → 3 buttons (Get in Touch, GitHub, Projects)
- Background: dot-grid texture + 2 animated orbs
- Gradient heading accent: `var(--accent-primary) → #818cf8 → #c084fc`
- Styles: `shared-cta-*` classes

#### Home (`src/pages/Home.jsx`)
- Replaced `CTA` import with `SharedCTA`
...
src/
  pages/
    About.jsx           ← v2.3.1 (FULL REDESIGN)
    Home.jsx            ← v2.3.1 (SharedCTA swap)
  components/
    home/
      Hero.jsx          ← v2.2.9 (unchanged)
      AboutMini.jsx     ← v2.2.9 (unchanged)
      ...
      Process.jsx       ← v2.2.9 (unchanged)
      Skills.jsx        ← v2.2.8 (unchanged)
      Testimonials.jsx  ← v2.2.8 (unchanged)
      Services.jsx      ← v2.2.8 (unchanged)
      CTA.jsx           ← v2.2.6 (REPLACED by SharedCTA on Home — file kept for reference)
      Stats.jsx         ← v2.2.0 (NOT shown on Home — see unused components list below)
      BlogMini.jsx      ← v2.2.0 (NOT shown on Home — see unused components list below)
      RecentProjectsOrginal.jsx ← backup file, not used
    shared/
      SharedCTA.jsx     ← v2.3.1 (NEW — used on Home + About)
      AdminQuickActions.jsx ← unchanged
      VisibilityGuard.jsx   ← unchanged
    layout/
      Layout.jsx        ← v2.3.1 (About page excluded from pt-navbar)
      Navbar.jsx        ← v2.2.9 (unchanged)
      Footer.jsx        ← v2.2.8 (unchanged)
  config/
    site.config.js      ← v2.3.0 (unchanged — version needs bump to v2.3.1)
  index.css             ← v2.2.9 (unchanged)
```

#### ⚠️ Home Page — Built Components NOT Currently Shown

These components exist and are built but are **not imported or rendered in Home.jsx**:

| Component | File | What it does | Why not shown |
|---|---|---|---|
| `Stats` | `home/Stats.jsx` | Animated stat counters (Dev yrs, Design yrs, Projects, Clients, Stars) from Supabase `site_settings` | Removed at some point — replaced by stats inside Hero and Skills sections |
| `BlogMini` | `home/BlogMini.jsx` | Shows recent blog/feed posts from Supabase, with time-ago, tags, card layout | Was between GithubStats and CTA — removed per spec (not in homepage plan) |
| `CTA` (old) | `home/CTA.jsx` | Original centered CTA section with dot-grid bg | Replaced by `SharedCTA` in v2.3.1 |
| `RecentProjectsOrginal` | `home/RecentProjectsOrginal.jsx` | Backup/original version of project cards | Backup file — not used, kept for reference |
...
- Navbar height: 68px (`--navbar-h`)
- Section padding: `clamp(3rem, 8vw, 6rem)`
- CSS class prefixes: `ab31-` (About v2.3.1), `shared-cta-` (SharedCTA)

#### About Page Architecture Details
- All styles in `<style>` tag at bottom of `About.jsx` — prefix `ab31-`
- `GoalCard` is a separate component (not inline in map) to respect React hooks rules (useRipple uses useState inside)
- Education timeline: `EduItem` component with own `useInView` ref; `isMobile` detected via window resize listener
- Timeline line: scroll listener on `timelineRef` computes `lineProgress` 0–1 → sets height on `.ab31-tl-line-fill`
- Skills: `skillSectionRef` → IntersectionObserver → `setSkillVisible(true)` → `SkillBar` animate
- Language flags: `flag-icons` v7.2.3 from cdnjs CDN — classes `fi fi-{iso2code}`
- Ripple: `useRipple()` hook returns `{ handleClick, rippleEls }` — CSS `@keyframes ab31-rip`
- Social cards: `--hc` CSS custom property per card for per-platform hover color + `color-mix()` shadow
...
```js
// DEV_SKILLS
AI Tools: 90% | HTML: 80% | CSS: 78% | Git/GitHub: 75% | Python: 55% | JS: 42% | Java: 35%

// LANGUAGES (flag codes)
Bengali: 100% (bd) | English: 65% (gb) | Hindi: 50% (in) | Urdu: 45% (pk)

// EDUCATION (8 entries)
St. Geroza → St. Geroza → Tulshiram Primary → Lions → SGSC 6-10 → SGSC SSC-26 (current) → HSC (future) → BSc CSE (dream)

// SOCIALS (9)
GitHub, LinkedIn, Facebook, Instagram, YouTube, Telegram, X, TikTok, Email
```

### Commit Message

```
v2.3.1 — About page redesign + SharedCTA + layout fix

1. ABOUT (full redesign): minimal hero with hero-back.webp (home-style gradient, no frame on tablet/mobile); new Info Details section with blockquote + story cards + stats strip; Education Timeline with scroll-driven animated center line (PC) / left line (mobile), left-right alternating cards, per-item entrance animation; Skills & Expertise with home-style animated progress bars + design/video/tools tabs; Language Proficiency with flag-icons country flags + animated fill bars with glow dot; Values & Personality grid (6 values) + Hobbies & Interests chips inside same section; Goals & Plans 3-col with click ripple effect; Find Me Online new social grid (9 platforms, per-platform hover color); SharedCTA at bottom
2. SHARED CTA: new SharedCTA component (src/components/shared/SharedCTA.jsx) — used on both Home and About; dot-grid bg + animated orbs + gradient heading + 3 buttons
3. HOME: replaced CTA with SharedCTA
4. LAYOUT: fixed extra top margin on About page — excluded from pt-navbar (hero manages its own padding)
5. All animations: scroll-driven timeline line, useInView skill bars, whileInView language bars, ripple on goal cards
6. flag-icons v7.2.3 via cdnjs CDN for language flags
```
