...
// APP.JSX — v2.3.6
// CHANGED (v2.3.6):
//   * RouteBoundary: Home ('hero') & About ('about') now show their own
//     bespoke composed skeletons instead of the generic PageSkeleton.
//   * /about given its own 'about' layout key (was sharing 'profile'
//     with /profile and /@:username, which would have shown the wrong
//     skeleton on those pages).
// PREVIOUS (v2.3.2):
...
import { HomeSkeleton }  from './components/skeletons/HomeSkeletons.jsx'
import { AboutSkeleton } from './components/skeletons/AboutSkeletons.jsx'
...
// v2.3.6: Home & About get bespoke, layout-matching skeletons instead of
// the generic PageSkeleton (still used for grid/detail/list/form/admin/etc).
function RouteFallback({ layout }) {
  if (layout === 'hero')  return <HomeSkeleton />
  if (layout === 'about') return <AboutSkeleton />
  return <PageSkeleton layout={layout} />
}

function RouteBoundary({ children, layout = 'blank', initialPending, onReady }) {
  return (
    <Suspense fallback={initialPending ? <PageLoader fullscreen /> : <RouteFallback layout={layout} />}>
    ...
    <AnimatePresence mode="wait" initial={false}>
      <Routes location={location} key={location.pathname}>
        <Route element={<Layout />}>
          ...
          <Route path="/about"          element={routeElement(<About />, 'about')} />
