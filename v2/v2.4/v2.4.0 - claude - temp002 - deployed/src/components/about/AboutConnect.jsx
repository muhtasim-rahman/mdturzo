// ============================================================
// AboutConnect.jsx — v2.3.6
// CHANGES (v2.3.6):
//   * Now fetches from Supabase `about_connect` table
//   * SOCIALS_FALLBACK (built from SITE_CONFIG -- always current) shown
//     instantly, swapped for live data if/when the fetch succeeds.
// PREVIOUS (v2.3.3):
...
import { getAboutConnect } from '../../services/supabase.js'
...
const SOCIALS_FALLBACK = [
  ...
  },
  {
    ...
    color: '#0C7C7C', bgColor: '#EA4335',
  },
]

export default function AboutConnect() {
  const [socials, setSocials] = useState(SOCIALS_FALLBACK)

  useEffect(() => {
    getAboutConnect()
      .then(rows => {
        if (!rows?.length) return
        setSocials(rows.map(r => ({
          svgSrc: r.svg_src, label: r.label, handle: r.handle, url: r.url,
          color: r.color, invertDark: r.invert_dark,
        })))
      })
      .catch(() => { /* keep fallback -- links must never disappear */ })
  }, [])
    ...
    <section className="section section-alt" id="about-connect">
      ...
      <style>{`
        ...
        .abc-card::after {
          ...
          background: color-mix(in srgb, var(--soc, var(--accent-primary)) 5%, transparent);
        ...
        .abc-card:hover {
          ...
          box-shadow: 0 8px 24px rgba(0,0,0,.12),
                      0 0 0 1px color-mix(in srgb, var(--soc, var(--accent-primary)) 28%, transparent);
          border-color: color-mix(in srgb, var(--soc, var(--accent-primary)) 40%, var(--border-color));
        }
        .abc-card:hover::before { transform: scaleY(1); }
        .abc-card:hover::after  { opacity: 1; }
        [data-theme=light] .abc-card:hover {
          box-shadow: 0 6px 18px rgba(0,0,0,.08),
                      0 0 0 1px color-mix(in srgb, var(--soc, var(--accent-primary)) 22%, transparent);
        }
        ...
        .abc-card:hover .abc-icon-wrap {
          background: color-mix(in srgb, var(--soc, #3B82F6) 15%, transparent);
        }
        .abc-svg-icon {
