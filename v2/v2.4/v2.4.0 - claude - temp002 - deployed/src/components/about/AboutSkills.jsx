// ============================================================
// AboutSkills.jsx — v2.3.6
// CHANGES (v2.3.6):
//   * Now fetches from Supabase `about_skills` (group_key: dev/design/video/tools)
//   * Each group keeps its own FALLBACK array -- shown instantly, swapped
//     for live data per-group if/when that group's rows are found.
// PREVIOUS (v2.3.2):
//   * Skills & Expertise section
//   * All 4 tabs use same 2-column panel layout (bars + info card)
//   * Progress bars animated in all tabs (same as programming tab)
// ============================================================
...
import { getAboutSkills } from '../../services/supabase.js'

// icon_key (Supabase) -> FontAwesome icon. Only the design/tools groups
// use icons -- dev/video rows just have a color + note.
const ICON_MAP = {
  palette: faPalette, camera: faCamera, handshake: faHandshake,
  globe: faGlobe, book: faBook, code: faCode,
  terminal: faTerminal, github: faGithub, gears: faGears, 'laptop-code': faLaptopCode,
}
const resolveIcon = (key) => ICON_MAP[key] ?? null

const DEV_SKILLS_FALLBACK = [
...
// Animated progress bar — same as home Skills pattern
...
export default function AboutSkills() {
  ...
  const [DEV_SKILLS,    setDevSkills]    = useState(DEV_SKILLS_FALLBACK)
  const [DESIGN_SKILLS, setDesignSkills] = useState(DESIGN_SKILLS_FALLBACK)
  const [VIDEO_SKILLS,  setVideoSkills]  = useState(VIDEO_SKILLS_FALLBACK)
  const [TOOLS_LIST,    setToolsList]    = useState(TOOLS_LIST_FALLBACK)
  const panelRef = useRef(null)
  const panelInView = useInView(panelRef, { once: true, margin: '-60px' })

  useEffect(() => {
    getAboutSkills() // all groups in one call, grouped client-side
      .then(rows => {
        if (!rows?.length) return
        const byGroup = (key) => rows.filter(r => r.group_key === key)
        const dev    = byGroup('dev')
        const design = byGroup('design')
        const video  = byGroup('video')
        const tools  = byGroup('tools')
        if (dev.length)    setDevSkills(dev.map(r => ({ name: r.name, pct: r.pct, color: r.color, note: r.note })))
        if (design.length) setDesignSkills(design.map(r => ({ name: r.name, pct: r.pct, color: r.color, icon: resolveIcon(r.icon_key) })))
        if (video.length)  setVideoSkills(video.map(r => ({ name: r.name, pct: r.pct, color: r.color })))
        if (tools.length)  setToolsList(tools.map(r => ({ name: r.name, color: r.color, icon: resolveIcon(r.icon_key) })))
      })
      .catch(() => { /* keep fallbacks -- never break this section */ })
  }, [])
  ...
  return (
    <section className="section section-alt" id="about-skills">
      <div className="container-xl">
        ...
        {/* Tab content — all tabs use same 2-col panel layout */}
      ...
      <style>{`
        ...
        /* Panel — universal 2-column layout */
