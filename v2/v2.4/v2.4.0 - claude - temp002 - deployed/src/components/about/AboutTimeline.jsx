// ============================================================
// AboutTimeline.jsx — v2.3.6
// CHANGES (v2.3.6):
//   * Now fetches from Supabase `about_timeline` table (getAboutTimeline)
//   * EDUCATION_FALLBACK kept as-is -- shown instantly, swapped for live
//     data if/when the fetch succeeds. Never blocks, never breaks.
// PREVIOUS (v2.3.5) FULL REDESIGN:
...
import { getAboutTimeline } from '../../services/supabase.js'

// icon_key (stored in Supabase) -> actual FontAwesome icon object.
// Keeping this map small & fixed means an admin panel can offer a
// dropdown of these exact keys later -- no arbitrary code in the DB.
const ICON_MAP = {
  school: faSchool, book: faBook, flask: faFlask,
  'graduation-cap': faGraduationCap, trophy: faTrophy,
  rocket: faRocket, atom: faAtom,
}
const resolveIcon = (key) => ICON_MAP[key] ?? faSchool

// Fallback -- shown instantly while Supabase loads, and used as-is if
// the about_timeline table is empty or unreachable. Same data that
// used to be the only source before v2.3.6.
...
function mapRow(row) {
  return {
    period: row.period, short: row.short_label, hover: row.hover_label,
    school: row.school, level: row.level, desc: row.description,
    color: row.color, icon: resolveIcon(row.icon_key),
    current: row.is_current, upcoming: row.is_upcoming,
  }
}
...
export default function AboutTimeline() {
  const [education, setEducation] = useState(EDUCATION_FALLBACK)
  const hasLoadedOnce = useRef(false)
  const [idx, setIdx] = useState(5) // matches EDUCATION_FALLBACK's "current" item
    ...
    getAboutTimeline()
      .then(rows => {
        ...
        if (!hasLoadedOnce.current) {
          const curIdx = mapped.findIndex(e => e.current)
          setIdx(curIdx >= 0 ? curIdx : Math.max(0, mapped.length - 2))
        }
      })
      .catch(() => { /* keep fallback -- never break this section */ })
