// ============================================================
// AboutGoals.jsx — v2.3.6
// CHANGES (v2.3.6):
//   * Now fetches from Supabase `about_goals` table (items stored as JSONB array)
//   * GOALS_FALLBACK shown instantly, swapped for live data if/when the
//     fetch succeeds.
//   * fadeUp/stagger inlined (was imported from the now-removed aboutData.js)
// PREVIOUS (v2.3.5):
//   * Goals: removed progress caption above bars; shimmer added
...
import { faCircleCheck, faFlag, faBullseye, faMountain } from '@fortawesome/free-solid-svg-icons'
import { getAboutGoals } from '../../services/supabase.js'
...
const ICON_MAP = { flag: faFlag, bullseye: faBullseye, mountain: faMountain }
const resolveIcon = (key) => ICON_MAP[key] ?? faFlag
...
export default function AboutGoals() {
  const [goals, setGoals] = useState(GOALS_FALLBACK)
  ...
  useEffect(() => {
    getAboutGoals()
      .then(rows => {
        ...
        setGoals(rows.map(r => ({
          period: r.period, subtitle: r.subtitle, color: r.color,
          icon: resolveIcon(r.icon_key), pct: r.pct, items: r.items ?? [],
        })))
      })
      .catch(() => { /* keep fallback */ })
  }, [])
