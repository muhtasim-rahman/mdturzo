...
// NAVBAR -- v2.3.6
// FIXES (v2.3.6):
//   * Mega menu backdrop: was `inset:0` (covered the ENTIRE screen
//     including the navbar itself, blocking nav clicks while the menu
//     was open) -- now starts at megaTopOffset so the navbar stays
//     interactive. Outside-click now reliably closes the menu, so the
//     old scroll-to-close listener was removed.
//   * DevBanner height tracked via ResizeObserver -- floating navbar +
//     mega menu (both `position:fixed`) now correctly offset below it.
//   * Auth-loading: tablet/mobile rows (both top + floating navbar) now
//     show a skeleton pill/circle while loading -- previously showed
//     nothing (a blank gap) until the auth state resolved.
// PREVIOUS (v2.2.8):
//   * NavRight: removed lg:hidden search icon + lg:hidden menu btn
//     (NavRight is only ever rendered inside hidden lg:flex containers)
//   * Mega nav: rendered ONCE at root level -- no more doubling
//     when both top navbar and float navbar are in DOM
//   * Mega nav outside-click: click handler now reliably closes
//   * nb-icon-btn: overflow changed to visible so CSS ::after
//     tooltips are no longer clipped -- title attr added as fallback
//   * IconBtn: title={label} added for native tooltip fallback
// ============================================================
...
// -- Right controls cluster (desktop only -- no menu/mobile search btn here)
function NavRight({ user, profile, isAdmin: isAdminProp, avatar, displayName, isLoggedIn, authLoading, unreadCount, openSearch, notifOpen, setNotifOpen, userOpen, setUserOpen, megaOpen, setMegaOpen }) {
  return (
    <div className="flex items-center gap-1.5 flex-shrink-0" data-nb-right>
      {/* Single search button -- no mobile duplicate here since NavRight is desktop-only */}
      <IconBtn icon={faSearch} onClick={openSearch} label="Search" />
      ...
      {/* NO menu button here -- NavRight is desktop-only, mobile handles its own menu */}
    </div>
  )
}
...
export function Navbar() {
  ...
  const isHomePage = ['/', '/home', '/about'].includes(location.pathname)

  useEffect(() => {
    const onScroll = () => {
      const scrolled = window.scrollY > FLOAT_THRESHOLD
      setFloatVisible(scrolled)
    }
    window.addEventListener('scroll', onScroll, { passive: true })
    return () => window.removeEventListener('scroll', onScroll)
  }, [])

  // v2.3.6: scroll-to-close removed — the backdrop fix below (spans from
  // megaTopOffset down, not the whole screen) means outside-click now
  // reliably closes the menu AND doesn't block navbar interactions while
  // it's open, so the scroll listener is no longer needed.

  // v2.3.6: track DevBanner's real height via ResizeObserver so the
  // floating navbar + mega menu (both `position: fixed`, so they ignore
  // normal document flow) can offset themselves below it. The top navbar
  // doesn't need this — it's contained inside Layout's relative wrapper,
  // which already sits below the banner in normal flow.
  const [devBannerH, setDevBannerH] = useState(0)
  useEffect(() => {
    const el = document.querySelector('.devb-wrap')
    if (!el || typeof ResizeObserver === 'undefined') return
    const obs = new ResizeObserver(entries => {
      setDevBannerH(Math.round(entries[0].contentRect.height))
    })
    obs.observe(el)
  ...
  // Click-outside handler for notifications and user dropdowns
  // (mega menu is closed by its own backdrop div)
  useEffect(() => {
    const h = (e) => {
  ...
  // Mega menu top offset: float nav bottom = 72px, top navbar = var(--navbar-h)
  // v2.3.6: + devBannerH, since this is `position:fixed` and ignores the
  // banner's space in normal document flow.
  const megaTopOffset = floatVisible
    ? `${72 + devBannerH}px`
    : `calc(var(--navbar-h) + ${devBannerH}px)`
    ...
    <>
      ...
      <nav
        ...
        <div className="navbar-inner flex items-center h-full gap-5">
          ...
          {/* Right -- desktop only (NavRight has no menu btn, no mobile search) */}
          <div className="hidden lg:flex ml-auto">
            <NavRight {...sharedProps} />
          </div>
          {/* Right -- tablet */}
          <div className="hidden md:flex lg:hidden items-center gap-1.5 ml-auto">
            {authLoading && !isLoggedIn && <div className="w-20 h-8 rounded-full sk" />}
            ...
            <button onClick={() => setMobileOpen(true)}
              className="nb-icon-btn" aria-label="Menu" title="Menu">
              <FontAwesomeIcon icon={faBars} />
            </button>
          </div>
          {/* Right -- mobile */}
          <div className="flex md:hidden items-center gap-1.5 ml-auto">
            {authLoading && !isLoggedIn && <div className="w-8 h-8 rounded-full sk flex-shrink-0" />}
      ...
      <AnimatePresence>
        {floatVisible && (
          <motion.div
            ...
            className="fixed left-0 right-0 z-[var(--z-sticky)] flex justify-center px-4 pt-3 pointer-events-none"
            style={{zIndex: 200, top: devBannerH}}>
      ...
      {/* == MEGA MENU -- single render, outside both navbars ===
          This prevents the double-render bug when both top + float navbars are in DOM.
          top offset switches between float nav bottom (72px) and top navbar height.
          v2.2.9: backdrop click on left/right empty space closes menu.
          v2.2.9: hidden when navbar is scrolled out of view (only show when float nav visible OR at top).
      */}
      <AnimatePresence>
        {megaOpen && (
          <>
            {/* Backdrop — clicking left/right empty space closes.
                v2.3.6 fix: was `inset: 0` (covered the ENTIRE screen,
                including the navbar itself, blocking clicks on nav
                buttons/links while the mega menu was open — even the
                toggle button that opened it). Now starts at megaTopOffset
                so the navbar above it stays fully interactive. */}
            <div
              key="mega-backdrop"
              onClick={() => setMegaOpen(false)}
              style={{
                position: 'fixed', top: megaTopOffset, left: 0, right: 0, bottom: 0,
                zIndex: 9999,
            ...
            <div
              key="mega-overlay"
              style={{ position:'fixed', top: megaTopOffset, left:0, right:0, zIndex:10000 }}>
      ...
      <style>{`
        ...
        /* v2.3.6 fix: removed the @media(min-width:1440px){padding-inline:0}
           override -- it squished content to the box edge on wide screens
           and didn't match the footer, causing visible misalignment. */
        ...
        /* == ICON BUTTONS ================================== */
        /* overflow: visible so ::after tooltip pseudo-elements show correctly */
        .nb-icon-btn {
          position: relative;
          width: 36px;
          height: 36px;
        ...
        .nb-icon-btn:active { transform: scale(0.9); }
        .nb-icon-btn--active {
        ...
        /* == TOOLTIP -- data-nb-tip (now visible since overflow:visible) */
        ...
        .float-nav-link:active { transform: scale(0.93) !important; }
        .float-nav-link--active {
          background: rgba(59,130,246,0.16) !important;
          border-color: rgba(59,130,246,0.30) !important;
          color: rgba(147,197,253,1) !important;
        }
        [data-theme="light"] .float-nav-link { color: rgba(30,41,59,0.70); }
        ...
        /* == MEGA MENU ====================================== */
        .mega-panel-wrap { /* no positioning needed -- parent uses fixed */ }
        .mega-glass-inner {
        ...
        .mega-item:active { transform: scale(0.96); }
        .mega-item--active {
          background: var(--accent-light);
          color: var(--accent-primary);
        }
        .mega-item--active svg,
        .mega-item--active span { color: var(--accent-primary) !important; }
        ...
        .mega-share-btn {
          ...
          box-shadow: 0 2px 8px rgba(59,130,246,0.28); border: none;
        }
        ...
        .mega-url-pill span {
          ...
          white-space: nowrap; font-size: 11px; color: var(--text-tertiary);
          font-family: var(--font-mono);
        }
        .mega-url-copy {
          width: 30px; height: 30px; border-radius: 999px;
          display: inline-flex; align-items: center; justify-content: center;
          color: var(--text-tertiary); transition: all 0.18s ease; flex-shrink: 0;
        }
        ...
        .mega-ver-badge strong { color:var(--text-primary); font-family:var(--font-mono); font-weight:500; margin-left:3px; }
