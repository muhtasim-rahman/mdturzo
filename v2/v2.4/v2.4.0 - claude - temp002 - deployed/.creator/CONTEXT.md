...
## v2.3.6 — Rollback Recovery: Global/Non-Project Polish, About→Supabase, Section Isolation (2026-06-27)

### Why this version exists
v2.4.1 onward introduced bugs that compounded across 2 major + 10+ minor
releases until v2.5.2, to the point where they were no longer practically
fixable forward. **Decision: roll the codebase back to v2.3.5** and redo
v2.4+ (Projects, Feed/Blogs/Posts) properly from a clean base. Before that
redo starts, this version (v2.3.6) cherry-picks every **global / non-project**
improvement that was actually good from the abandoned v2.4.x–v2.5.x attempts
— Navbar/Footer fixes, DevBanner, theme system, About page architecture,
Home page polish, error isolation — without touching anything Projects/Feed/
Blog/Post-shaped. Those get rebuilt properly starting at a fresh v2.4.0.

### Changes summary

**Per-section error isolation (new architecture, applies sitewide):**
- `components/ui/SectionErrorBoundary.jsx` — class-based error boundary,
  minimal/neutral fallback card (no red alarm colors), "Try again" button
  remounts just that section. A crash in one section can no longer take
  down the rest of the page.
- `components/ui/SectionReveal.jsx` — IntersectionObserver wrapper, shows
  a section-specific skeleton until scrolled near, then fades content in.
- `components/ui/Section.jsx` — combines both into one wrapper:
  `<Section name="..." skeleton={<X/>}>{children}</Section>`
- Applied to every section on Home.jsx and About.jsx (Hero/AboutHero excluded
  — those render immediately, above the fold).

**DevBanner (new, minimal/neutral — not amber/danger):**
- `components/ui/DevBanner.jsx` — thin info strip above the Navbar.
  Admin-configurable via `site_settings` (`dev_banner_enabled`,
  `dev_banner_text`, `dev_banner_progress`) with a safe hardcoded
  fallback if that fetch fails. Dismissible (3-day localStorage).
- `Layout.jsx` restructured: DevBanner sits above an inner
  `relative flex-1 flex flex-col` wrapper that contains Navbar/main/Footer
  — so the absolute-positioned Navbar (home/about) can never visually
  overlap the banner.
- `Navbar.jsx`: tracks the banner's real height via `ResizeObserver` on
  `.devb-wrap`, offsets the floating navbar's `top` and the mega menu's
  `megaTopOffset` accordingly (both are `position:fixed`, so they ignore
  normal document flow and need this explicitly).

**Navbar/Footer bug fixes (pre-existing in v2.3.5, found during this pass):**
- **Double-padding misalignment at ≥1440px**: `.navbar-inner` had
  `@media(min-width:1440px){padding-inline:0}` and `.nf-footer` had its
  OWN `padding:0 1.75rem` stacked on top of `.nf-inner`'s padding. Removed
  both — navbar and footer content now align identically at every width.
- **Mega menu backdrop blocked the navbar itself**: backdrop was
  `position:fixed; inset:0` — covered the ENTIRE screen including the
  navbar, so clicking the toggle button (or anything else in the navbar)
  while the mega menu was open didn't work. Now starts at `megaTopOffset`
  instead. The old scroll-to-close listener was removed since outside-click
  is now fully reliable.
- **Auth-loading skeleton gaps**: tablet/mobile navbar rows (both top +
  floating navbar) showed nothing while `authLoading` was true — desktop
  already had a skeleton circle. Added matching pill/circle skeletons to
  all 4 missing spots.

**Auth — no more avatar/name flash on reload:**
- `store/authStore.js`: added a localStorage profile cache (24h TTL,
  keyed by uid).
- `hooks/useAuth.js`: as soon as Firebase resolves the user, the cached
  profile (same uid) is applied immediately and `authLoading` flips false
  right away — avatar/name appear instantly instead of waiting on the
  Supabase round-trip. The real fetch still runs right after and silently
  corrects/refreshes the cache.

**Theme — default is now always light:**
- `store/themeStore.js`: `getSavedTheme()` default changed `'dark'` →
  `'light'`. Manual toggle / system-detect option unchanged.
- `index.html`: `data-theme` default changed to `"light"`, `theme-color`
  meta updated to match. Added a small synchronous pre-hydration `<script>`
  in `<head>` that resolves the real theme (saved pref, or system if pref
  is `'system'`, or light by default) and sets `data-theme` BEFORE first
  paint — fixes a pre-existing flash-of-wrong-theme bug for any returning
  visitor, not just the default case.
- `index.css`: `:root`'s bare fallback moved from the dark block to the
  light block (was `[data-theme="dark"], :root {...}` — meant a missing
  `data-theme` attribute fell back to DARK, the opposite of what we now
  want as the default).

**Home page:**
- `GithubActivity.jsx` (renamed from `GithubStats.jsx`): fixed the
  streak-stats URL still pointing at the long-dead
  `github-readme-streak-stats.herokuapp.com` → migrated to the maintained
  `streak-stats.demolab.com` fork. Fixed `sideNums`/`currStreakNum`/
  `sideLabels`/`dates` being hardcoded near-white regardless of theme
  (invisible text in light mode). Added `onError` fallback for both stat
  images so a failed external embed shows a small neutral placeholder
  instead of a broken-image icon.
- Renames to match actual on-page headings: `AboutMini.jsx`→`AboutSection.jsx`,
  `Process.jsx`→`WorkflowSetup.jsx`, `Testimonials.jsx`→`Reviews.jsx`.
- `RecentProjects.jsx`: **removed the hardcoded FALLBACK demo-project
  array entirely.** Home now shows ONLY real published+featured projects
  from Supabase; if there are none, a clean neutral empty state shows
  instead (with a "Try again" retry button if the fetch genuinely failed,
  vs. a softer message if it's just genuinely empty).
- `components/projects/ProjectCard.jsx` (new, shared): the project card
  markup/design extracted out of `RecentProjects.jsx` into its own file —
  same exact design, now a single source of truth that the Home page's
  Featured Projects AND the future Projects page (when that work resumes)
  both import. One fix/tweak there updates both places.
- `hooks/useSiteSettings.js` + `services/supabase.js`: added
  `getProjectCount()` (published+public count). `statsProjects` now
  prefers this LIVE count over the static `site_settings` value whenever
  the count fetch succeeds — Hero/Skills/AboutStory stats are no longer
  stuck at a manually-typed number.
- Dead file cleanup: `home/Stats.jsx`, `home/BlogMini.jsx`, `home/CTA.jsx`
  (a stale duplicate — `About.jsx` was still importing this one instead of
  `shared/SiteCTA.jsx`, now fixed), `home/RecentProjectsOrginal.jsx`,
  `layout/Navbar.jsx.patch`, `services/worker.js` (all confirmed unused
  anywhere in the codebase before deleting).

**About page → Supabase (biggest item this version):**
- 7 new tables: `about_timeline`, `about_skills`, `about_languages`,
  `about_values`, `about_hobbies`, `about_goals`, `about_connect`.
- `services/supabase.js`: matching `getAbout*()` fetchers, all ordered by
  `sort_order` (an admin panel can reorder content later with just an
  UPDATE, no code change).
- Every About component (`AboutTimeline`, `AboutSkills`, `AboutLanguages`,
  `AboutConnect`, `AboutValues`, `AboutGoals`) now fetches from Supabase
  on mount — but keeps its **exact current hardcoded data** as a
  `*_FALLBACK` constant, shown instantly and used as-is if the fetch fails
  or a table is empty. The About page can never break or go blank because
  of this migration.
- Icon fields (FontAwesome icon objects can't be stored in a DB row) use a
  small `icon_key` string column (e.g. `'school'`, `'mosque'`, `'github'`)
  mapped to the real icon via a fixed `ICON_MAP` in each component — ready
  for an admin panel dropdown later, no arbitrary code in the DB.
- `aboutData.js` deleted (its `EDUCATION`/`DEV_SKILLS`/`LANGUAGES`/`SOCIALS`
  exports were already stale/unused — each component had its own local,
  more current copy of that data, which is what got migrated). The
  `fadeUp`/`stagger` animation variants it exported (still actively used by
  `AboutHero`, `AboutValues`, `AboutGoals`) were inlined into those 3 files.
- `App.jsx`: `/about` given its own `'about'` RouteBoundary layout key
  (was incorrectly sharing `'profile'` with `/profile` and `/@:username` —
  giving About its own bespoke skeleton would have wrongly shown on those
  two other pages too).

**New layout-matching skeletons (not generic placeholders):**
- `components/skeletons/HomeSkeletons.jsx` — bespoke skeleton per Home
  section (Hero, AboutSection, Projects grid, Skills, Workflow, Services,
  Reviews, GitHub Activity, CTA), shaped to each section's real layout.
- `components/skeletons/AboutSkeletons.jsx` — same for every About section.
- Used both by `SectionReveal` (per-section, before scroll-into-view) and
  by `App.jsx`'s `RouteBoundary` (whole-page, while the route chunk is
  still loading) — defined once, reused both places.

### Explicitly NOT touched (deferred to the real v2.4.0/v2.5.0 redo)
Projects page/detail/cards (beyond the ProjectCard extraction above),
project search engine, project reviews/comments/likes/reports interaction
system, ImagePreviewModal, Carousel, RelatedProjectsRow, `/project/:slug`
canonical routing, the entire Feed/Blogs/Posts system, view-dedup tracking,
Projects/ProjectDetail skeletons (left on the generic `PageSkeleton` since
those pages are still the "coming in v2.4.0" placeholders).

### Supabase
- `supabase-hard-reset.sql` — wipes the entire `public` schema clean
  (tables/functions/views/types/sequences), regardless of current state.
- `supabase-master-v2.3.6.sql` — full rebuild: the 18-table v2.0 foundation
  (reconstructed from what the codebase actually queries, since the
  original `supabase-schema-v2.0.0.sql` wasn't available this round) + the
  7 new `about_*` tables + RLS policies + indexes + seed data (site_settings
  defaults + the exact About page content above, so it looks identical
  pre/post migration). Both scripts were actually executed against a real
  local Postgres 16 instance during this session (reset → master → reset
  → master, full cycle) to confirm they run clean with zero errors before
  being handed off — not just visually reviewed.
- Both files live in `.creator/supabase/`.

### Housekeeping
- `creator-guide/` folder renamed to `.creator/`; `master-context.md`
  renamed to `CONTEXT.md` (this file).
- `site.config.js` + `package.json` version bumped to `2.3.6`.
- Full `npm run build` (Vite) run against the final code before packaging
  — 1096 modules transformed, zero errors.

### Files changed
- `src/components/ui/SectionErrorBoundary.jsx` — new
- `src/components/ui/SectionReveal.jsx` — new
- `src/components/ui/Section.jsx` — new
- `src/components/ui/DevBanner.jsx` — new
- `src/components/skeletons/HomeSkeletons.jsx` — new
- `src/components/skeletons/AboutSkeletons.jsx` — new
- `src/components/projects/ProjectCard.jsx` — new (extracted from RecentProjects.jsx)
- `src/components/layout/Layout.jsx` — DevBanner + relative wrapper restructure
- `src/components/layout/Navbar.jsx` — mega backdrop fix, devBannerH tracking, auth-loading skeletons
- `src/components/layout/Footer.jsx` — double-padding fix
- `src/store/authStore.js` — localStorage profile cache
- `src/store/themeStore.js` — default light
- `src/hooks/useAuth.js` — optimistic cached profile on auth resolve
- `src/hooks/useSiteSettings.js` — live project count
- `src/services/supabase.js` — getProjectCount + 7 getAbout*() fetchers
- `src/pages/Home.jsx` — renamed imports, Section wrapping
- `src/pages/About.jsx` — fixed CTA import, Section wrapping, own RouteBoundary layout key
- `src/App.jsx` — RouteBoundary uses HomeSkeleton/AboutSkeleton, `/about` layout key
- `src/components/home/RecentProjects.jsx` — no demo data, shared ProjectCard, retry-aware empty state
- `src/components/home/GithubStats.jsx` → `GithubActivity.jsx` — renamed + 2 bug fixes + onError fallback
- `src/components/home/AboutMini.jsx` → `AboutSection.jsx` — renamed
- `src/components/home/Process.jsx` → `WorkflowSetup.jsx` — renamed
- `src/components/home/Testimonials.jsx` → `Reviews.jsx` — renamed
- `src/components/about/AboutHero.jsx` — fadeUp/stagger inlined
- `src/components/about/AboutTimeline.jsx` — Supabase fetch + fallback
- `src/components/about/AboutSkills.jsx` — Supabase fetch + fallback
- `src/components/about/AboutLanguages.jsx` — Supabase fetch + fallback
- `src/components/about/AboutConnect.jsx` — Supabase fetch + fallback
- `src/components/about/AboutValues.jsx` — Supabase fetch + fallback, fadeUp/stagger inlined
- `src/components/about/AboutGoals.jsx` — Supabase fetch + fallback, fadeUp/stagger inlined
- `src/components/about/aboutData.js` — deleted (dead/stale, superseded by per-component fallbacks)
- `src/components/home/Stats.jsx`, `BlogMini.jsx`, `CTA.jsx`, `RecentProjectsOrginal.jsx` — deleted (unused dead code)
- `src/components/layout/Navbar.jsx.patch`, `src/services/worker.js` — deleted (unused dead code)
- `index.html` — theme default + pre-hydration script
- `src/index.css` — theme cascade fix, double-padding fix
- `site.config.js`, `package.json` — version `2.3.6`
- `.creator/supabase/supabase-hard-reset.sql` — new
- `.creator/supabase/supabase-master-v2.3.6.sql` — new
- `creator-guide/master-context.md` → `.creator/CONTEXT.md` — renamed + this entry
...
## v2.4.0 — Projects System (Full Build)
**Date:** June 2026 | **Status:** Complete

### What was built
Full projects system from scratch — listing page, detail page, and 6 shared interaction components.

### New pages
| Page | Path | Notes |
|---|---|---|
| Projects | `/projects` | Grid/List, search, category filters, sort, load-more |
| ProjectDetail | `/projects/:slug` | 2-col layout, HTML render, gallery, all interactions |

### New shared components (`src/components/shared/`)
| Component | Purpose | Reusable for |
|---|---|---|
| `ShareModal.jsx` | YouTube-style share popup (8 platforms + QR + embed) | Any page |
| `CommentsSection.jsx` | Nested 2-level comments, likes, sort, auth redirect | Any content |
| `LikeDislikeBar.jsx` | Toggle like/dislike with optimistic UI | Any content |
| `RatingWidget.jsx` | 5-star rating with distribution | Projects only |
| `ReportModal.jsx` | 6-reason report dialog | Any content |
| `ViewTracker.jsx` | Invisible view dedup (3-day cooldown) | Any content |

### Auth redirect pattern
When unauthenticated user tries to interact:
```js
navigate(`/login?redirect=${encodeURIComponent(location.pathname + '#section')}`)
// After login: navigate(redirect || '/')
// Browser auto-scrolls to #section via native hash behavior
```
Sections: `#comments`, `#rating`, `#interactions`

### Database changes (run `.creator/v2.4/supabase-v2.4.0.sql`)
- `projects` table: +48 columns (total ~65). Key additions:
  - JSONB: `features_list`, `tech_stack_detail`, `screenshots`, `timeline_events`, `key_achievements`, `custom_sections`
  - Counters: `likes_count`, `dislikes_count`, `comments_count`, `shares_count`, `rating_count`, `rating_avg`
  - Layout: `detail_layout`, `card_style`, `accent_color`, `gradient_from/to`
  - Toggles: `comments_enabled`, `likes_enabled`, `shares_enabled`, `ratings_enabled`, `report_enabled`
  - SEO: `seo_title`, `seo_description`, `seo_keywords`
- NEW: `project_ratings` (1 per user per project, triggers recalc avg)
- NEW: `project_views` (dedup tracking, used by `track_project_view` RPC)
- NEW: `comment_likes` (thumbs on individual comments)
...
- 19 projects seeded from `projects-v2.md`

### Important notes for next AI session
- Rating is PROJECTS-ONLY. Home page reviews section is separate (site-wide testimonials, v2.10.0).
- `CommentsSection` uses `content_type='project'` and `content_id=slug` (not numeric ID).
- `ShareModal` calls `increment_share_count(slug)` once per modal-open (not per platform click).
- `ViewTracker` runs on mount with `slug` prop. guest key stored as `mdturzo_vk` in localStorage.
- `full_description_html` renders via `dangerouslySetInnerHTML` inside `.proj-html` class which has scoped website CSS applied.
- `supabase.js` now has 2 separate project fetch functions: `getPublishedProjects()` (old, used by Home page featured) and `getPublishedProjectsV2()` (new, used by Projects page).
- `useProjects.js` hook handles all Projects page state; do not lift state into the page.
- `comment_likes` is separate from `likes` table. `likes` table handles project/feed likes. `comment_likes` handles individual comment reactions.
- `auto_approve_comments` per-project flag: if true, comments insert as `status='approved'` directly (implement in `submitComment` service function when building auth in v2.7.0).

### Deferred to future versions
- Feed/Blogs/Posts system → v2.5.0
- Auth pages full build (login/signup forms, email verify) → v2.7.0
- Comment edit/delete (needs auth UI) → v2.7.0
- Auto-approve comments flag enforcement → v2.7.0
- Admin panel → v2.10.0
- Contact page → v2.6.0
