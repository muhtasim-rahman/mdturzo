...
export async function getPublishedProjects({ limit, offset = 0, category, tag } = {}) {
  let query = supabase
    .from('projects')
    .select('id, slug, title, short_description, thumbnail_url, github_link, live_link, pdf_link, tags, category, status, is_featured, featured_order, views_count, likes_count, dislikes_count, created_at, updated_at')
    .eq('status', 'published')
