...
### Added
- **Projects page** — full list with hero, sticky search/filter/sort/view-toggle, active filter chips, result count, load-more pagination, empty state
- **ProjectDetail page** — thumbnail hero, metadata, all links (GitHub/Live/PDF/Custom), like/dislike, share, comments, related projects
- **ProjectCard component** — grid + list variants, shared between /projects and home featured section, compact mode for related
- **RecentProjects (home) — fully dynamic** — reads `is_featured=true, featured_order ASC` from Supabase; hides if empty (no hardcoded fallback)
- **LikeDislike** — optimistic updates, vote toggle, auth guard, animated counter
- **ShareButtons** — full bar + compact popup, Facebook/X/LinkedIn/WhatsApp/Telegram/Copy
- **Breadcrumb** — reusable nav trail component
- **CommentSection** — auth-aware, 1000 char limit, 10/day rate limit, pending approval flow
- **useProjects hook** — single-fetch + client-side `useFilteredProjects`
- **Supabase functions** — getUserLikeStatus, toggleLike, submitComment, trackContentView, getRelatedProjects, getFeaturedCount
- **Supabase RPCs** — `increment_count`, `decrement_count` for atomic counter updates
- **creator-guide** — reference files now versioned and committed: supabase-schema-v2.0.0.sql, cloudflare-worker-v2.0.0.js, firebase-rtdb-rules-v2.0.0.json, supabase-v2.4.0.sql

### Changed
- `formatters.js` — added `formatRelativeTime` alias for `timeAgo`
- `site.config.js` — version v2.4.0
- `package.json` — version 2.4.0
- `index.css` — large v2.4.0 block: all project card, project list, detail page, filter bar CSS

### Infrastructure
- `creator-guide/` folder convention established: all reference files (SQL, worker, RTDB rules) stored here with version tags; future AIs should read all files in this folder at session start
