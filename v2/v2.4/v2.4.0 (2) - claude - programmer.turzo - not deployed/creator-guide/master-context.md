...
## v2.4.0 — Projects Page: Dynamic List, Detail, Featured Home Section (2026-05-31)

### Summary
Full v2.4.0: Projects page with search/filter/grid-list toggle, ProjectDetail page with likes/comments/share/related, dynamic featured projects on home page from Supabase, shared components (LikeDislike, ShareButtons, Breadcrumb, CommentSection), new Supabase RPC functions for atomic counter updates.

### What was built

**Projects.jsx (full rewrite):**
- Hero section with project count from Supabase, stats (total/years/collab status)
- Sticky filters bar: search (debounced 280ms), category pills, tags dropdown (multi-select), sort (newest/oldest/views), grid/list toggle
- `useProjects()` hook: loads all published projects once, `useFilteredProjects()` does client-side filtering/sort
- Active filter chips show what's applied, with individual removal + "Clear all"
- Result count updates live
- Grid view: 3-col desktop → 2-col tablet → 1-col mobile
- List view: horizontal card rows with thumbnail + meta
- `PAGE_SIZE=9` load-more pagination (no API calls per page)
- Empty state with context-aware message
- Bottom CTA → Contact page
- VisibilityGuard wrapped

**ProjectDetail.jsx (full build):**
- Loads by slug from Supabase, 404 handling, error state
- `trackContentView()` called on load (session-deduplicated)
- Hero: left=meta (category, title, description, dates, views, tags, links, like+share), right=thumbnail
- Hero gradient uses project's accent color via `getAccentColor()`
- All project links: GitHub, Live, PDF, Custom (only shown if set)
- Content section: TipTap HTML rendered in `.pjd-prose` with full heading/code/image styles
- Sticky sidebar (desktop): links + tags + share
- Bottom reactions: LikeDislike (lg size) + ShareButtons
- CommentSection component
- RelatedProjects: same category, excludes current, max 3
- Back to /projects link
- Full Helmet SEO meta including og:image from thumbnail

**ProjectCard.jsx (new shared component):**
- `variant="grid"` (default): 160px thumbnail, tags, title, desc, meta row (views/likes/date), themed footer
- `variant="list"`: horizontal layout, left accent line, thumbnail, full meta
- `compact` prop: hides meta row (for related/mini contexts)
- `getAccentColor(project)` exported — used by ProjectDetail hero
- Full-card Link overlay at z-index 1; external links at z-index 2 (stop propagation)
- Card hover: border color = accent, translateY(-3px), box-shadow

**RecentProjects.jsx (complete rewrite):**
- 100% dynamic from Supabase `getFeaturedProjects()`
- Fetches `is_featured=true` ordered by `featured_order ASC` limit 6
- If Supabase returns 0 items → section is hidden entirely (no fallback hardcoded data)
- Grid adapts: 6 items=3col, 4 items=2col, 3 items=3col
- Mobile shows max 3; tablet shows max 4
- Uses same `ProjectCard` component as /projects page
- 500ms minimum skeleton display time

**LikeDislike.jsx (new shared component):**
- Reads user's current vote from Supabase on mount (if logged in)
- Optimistic updates: UI changes instantly, rolls back on error
- `toggleLike()` in supabase.js handles add/remove/switch
- Animated counter (AnimatePresence mode="popLayout")
- Login toast if unauthenticated

**ShareButtons.jsx (new shared component):**
- Full bar and compact (popup toggle) variants
- Platforms: Facebook, X/Twitter, LinkedIn, WhatsApp, Telegram, Copy Link
- `buildShareUrl()` generates correct share URLs per platform
- Copied state with checkmark icon + success toast

**Breadcrumb.jsx (new shared component):**
- Simple ol/li with chevron separators
- House icon on home crumb
- Framer Motion fade-in

**CommentSection.jsx (new shared component):**
- Loads approved comments from Supabase
- Auth-aware: shows login prompt if not signed in
- 1000 char limit with live counter
- Rate limit: 10 comments/day per user (checked against comments table by user_id + created_at window)
- On submit: pending status (awaiting admin approval per site_settings.comment_auto_approve)
- Success toast: "pending review" message
- Error toast with rate-limit detection

**useProjects.js (new hook):**
- `useProjects()`: single fetch of all published projects, 500ms min skeleton
- `useFilteredProjects()`: pure useMemo transform — search (title/desc/tags/category), category, tags (must match ALL), sort (newest/oldest/views)

**supabase.js additions:**
- `getUserLikeStatus(contentType, contentId, userId)` → 'like'|'dislike'|null
- `toggleLike(contentType, contentId, userId, type)` → add/remove/switch + counter update via RPC
- `submitComment({contentType, contentId, contentSlug, userId, text})` → 10/day rate limit
- `trackContentView(contentType, contentId)` → session-deduplicated view tracking via sessionStorage
- `getRelatedProjects(category, excludeId, limit)` → same category, different id
- `getFeaturedCount()` → count of is_featured=true (for admin max-6 enforcement)

**formatters.js:**
- Added `formatRelativeTime` alias for `timeAgo` (used in CommentSection)

**index.css additions (v2.4.0 block):**
- `.pjc-grid`, `.pjc-list` card styles (grid + list variants)
- `.pj-hero-*` projects page hero
- `.pj-filters-bar`, `.pj-fb-row1/2/3` sticky filters
- `.pj-search-*`, `.pj-sort-sel`, `.pj-view-toggle`, `.pj-cat-pill`, `.pj-tags-*`
- `.pj-af-*` active filter chips
- `.pj-grid`, `.pj-list-view`, `.pj-empty`, `.pj-load-*`, `.pj-cta-*`
- `.pjd-hero-*`, `.pjd-content-grid`, `.pjd-prose` (full rich text styles)
- `.pjd-sidebar-*`, `.pjd-link-btn`, `.pjd-reactions`, `.pjd-bottom-reactions`
- `.pjd-related-grid`, `.pjd-back-btn`

**supabase-v2.4.0.sql (creator-guide):**
- `increment_count(tbl, row_id, col)` RPC — atomic counter increment
- `decrement_count(tbl, row_id, col)` RPC — atomic counter decrement (floor 0)
- Ensures `is_featured` + `featured_order` columns on projects
- Ensures `content_slug` column on comments
- New index `idx_projects_featured_v240`
- Optional seed data (commented out)

### creator-guide folder convention (NEW — for all future AIs)
The `creator-guide/` folder contains important reference files that persist across versions:
- Files are named `{type}-v{version}.{ext}` — e.g. `supabase-schema-v2.0.0.sql`
- The full schema baseline is `supabase-schema-v2.0.0.sql` (run once to create all tables)
- Each version's new SQL changes go in `supabase-v{version}.sql`
- `cloudflare-worker-v2.0.0.js` — the Cloudflare Worker code (update manually on changes)
- `firebase-rtdb-rules-v2.0.0.json` — Firebase RTDB security rules
- `master-context.md` — this file, updated every version
- Any new version-specific files the user manually adds will also live here
- Future AIs should read ALL files in this folder at the start of each session

### Files changed
- `src/pages/Projects.jsx` — full rewrite
- `src/pages/ProjectDetail.jsx` — full build
- `src/components/projects/ProjectCard.jsx` — new
- `src/components/home/RecentProjects.jsx` — full rewrite
- `src/components/shared/LikeDislike.jsx` — new
- `src/components/shared/ShareButtons.jsx` — new
- `src/components/shared/Breadcrumb.jsx` — new
- `src/components/shared/CommentSection.jsx` — new
- `src/hooks/useProjects.js` — new
- `src/services/supabase.js` — new functions appended
- `src/utils/formatters.js` — formatRelativeTime alias
- `src/index.css` — v2.4.0 block appended
- `src/config/site.config.js` — version v2.4.0
- `package.json` — version 2.4.0
- `creator-guide/supabase-v2.4.0.sql` — new
- `creator-guide/supabase-schema-v2.0.0.sql` — added to repo
- `creator-guide/cloudflare-worker-v2.0.0.js` — added to repo
- `creator-guide/firebase-rtdb-rules-v2.0.0.json` — added to repo
- `creator-guide/master-context.md` — this update

### Decisions & notes
- `RecentProjects` fallback data REMOVED intentionally — if Supabase has no featured projects, section hides. Admin must mark projects as featured. This makes the home page truly dynamic.
- Comment rate limit is 10/day (per master prompt note) — checked client-side by querying comments table; actual enforcement should also be server-side via Supabase trigger or Cloudflare Worker in future.
- `toggleLike` updates count columns via `increment_count`/`decrement_count` RPC. These are SECURITY DEFINER functions so they bypass RLS and run atomically.
- ProjectDetail shows the full content TipTap HTML via `dangerouslySetInnerHTML`. This is safe because content is admin-authored.
- `trackContentView` uses `sessionStorage` to prevent counting the same user twice per session. Does NOT use IP (privacy-friendly).
- `getAccentColor()` is exported from ProjectCard.jsx and also used in ProjectDetail.jsx hero — single source of truth for category color mapping.
- Old `RecentProjectsOrginal.jsx` (with typo) still exists in the zip from v2.3.5; it's a legacy file, can be removed in a future cleanup pass.
- Blogs/Posts → Feed page is still placeholder (v2.5.0). The master prompt notes say blogs+posts are merged into "Feed" — `src/pages/Feed.jsx` exists from v2.0.0.

### Next version (v2.5.0)
- Feed page (combined blogs + posts): list view, pinned, search, filter, type toggle
- Feed detail page: reading progress bar, table of contents, like/dislike, comments, share, related
- Both use same `CommentSection`, `LikeDislike`, `ShareButtons` shared components
