...
## v2.4.2 — Project System Major Overhaul

**Release date:** June 2026
**Theme:** Advanced project card redesign, rich detail page, advanced search engine, Firebase interaction tracking, new Supabase schema

### Summary of Changes

#### 🃏 ProjectCard.jsx — Complete Redesign
- Uniform `--accent-primary` theme for all cards (no per-category color theming)
- Thumbnail dark gradient overlay (bottom) + category badge always visible (top-left)
- GitHub/Live action icons animate in on hover (top-right), hidden at rest
- Tags displayed in single horizontal line below title (`overflow-hidden`, `whitespace-nowrap`)
- Card-wide hover: `translateY(-3px) scale(1.006)` + border-color → accent-primary
- **Title does NOT change color on hover** — removed all `group-hover:text-[accent]` on title
- Click press effect: `active:scale(0.99)` on `.proj-card`
- No-thumbnail placeholder: centered icon + category pill on gradient bg
- List view completely redesigned: horizontal layout, proper thumbnail, category label, tags, hover icons

#### 🏠 RecentProjects (Home) — Grid Responsive
- `.rp-grid`: 3 columns PC (show all 6), 2 cols tablet (CSS `nth-child(n+5) { display:none }`), 1 col mobile (CSS `nth-child(n+4) { display:none }`)
- Net result: PC=6 cards, Tablet=4 cards, Mobile=3 cards

#### 🔍 Projects Page — Header + Search + Dropdown
- Header: gradient text on "Projects Catalog" heading (`.projects-heading-gradient`), accent icon pill
- Sort dropdown: custom styled with `bg-[--bg-surface]` + border + rounded-xl, `color-scheme` for native option theming
- Search bar: debounced (280ms), clear button always works, `appliedSearch` vs `searchInput` split
- Category scroll: left/right arrow buttons on desktop, touch-scrollable on mobile
- When searching: sort dropdown hidden → replaced with "By relevance" badge
- Responsive page size: PC=12, Tablet=8, Mobile=6

#### 🔎 projectSearch.js — NEW Advanced Search Engine (`src/services/projectSearch.js`)
- Completely separate file, used by Projects.jsx
- Multi-word query support (up to 8 terms, scored independently then summed)
- Scoring weights by field: title=120/60, slug=40, tags=35/tag, tagline=30, category=25, short_desc=22, detailed_desc=15, content(HTML stripped)=12, tech_stack=10, key_features=10, languages=8, dependencies=7, platform=6, version=5, notes=5, links=4, meta_data=3
- Results sorted by descending score, score=0 items excluded
- Exported: `searchProjects(projects, query)`, `getProjectScore(project, query)`

#### 📄 ProjectDetail.jsx — Complete Redesign
- **2-column layout** on PC (`grid-cols-[1fr_290px]`), single column on mobile/tablet
- **Left column:** title + tagline, short_description, tags row (clickable links), action buttons (Live/GitHub/Demo/PDF/Custom), engagement row (LikeDislike + ReportButton + views), share panel (collapsible toggle with 6 platforms + copy + native share), detailed_description or content HTML, mobile related projects, comments
- **Right sidebar (sticky):** MetaInfoCard (category, type, version, platform, team, role, license, views, published, development_status, complexity badge, is_open_source), timeline card, TechStackCard (handles both JSONB object groups and flat array), FeaturesCard (key_features checklist), TagsCard (clickable links → search), desktop related projects
- **Breadcrumb fix:** title passed through `bcLabel` truncation (38 chars max + ellipsis) — also Breadcrumb.jsx redesigned with proper `overflow-hidden` + `truncate` and `title` tooltip attribute
- **OG meta:** `og_image_url || thumbnail_url || SITE_CONFIG.seo.defaultOGImage` used for all OG/Twitter image meta tags — link share previews now show project thumbnail
- **Firebase view tracking:** `trackInteractionView('project', project.id)` called on load
- **Content images:** clickable → opens ImagePreviewModal with all `<img>` tags from content
- **JSON-LD:** SoftwareApplication schema added

#### 🖼️ ImagePreviewModal.jsx — Improved
- **Scroll wheel zoom:** `onWheel` handler, delta-based zoom ±0.15, clamped 0.5x–4x
- **Drag to pan** when zoomed (`isDragging` state, `cursor: grab/grabbing`)
- **Double-click** resets zoom/pan to 1x
- **Pinch-to-zoom** via `onTouchStart`/`onTouchMove` — calculates 2-finger distance ratio
- **Keyboard:** Esc=close, ←/→=navigate, +/-=zoom
- **Zoom indicator** in header shows current percentage
- **Bottom buttons in 2 groups:**
  - Group 1 (image): "Copy Image" (clipboard write as blob), "Image URL" (copy URL text)
  - Group 2 (project): "Project URL" (copy page URL), "Share" (navigator.share or fallback)
  - Both groups have rounded-xl container with subtle bg
  - Green checkmark + "Copied!" feedback per button

#### 🔥 Firebase RTDB — Interaction Tracking
- New functions in `firebase.js`: `trackInteractionView`, `trackInteractionLike`, `trackInteractionDislike`, `trackInteractionComment`, `listenToInteractions`
- RTDB path: `/interactions/{type}/{contentId}/{ views | likes | dislikes | comments }`
- New rules file: `creator-guide/firebase-rtdb-rules-v2.4.2.json` (adds `interactions` node with read:true/write:true for counts)

#### 🗄️ Supabase Schema — v2.4.2 (`creator-guide/supabase-schema-v2.4.2.sql`)
- **Drop & recreate** projects table (delete old table first in Supabase dashboard)
- **53 columns total** — see full list in schema file
- New columns vs v2.4.1: `short_name`, `tagline`, `detailed_description`, `banner_url`, `preview_gif_url`, `og_image_url`, `screenshots[]`, `custom_link`, `demo_link`, `subcategory`, `type`, `visibility`, `development_status`, `languages[]`, `tech_stack JSONB`, `key_features[]`, `dependencies[]`, `platform`, `version`, `start_date`, `end_date`, `project_timeline`, `complexity_level`, `team_size`, `role`, `license`, `is_open_source`, `seo_title`, `seo_description`, `seo_keywords[]`, `notes`, `awards JSONB`, `changelog JSONB`, `related_slugs[]`, `meta_data JSONB`, `accent_color`, `shares_count`, `published_at`
- GIN index on `tags[]`
- New RPC functions: `increment_likes_count`, `update_comment_count`

#### 📊 Rich CSV (`creator-guide/projects-v2.4.2.csv`)
- All 19 projects with complete data for all 53 columns
- Projects: linkivo, qr-prism, project-exporter-pro, ufmt-ssc26, notification-panel, sgsc-web-campus, mdturzo-portfolio, halal-the-world-of-muslim, odoo-website-assets, web-templates-2023, turzo-express, master-chef, study-with-muhtasim, portfolio-2023, basic-cv, css-calculator-design, github-profile-readme, iftar-mahfil-banner-1447, golden-days-poster, turzo-odoo-legacy

#### 📋 AI Rules File (`creator-guide/project-csv-rules-v2.4.2.md`)
- Complete field-by-field specification for generating project CSV entries
- Array format, JSONB format, enum values, quality checklist

#### 🎨 CSS (`src/index.css`)
- `.projects-heading-gradient` — gradient heading for projects page
- `select option` dark mode theming with `color-scheme`
- `.scrollbar-none` utility
- `.proj-list-card` hover: `translateX(3px)`, border accent
- `.proj-card:hover` — updated: `translateY(-3px) scale(1.006)`, accent border, no title color change
- `.rp-grid` — 3 cols + nth-child hiding for tablet/mobile
- `.prose-content img` — zoom cursor + hover scale for content images

### Files Changed
```
src/services/projectSearch.js            ← NEW
src/components/projects/ProjectCard.jsx  ← REDESIGN
src/pages/Projects.jsx                   ← REDESIGN (header + search + dropdown)
src/pages/ProjectDetail.jsx              ← REDESIGN (2-col layout, all data)
src/components/shared/Breadcrumb.jsx     ← FIX truncation
src/components/shared/ImagePreviewModal.jsx ← IMPROVE (zoom + buttons)
src/services/supabase.js                 ← getPublishedProjects wider select
src/services/firebase.js                 ← interaction tracking functions
src/index.css                            ← grid/card CSS updates
src/config/site.config.js                ← v2.4.2
package.json                             ← v2.4.2
creator-guide/supabase-schema-v2.4.2.sql ← NEW 53-col schema
creator-guide/projects-v2.4.2.csv       ← NEW 19-project rich CSV
creator-guide/project-csv-rules-v2.4.2.md ← NEW AI field rules
creator-guide/firebase-rtdb-rules-v2.4.2.json ← NEW interactions node
creator-guide/master-context.md          ← THIS FILE (v2.4.2 section added)
```

### Deployment Notes
1. **Supabase:** Drop old `projects` table, run `supabase-schema-v2.4.2.sql`, import `projects-v2.4.2.csv`
2. **Firebase RTDB:** Update rules using `firebase-rtdb-rules-v2.4.2.json`
3. `getPublishedProjects` now selects 37 fields (no `content` or `screenshots` in list query for performance)
4. `projectSearch.js` handles all client-side searching — no Supabase FTS dependency
5. `ImagePreviewModal` receives `projectUrl` prop from ProjectDetail — ensure it's passed as `pageUrl`
