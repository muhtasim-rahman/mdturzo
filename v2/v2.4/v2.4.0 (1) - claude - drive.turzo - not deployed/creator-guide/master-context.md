...
## v2.4.0 — Projects Page (2026-05-31)

### Summary
Full Projects page and Project Detail page. 100% dynamic from Supabase. No hardcoded fallback data anywhere.

### Changes

**New Pages:**

**`Projects.jsx` — full projects listing:**
- Hero section: title, badge, animated project count badge
- Sticky controls bar (top: navbar-h): search input (debounced 350ms), category filter chips (scrollable), grid/list view toggle
- URL-synced filters: `?q=search&cat=category` via useSearchParams
- Grid view: 3-col → 2-col → 1-col responsive
- List view: horizontal row layout with thumbnail, meta, links
- Load More pagination (PAGE_SIZE = 9, shows +9 on click)
- Skeleton loading (grid: SkeletonProjectCard, list: SkeletonProjectRow)
- Empty state (no results), Error state (with retry)
- Active filter pills with individual clear buttons
- result count display ("X results found")

**`ProjectDetail.jsx` — project detail:**
- Helmet: full SEO (title, description, OG tags, canonical, JSON-LD SoftwareApplication schema)
- Breadcrumb: Home > Projects > [Title]
- Hero thumbnail (full width, max-height 420px, gradient glow underneath)
- Header: category badge, views + date meta, h1 title, short description, tags row, action link buttons (GitHub, Live Demo, PDF, More)
- Two-column layout (1fr + 280px sidebar): content area + sidebar
- Content renders TipTap HTML with full tiptap-render CSS (h1-h4, p, ul, ol, code, pre, blockquote, img, hr, strong)
- Interaction bar: LikeDislike + ShareButtons + ReportButton
- CommentSection (login + email verify required, 10/day limit)
- Sidebar: Project Info card (category, dates, views, likes), Links card, Tags card, Back button
- Related projects (3 cards, same category, from Supabase)
- Views auto-increment via `incrementProjectViews()` on page load (session dedup via sessionStorage)
- Mobile: sidebar goes above content

**Updated `RecentProjects.jsx` (home page):**
- Removed all hardcoded FALLBACK data array
- Shows skeleton while loading
- Empty state with "Browse all projects" link if no featured projects in Supabase
- Error state with retry button
- Uses `is_featured: true` (correct column name per schema)
- Uses `accent_color` field from project row (new in v2.4.0)

**New shared components:**
- `src/components/shared/Breadcrumb.jsx` — reusable breadcrumb nav
- `src/components/shared/LikeDislike.jsx` — like/dislike with optimistic updates, auth required
- `src/components/shared/ShareButtons.jsx` — Facebook, X/Twitter, LinkedIn, WhatsApp, Telegram + copy link
- `src/components/shared/ReportButton.jsx` — report modal with reason select
- `src/components/shared/CommentSection.jsx` — full comment system (list + submit)

**Updated `src/services/supabase.js`:**
- Fixed `getFeaturedProjects()` — uses `is_featured: true` (was accidentally changed, now correct)
- Added `getProjectCategories()` — unique categories for filter chips
- Added `countPublishedProjects()` — total count for pagination
- Added `incrementProjectViews()` — calls Supabase RPC with sessionStorage dedup
- Added `getRelatedProjects()` — same category, exclude current slug
- Added `getUserVote()` — get current user's like/dislike for a content item
- Added `toggleLike()` — insert/update/delete like, returns fresh stats
- Added `submitComment()` — insert new comment (pending), returns data
- Added `checkCommentRateLimit()` — 10/day per user check
- Added `submitReport()` — insert report
- Updated `getPublishedProjects()` — added `search` (ilike title+description), `orderBy` params

**Updated `src/utils/formatters.js`:**
- Added `formatDistanceToNow()` — relative time ("2h ago", "3d ago")
- Added `formatDateShort()` — "Jan 5, 2024" format
- Kept existing functions

**Updated `src/config/site.config.js`:**
- `version: 'v2.4.0'`

### Supabase changes (run `supabase-v2.4.0.sql`)
- `projects.accent_color TEXT DEFAULT '#3B82F6'` — new column
- `feed.accent_color TEXT DEFAULT '#3B82F6'` — new column
- `increment_project_views(UUID)` — new RPC function
- `increment_feed_views(UUID)` — new RPC function (prep for v2.5.0)
- Fixed index name `idx_projects_is_featured`
- `user_comment_counts_today` view (helper)

### creator-guide folder convention (IMPORTANT for all future AIs)
The `creator-guide/` folder stores ALL non-deployable reference files:
- `master-context.md` — project history and context (this file)
- `supabase-schema-v2.0.0.sql` — full initial DB schema
- `supabase-v2.X.0.sql` — incremental SQL for each version
- `cloudflare-worker-v2.0.0.js` — Cloudflare Worker script (update on changes, add new version suffix)
- `firebase-rtdb-rules-v2.0.0.json` — RTDB security rules (update on changes)

**Rule:** Every time a new version creates or updates these files, save them in `creator-guide/` with the version suffix: `filename-v2.X.0.ext`. Never delete old versions — append new ones. Each future AI MUST save updated versions of these files to `creator-guide/` at the end of their version.

### Files changed in v2.4.0
- `src/pages/Projects.jsx` — FULL BUILD (was placeholder)
- `src/pages/ProjectDetail.jsx` — FULL BUILD (was placeholder)
- `src/components/home/RecentProjects.jsx` — dynamic-only, no fallback, accent_color support
- `src/services/supabase.js` — many additions/fixes
- `src/utils/formatters.js` — added formatDistanceToNow, formatDateShort
...
- `src/components/shared/ReportButton.jsx` — NEW
- `src/components/shared/CommentSection.jsx` — NEW
- `creator-guide/supabase-v2.4.0.sql` — NEW
- `creator-guide/supabase-schema-v2.0.0.sql` — added (was missing from ZIP)
- `creator-guide/cloudflare-worker-v2.0.0.js` — added (was missing from ZIP)
- `creator-guide/firebase-rtdb-rules-v2.0.0.json` — added (was missing from ZIP)

### Notes for next AI (v2.5.0)
- v2.5.0 = Feed page (combined blogs + posts). Uses `feed` table (type: 'blog'|'post').
- Supabase `feed` table now has `accent_color` column (added in v2.4.0 SQL).
- Feed detail page needs ReadingProgress bar + TableOfContents (blog type only).
- GitHub cached data plan still pending (see v2.3.4 notes). Can be done as v2.4.x minor fix.
- `VisibilityGuard` in `Projects.jsx` uses `page="projects"` + `skeleton="grid"` — ensure About page uses same pattern.
- `ShareButtons` uses `faXTwitter` from `@fortawesome/free-brands-svg-icons` — verify it's in the package.
- Comment system: `checkCommentRateLimit` queries by `user_id + date`. Ensure Supabase RLS allows authenticated users to SELECT their own comments count.
- Views RPC: if `increment_project_views` RPC call fails, there's a fallback block in `incrementProjectViews()` that won't work correctly (uses wrong syntax) — the RPC is the correct path, just make sure SQL script is run.
