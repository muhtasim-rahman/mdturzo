// ProjectsPage.jsx — v2.4.6
// Changes from v2.4.5:
//   - Controls layout: search bar LEFT, sort+view RIGHT on lg screens
//   - Custom styled select (proj-select CSS class) — dark bg in dark mode
//   - 2 new sort options: featured_first, az (alphabetical)
//   - Small screen: right controls wrap below search on sm breakpoint
//   - Skeleton for controls row + category filter while loading
//   - Progressive card reveal: first 6 shown, then +6 every 280ms
//   - Banner (header) always loads first, controls + cards after data
//   - EmptyState replaced with shared NoProjectsPlaceholder
//   - SkeletonGrid list rows updated to match new list card height
...
import {
  ...
  faSort, faLayerGroup, faChevronDown
...
// ── Controls skeleton (shown while loading) ───────────────────
  ...
  return (
    <motion.div
      ...
      <div className="flex items-center gap-2 mb-6 pb-4 border-b border-[var(--border-color)]">
        <div className="sk h-3 w-12 rounded mr-1 hidden sm:block" />
        {[60, 44, 72, 56, 80, 48, 64].map((w, i) => (
          <div key={i} className="sk rounded-xl flex-shrink-0" style={{ height: 32, width: w }} />
        ))}
      </div>
    </motion.div>
  )
...
function SkeletonGrid({ view, count = 6 }) {
  ...
  return view === 'grid' ? (
    <div className="proj-page-grid">
      {items.map(i => (
        <div key={i}
          className="bg-[var(--bg-surface)] rounded-2xl overflow-hidden border border-[var(--border-color)]">
    ...
    <div className="space-y-2.5">
      {items.map(i => (
        <div key={i}
          className="flex bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl overflow-hidden"
          style={{ minHeight: 108 }}>
          <div className="w-44 sm:w-52 flex-shrink-0 sk" style={{ animationDelay: `${i * 0.04}s` }} />
    ...
    <div className="flex items-center justify-center gap-1.5 mt-10">
      ...
      {visible.reduce((acc, p, i, arr) => {
        if (i > 0 && arr[i - 1] !== p - 1) {
          acc.push(
            <span key={`e-${p}`}
              className="w-9 h-9 flex items-center justify-center text-[var(--text-tertiary)] text-xs">
              …
            </span>
          )
        }
...
// ── Styled select wrapper (dark mode aware) ───────────────────
function SortSelect({ value, onChange }) {
  return (
    <div className="relative flex items-center">
      <FontAwesomeIcon
        icon={faSort}
        className="absolute left-2.5 text-[10px] text-[var(--text-tertiary)] pointer-events-none z-10"
      />
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="proj-select pl-7"
      ...
      <FontAwesomeIcon
        icon={faChevronDown}
        className="absolute right-2 text-[9px] text-[var(--text-tertiary)] pointer-events-none"
      />
    </div>
  )
}
...
function ProjectsContent() {
  ...
  // Progressive reveal: start at 6, increment +6 every 280ms
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      return searchProjects(list, search)
    }
    ...
    if (sortBy === 'latest') {
      ...
      list.sort((a, b) => {
        if (b.featured === a.featured) return new Date(b.created_at) - new Date(a.created_at)
  ...
  // Reset page + visible count on filter change
  useEffect(() => {
    setPage(1)
    setVisibleCount(6)
  }, [search, activeCategory, sortBy, pageSize])

  // Also reset visibleCount on page navigation
  useEffect(() => { setVisibleCount(6) }, [page])

  // Progressive reveal: keep incrementing +6 until all paged cards visible
  useEffect(() => {
    if (visibleCount >= paged.length) return
    const t = setTimeout(() => {
      setVisibleCount(prev => Math.min(prev + 6, paged.length))
    }, 280)
  ...
  const visiblePaged = paged.slice(0, visibleCount)
  ...
  return (
    <>
      ...
      <div className="container-xl py-6 lg:py-10" ref={topRef}>
        ...
        {/* ── Banner / Page Header — always renders first ─────── */}
        <motion.div
          className="mt-5 mb-7"
        ...
        {/* ── Controls — skeleton while loading ────────────────── */}
        {loading ? (
          <ControlsSkeleton />
        ) : (
          <motion.div
            ...
            {/* Controls row:
                - sm+: flex-row with search left, sort+view right
                - xs:  flex-col, search full width; sort+view right-aligned below */}
            <div className="flex flex-col gap-3 mb-4">

              {/* Row 1: search (full width on mobile, flex-1 on sm+) */}
              {/* Row 2 on mobile / merged on sm+:  sort + view-toggle */}
              <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">

                {/* Search bar — always left on lg */}
                <div className="relative flex-1 group">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-20 py-2.5 rounded-xl
                      ...
                      outline-none text-xs text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] transition-all"
                  />
        ...
        {/* ── Cards / empty / error ─────────────────────────────── */}
        {loading ? (
          <SkeletonGrid view={view} count={pageSize > 6 ? 6 : pageSize} />
        ) : error ? (
          <div className="text-center py-20 border border-dashed border-[var(--border-color)] rounded-2xl">
            ...
            <p className="text-sm font-semibold text-[var(--text-secondary)]">Failed to load projects</p>
        ...
        {/* Pagination */}
        {!loading && totalPages > 1 && visibleCount >= paged.length && (
