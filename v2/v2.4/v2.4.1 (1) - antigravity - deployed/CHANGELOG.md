...
## v2.4.0 — Projects Page + Dynamic Featured + Full Detail

### New Pages
- **Projects.jsx** — Full projects list page
  - Grid / List view toggle
  - Category filter pills (dynamic from Supabase)
  - Within-page search (title + description + tags + category)
  - URL-synced filters (?q=, ?cat=) — bookmarkable
  - 12 per page with clean pagination
  - Empty state with clear-filters option
  - Full skeleton loading on all states
- **ProjectDetail.jsx** — Full project detail
  - Hero section (thumbnail or gradient fallback)
  - Like/Dislike with optimistic update
  - Share (native API + popup: Facebook, X, LinkedIn, WhatsApp, Telegram, copy link)
  - Report (login required, 6 reasons, modal)
  - Comments (login + email-verified, 1000 chars max, 10/day limit, pending approval)
  - Related projects grid (same category)
  - View counter (increment on load)
  - Sticky sidebar: links, stats, tags

### New Components
- `ProjectCard.jsx` — Grid + List variants, shared between home and projects page
- `Breadcrumb.jsx` — Reusable breadcrumb nav
- `LikeDislike.jsx` — Vote buttons with ratio bar
- `ShareButtons.jsx` — Multi-platform share popup
- `ReportButton.jsx` — Report modal
- `CommentSection.jsx` — Full comment system
- `RelatedContent.jsx` — Related projects grid

### Dynamic Home Page Projects
- `RecentProjects.jsx` rebuilt — reads `is_featured=true` from Supabase
- Uses `featured_order` for display order (1–6)
- Static fallback if no featured projects in DB
- Any change in Supabase reflects on home immediately

### Service Updates
- `supabase.js` — added: getFeaturedProjects, getRelatedProjects, getProjectCategories,
  incrementProjectViews, getUserLikeStatus, toggleLike, submitComment, deleteOwnComment,
  getCommentRateLimit, submitReport
- `formatters.js` — added: formatDistanceToNow, formatNumber

### CSS Additions (index.css)
- `.rp-grid` — 3/2/1 col responsive home featured grid
- `.proj-page-grid` — 3/2/1 col responsive projects list grid
- `.proj-card` — hover lift + border-color transition
- `.prose-content` — full rich text styles (h1-h4, p, a, ul, ol, blockquote, code, pre, table)
- `.breadcrumb-nav` — breadcrumb styles
- `.scrollbar-none` — hide scrollbar utility

## v2.4.1 — Projects Table Recreate, Image Preview Popup, Card & Page Redesign (2026-05-31)

### Added
- `creator-guide/supabase-schema-v2.4.1.sql` — SQL migration script to drop and recreate the `projects` table with rich details columns.
- `creator-guide/Projects-table-programmer.turzo-2.4.1.csv` — seed data CSV file containing all 19 projects and their custom metadata/images.
- `src/components/shared/ImagePreviewModal.jsx` — reusable full-screen image preview component featuring filename header, sharing utilities, swipe/arrow navigation, and body scroll-lock.

### Redesigned / Polished
- **Project Card Layout:** Ultra-premium, minimal look with gradient thumbnail overlays, unified styling, tags mapping (up to 3 and `+N`), floating details trigger, and hover translate/shadow lift.
- **Projects Catalog Page:** Redesigned header and controls. Responsive page sizing (12 PC, 8 Tablet, 6 Mobile). Added custom sorting (latest, oldest, most liked, most viewed) and a horizontally scrollable categories badge slider with mouse navigation buttons.
- **Project Detail Page:** Two-column visual layout. Left side renders metadata and title, description, content image click bubbles to preview, and comments. Right sticky column holds project links and a custom share dashboard (Facebook, LinkedIn, WhatsApp, X, Telegram, Email, and Copy Link).
- **Responsive Suggestions:** Suggested/Related projects section moves below the sticky card on PC, and above comments on Tablet/Mobile.

### Changed
- `src/config/site.config.js` — bumped version to `v2.4.1`.
- `RecentProjects.jsx` — updated static fallbacks to map correct image preview URLs.
