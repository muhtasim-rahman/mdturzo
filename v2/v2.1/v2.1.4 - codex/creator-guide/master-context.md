...
## v2.1.3 - Navbar/Footer fix using Codex

**Summary**
- Integrated the supplied navbar v2 and footer v9 designs into the React project.
- Updated `SITE_CONFIG.version` to `v2.1.3`.
- Navbar display name is `Muhtasim`; footer uses `Muhtasim Rahman`.
- Navbar/footer logos try `/logo.webp` first and fall back to `/android-chrome-192x192.png` because `logo.webp` was not available in the workspace.
- Active status dot is implemented in navbar/footer. Future Firebase RTDB status modes should support `active`, `busy`, `away`, and `offline`; current mode is hard-coded to `active`.
- Footer subscriber count uses the existing Firebase RTDB listener function. Full Firebase subscriber/status work remains for later versions.
- Desktop search popup and mobile/sidebar search placeholders are implemented. Full search engine remains for a later version.
- Mega menu header was removed; footer row now has share buttons, current URL copy, and version badge.
- Toast stack is capped at 3 with 3-second default timeout.
...
**Verification Note**
`npm run build` could not be verified because `vite` was missing and `npm install` did not finish within the available command timeout.

**Verification Update**
- Added `public/logo.webp` from the sibling deploy copy.
...
## v2.1.4 - finalize navbar, search, and loading polish

> creator note: ai forgot to update this for v1.1.4, so creator addder this manually. next ai shound not avoid adding contexts lke fvious onsece...

- Replace loader with attached conic/grid CSS animation.
- Use first-load loader overlay and route-shaped skeletons after that.
- Add skeleton fallbacks for VisibilityGuard access checks.
- Polish navbar right-side custom tooltips.
- Remove top navbar inner padding at 1250px+ screens.
- Force active mega-menu icons and text to blue.
- Preserve scroll reset and progress on pathname/search changes.
- Finalize search, share, sidebar, click effect, and navbar/footer polish.
- Update v2.1.4 context notes.
