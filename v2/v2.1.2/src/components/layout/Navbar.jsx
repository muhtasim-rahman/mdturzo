// ============================================================
// NAVBAR — v2.1.2
// Changes: name → "Md Turzo", T-logo with green dot corner,
//   floating navbar advanced glass effect, mega nav minimal,
//   icon tooltips (data-tooltip), nav links title attr
// ============================================================
...
const megaVariants = {
  ...
  exit:    { opacity: 0, y: -8, scaleY: 0.97, transition: { duration: 0.14 } },
}
...
// ── Logo Component (T image + green dot + name) ─────────────
function NavLogo({ size = 'md', showName = true, onClick }) {
  const logoSize = size === 'sm' ? 'w-7 h-7 text-sm rounded-lg' : 'w-9 h-9 text-base rounded-xl'
  const dotSize  = size === 'sm' ? 'w-2.5 h-2.5 border-[2px]' : 'w-3 h-3 border-2'
  return (
    <Link to="/" onClick={onClick}
      className="flex-shrink-0 flex items-center gap-2.5 select-none group">
      {/* T Avatar */}
      <div className={`relative ${logoSize} flex items-center justify-center flex-shrink-0`}
        style={{ background: 'linear-gradient(135deg,#6366f1,#3b82f6)' }}>
        <span className="text-white font-bold">T</span>
        {/* Green active dot on corner */}
        <span className={`absolute -bottom-0.5 -right-0.5 ${dotSize} rounded-full bg-[var(--clr-success)] border-[var(--bg-page)]`} />
      </div>
      {/* Name */}
      {showName && (
      ...
      <motion.div variants={megaVariants} initial="hidden" animate="visible" exit="exit"
        ...
        <div className="max-w-[1120px] mx-auto px-4 pt-2">
          <div className="rounded-2xl bg-[var(--bg-surface)] border border-[var(--border-color)] shadow-[var(--shadow-xl)] overflow-hidden">
            ...
            <div className="h-[1.5px] w-full" style={{background:'linear-gradient(90deg,transparent,rgba(99,102,241,0.4) 40%,rgba(59,130,246,0.4) 60%,transparent)'}} />
            <div className="grid grid-cols-4 gap-0 p-1">
              {MEGA_COLS.map((col, ci) => (
                <div key={col.label} className={`p-4 ${ci < 3 ? 'border-r border-[var(--border-color)]' : ''}`}>
                  {/* Category header */}
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--text-tertiary)] mb-3 px-2">{col.label}</p>
                  <div className="space-y-0.5">
                    {col.items.map(item => (
                      <Link key={item.path} to={item.path} onClick={onClose}
                        className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-[var(--bg-surface-2)] transition-colors group">
                        <FontAwesomeIcon icon={item.icon}
                          className="w-3.5 text-center text-[var(--text-tertiary)] group-hover:text-[var(--accent-primary)] transition-colors flex-shrink-0" />
                        <div className="min-w-0">
                          <p className="text-[13px] font-medium text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition-colors leading-tight">{item.label}</p>
                          <p className="text-[11px] text-[var(--text-tertiary)] mt-0.5 truncate leading-none">{item.desc}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
...
// ── Nav Right Icons ─────────────────────────────────────────
function NavRight({ user, profile, isAdmin: isAdminProp, avatar, displayName, isLoggedIn, authLoading, unreadCount, openSearch, notifOpen, setNotifOpen, userOpen, setUserOpen, megaOpen, setMegaOpen, onMenuOpen }) {
  return (
        ...
        {floatVisible && (
          <motion.div
            ...
            <nav className="float-nav relative pointer-events-auto flex items-center gap-4 w-full max-w-[1120px] h-[52px] px-4 rounded-full">
              ...
              {megaOpen && <div className="absolute inset-x-0 top-full"><MegaMenu onClose={() => setMegaOpen(false)} /></div>}
            </nav>
          </motion.div>
        )}
      </AnimatePresence>
