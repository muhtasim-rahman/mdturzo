...
// APP.JSX — v2.3.2
// Changes:
//   - ClickEffect: minimal burst, light/dark aware, expanded selector
//   - SamePageScrollTop: clicking same-page link scrolls to top smoothly
// ============================================================
...
// ── Click Effect ─────────────────────────────────────────────
// Minimal burst on all interactive elements — light/dark aware
...
// When clicking a link pointing to the current page: scroll to top,
// no reload, no navigation event.
function SamePageScrollTop() {
  const location = useLocation()

  useEffect(() => {
    const normalize = (p) => {
      const base = (p === '/home' ? '/' : p).split('?')[0].split('#')[0]
    ...
    const onClick = (e) => {
      ...
      if (normalize(href) === normalize(location.pathname)) {
        e.preventDefault()
