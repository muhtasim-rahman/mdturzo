// Hero.jsx -- v2.2.8
// CHANGES:
//   * Large screen .himg-frame: bg, border, shadow, top-bar (::before), inner-gradient (::after) ALL removed
//     -- image appears floating with no frame decoration on desktop
//   * himg-box height increased ~35px across all desktop breakpoints
//   * "Available for hire" badge removed entirely
//   * Tablet (640-899px): name is single line "Muhtasim Rahman (Turzo)"
//   * Mobile (<640px): "(Turzo)" is smaller text on its own line below
//   * Tablet/mobile circular frame: alt accent-tinted bg added
//   * Bottom gradient, glow, particles, stars all unchanged
...
.himg-box{
  ...
  /* +35px from v2.2.7 heights */
  height:clamp(375px,40vw,615px);
  flex-shrink:0;
}

/* Glow behind image -- unchanged */
.himg-glow{
...
/* Image frame -- DESKTOP: no bg, no border, no shadow, no pseudo-elements */
.himg-frame{
  position:relative;z-index:1;
  width:100%;height:100%;
  border-radius:20px;
  overflow:hidden;
  /* Desktop: completely transparent frame -- image floats on its own */
  background:transparent;
  border:none;
  box-shadow:none;
}

/* Remove top bar and bottom gradient on desktop */
.himg-frame::before,
.himg-frame::after{
  display:none;
}
...
[data-theme=dark] .himg-photo{filter:drop-shadow(0 8px 40px rgba(0,0,0,.5))}
[data-theme=light] .himg-photo{filter:drop-shadow(0 8px 32px rgba(15,23,42,.18))}
...
/* md: <899px — single column, round frame, accent bg */
@media(max-width:899px){
  ...
  .himg-frame{
    border-radius:50%;
    /* Alt accent-tinted bg for circular frame */
    background:color-mix(in srgb, var(--accent-primary) 14%, var(--bg-surface-2));
    border:2px solid color-mix(in srgb, var(--accent-primary) 22%, var(--border-color));
  ...
  [data-theme=light] .himg-frame{
    background:color-mix(in srgb, var(--accent-primary) 10%, #f0f4f8);
  }
  .himg-glow{border-radius:50%}
}
@keyframes hring-p{
  0%,100%{box-shadow:0 0 0 4px var(--bg-page),0 0 0 6px var(--border-color),0 0 20px rgba(37,99,235,.1)}
  50%{box-shadow:0 0 0 4px var(--bg-page),0 0 0 6px #60A5FA,0 0 36px rgba(37,99,235,.26)}
}

/* sm: 640-899 -- name single line */
@media(min-width:640px) and (max-width:899px){
  .himg-box{width:clamp(200px,44vw,260px);height:clamp(200px,44vw,260px)}
  /* Single-line name on tablet */
  .hname{font-size:clamp(1.6rem,4vw,2.4rem);white-space:nowrap}
  .hname-line{display:inline;overflow:visible;line-height:1.2}
  .hname-line:first-child::after{content:' '}
  .hname-nick{font-size:.38em;vertical-align:middle;display:inline}
}

/* xs: <640 -- (Turzo) on its own line, smaller */
