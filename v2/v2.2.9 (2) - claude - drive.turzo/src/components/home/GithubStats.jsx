    ...
    <motion.div className="gh-profile-card"
      ...
      <div className="gh-profile-left">
        ...
        <div className="gh-profile-meta">
          ...
          {profile.location && (
            <span className="gh-detail-item" style={{marginTop:3}}>
...
export default function GithubStats() {
  ...
  const load = useCallback(async (attempt = 0) => {
    ...
    try {
      ...
      if (profRes.status === 403 || profRes.status === 429) {
        ...
        if (attempt < 3) {
          // Retry up to 3 times every 5 seconds
          const timer = setTimeout(() => {
            setRetryCount(attempt + 1)
            load(attempt + 1)
          }, attempt < 3 ? 5000 : waitMs)
          setRetryTimer(timer)
          setLoading(false)
          setError(`rate_limited:${attempt}`)
          return
        } else {
          // After 3 tries, wait the ideal time
          const timer = setTimeout(() => {
            setRetryCount(0)
            load(0)
          }, waitMs)
  ...
  const isRateLimitedWait = error === 'rate_limited_wait'
        ...
        {!loading && isRateLimited && !profile && (
          <div className="flex flex-col items-center py-14 gap-3 text-center">
            <p className="text-[var(--text-secondary)] text-sm">
              {isRateLimitedWait
                ? 'GitHub API rate limited. Auto-retrying when the limit resets…'
                : `GitHub API rate limited. Retrying… (${retryCount}/3)`}
            </p>
            <div className="flex items-center gap-2 text-[var(--accent-primary)] text-xs font-mono">
              <FontAwesomeIcon icon={faRotate} className="animate-spin" />
              {isRateLimitedWait ? 'Waiting for rate limit reset' : 'Retrying in 5s'}
            </div>
