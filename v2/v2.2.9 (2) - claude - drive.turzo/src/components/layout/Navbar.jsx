...
export function Navbar() {
  ...
  useEffect(() => {
    const onScroll = () => {
      const visible = window.scrollY > FLOAT_THRESHOLD
      setFloatVisible(visible)
      // Close mega nav whenever user scrolls
      setMegaOpen(false)
    }
    ...
    <>
      ...
      */}
      <AnimatePresence>
        {megaOpen && (
          <>
            {/* Full-width invisible backdrop — clicking empty sides closes mega nav */}
            <div
              key="mega-backdrop"
      ...
      <style>{`
        ...
        [data-nb-tip]::after {
          ...
          top: calc(100% + 8px);
          left: 50%;
          transform: translateX(-50%) translateY(4px) scale(0.93);
