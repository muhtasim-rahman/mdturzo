...
> v2.3.6 এ already হয়ে গেছে (tai এখানে repeat korar dorkar nai): per-section
> `SectionErrorBoundary` + `SectionReveal` infra (`components/ui/Section.jsx`),
> shared `components/projects/ProjectCard.jsx`, DevBanner, theme default light,
> About→Supabase pattern. v2.4.0 শুরু করার আগে `.creator/CONTEXT.md` এর v2.3.6
> entry পড়ে নেবে — কী already আছে clear হয়ে যাবে।
- Grid/List toggle (`ProjectCard.jsx` already shared — extend it, **নতুন কোরো না**),
  filter by category/tag, within-page weighted multi-field search
  (title/desc/tags সব মিলিয়ে score, ekta `utils/projectSearch.js`)
- Project detail — two-column layout (sticky meta sidebar + content),
  like/dislike/views/comment/report, share, related projects, breadcrumb
- Comments/Reviews/Likes/Report — generic `content_type`+`content_id` pattern
  (already schema-ready in v2.3.6's master SQL: `comments`, `likes`, `reports`
  tables) — ekhane just frontend wire up korte hbe, table notun lagbe na
- Image lightbox — zero-dependency custom (Framer Motion pinch-to-zoom),
  third-party lightbox lib install করার আগে চেক করো সেটা actually কাজ করে কিনা
- Every section/sub-section nijer `<Section name="..." skeleton={<X/>}>` দিয়ে
  wrap কোরো (v2.3.6 এর pattern follow করো) — ekta section crash hole baki page
  bachte hbe
- **Schema discipline:** projects table যতটুকু লাগে ঠিক ততটুকু column রাখো —
  আগের চেষ্টায় ৬০+ column এর "mega schema" বানানোর পর সেটা maintain করা কঠিন
  হয়ে গিয়েছিল, যেটা পরের বাগগুলোর একটা কারণ। ছোট, focused schema → পরে দরকার
  হলে migration দিয়ে column add করা সহজ।
- Card design: minimal রাখো, category অনুযায়ী অতিরিক্ত আলাদা রঙ না দিয়ে uniform
  accent থিম (CAT_COLORS শুধু একটা subtle badge tint এর জন্য, পুরো card জুড়ে নয়)
- **Process lesson:** ছোট ছোট testable change করে এগিও, একসাথে অনেক কিছু rewrite
  কোরো না — v2.4.1 এর প্রথম দিকের একটা bug পরের ১০+ version জুড়ে compound হয়ে
  এখানেই পুরো rollback করতে হয়েছিল। প্রতি commit এর পরে CONTEXT.md সাথে সাথে
  update করো, পরে "মনে করে লেখার" দরকার না পড়ুক

### v2.5.0 — Feed Page (Blogs + Posts unified)
> v2.0.0 এর original schema এ আসলে একটাই `feed` table ছিল (type: blog|post) —
> সেটাই v2.3.6 এ restore করা হয়েছে। তাই এই version এর কাজ হলো এই single table
> এর উপর সুন্দর একটা ফিড frontend বানানো, blogs/posts আলাদা table এ split করার
> দরকার নাই (আগের চেষ্টায় split করে পরে আবার জটিলতা বেড়েছিল)।
- Facebook/LinkedIn-style hybrid feed — single scroll, type badge (Blog/Post)
  per card, media grid for posts with images/video
- Blog-type items: Reading Progress Bar + Table of Contents sidebar
- Pinned items, filter by type/category/tag, search (same weighted approach
  as Projects এর search engine থেকে reuse করো — duplicate কোরো না)
- like/dislike/views/comment/report/share/related — Projects এ যেই shared
  components বানিয়েছ (CommentSection, LikeDislike, ShareButtons, ReportButton,
  ImagePreviewModal/Carousel) ওগুলোই এখানে reuse করো, নতুন বানানোর দরকার নাই
- Comment limit: max 1000 char, login + email-verified, 10/day limit, pending
  → admin approves (already decided — দ্রঃ "Small edit by user" section নিচে)
- View dedup: একই user/device বারবার view count না বাড়াক (device fingerprint +
  localStorage/IP heuristic) — generic utility বানাও, Projects এর view count
  এও একই utility ব্যবহার করবে
- এই version Posts ফিচারও absorb করে নিয়েছে, তাই v2.6.0 শুধু Contact Page থাকবে
  (নিচের v2.6.0 section অপরিবর্তিত আছে, কিন্তু Posts অংশটা আসলে এখানেই হয়ে যাবে)
